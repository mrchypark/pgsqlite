...
async fn test_batch_delete_single_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_delete_users (id INTEGER PRIMARY KEY, name TEXT, status TEXT)")
        .await?;
    ...
    let select_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_users")
        .await?;
    let count: i32 =
        String::from_utf8(select_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
    assert_eq!(count, 7); // 10 - 3 = 7 remaining

    // Verify specific rows were deleted
    let remaining_result = db_handler
        .query("SELECT id FROM batch_delete_users WHERE id IN (2, 4, 6)")
        .await?;
...
async fn test_batch_delete_multi_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = db_handler.execute(query).await?;
    println!(
        "Multi-column batch delete affected {} rows",
    ...
    // Verify remaining rows
    let count_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_products")
        .await?;
    let count: i32 =
        String::from_utf8(count_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
    assert_eq!(count, 4); // 6 - 2 = 4 remaining

    // Verify specific rows were deleted
    let check_result = db_handler
        .query("SELECT id FROM batch_delete_products WHERE id IN (1, 5) ORDER BY id")
        .await?;
...
async fn test_batch_delete_with_quotes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_delete_quotes (id INTEGER PRIMARY KEY, description TEXT)")
        .await?;
    ...
    // Verify remaining rows
    let remaining_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_quotes")
        .await?;
    let count: i32 =
        String::from_utf8(remaining_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
...
async fn test_batch_delete_no_alias() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_delete_simple (id INTEGER PRIMARY KEY, value INTEGER)")
        .await?;
    ...
    for i in 1..=5 {
        db_handler
            .execute(&format!(
                "INSERT INTO batch_delete_simple VALUES ({}, {})",
                i,
                i * 10
            ))
            .await?;
    ...
    let result = db_handler.execute(query).await?;
    println!(
        "No alias batch delete affected {} rows",
        result.rows_affected
    );
    ...
    // Verify remaining rows
    let remaining_result = db_handler
        .query("SELECT id FROM batch_delete_simple ORDER BY id")
        .await?;
...
async fn test_batch_delete_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_delete_perf (id INTEGER PRIMARY KEY, value INTEGER)")
        .await?;
    ...
    for i in 1..=1000 {
        db_handler
            .execute(&format!(
                "INSERT INTO batch_delete_perf VALUES ({}, {})",
                i,
                i * 10
            ))
            .await?;
    ...
    let count_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_perf")
        .await?;
    let count: i32 =
        String::from_utf8(count_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
    ...
    // Verify specific rows were deleted
    let check_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_perf WHERE id <= 100")
        .await?;
    let deleted_check: i32 =
        String::from_utf8(check_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
...
async fn test_regular_delete_still_works() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE regular_delete_test (id INTEGER PRIMARY KEY, name TEXT)")
        .await?;

    // Insert test data
    db_handler
        .execute("INSERT INTO regular_delete_test VALUES (1, 'Keep'), (2, 'Delete')")
        .await?;
    ...
    let select_result = db_handler
        .query("SELECT name FROM regular_delete_test")
        .await?;
...
async fn test_batch_delete_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_delete_edge (id INTEGER PRIMARY KEY, data TEXT)")
        .await?;
    ...
    for i in 1..=5 {
        db_handler
            .execute(&format!(
                "INSERT INTO batch_delete_edge VALUES ({i}, 'data{i}')"
            ))
            .await?;
    }
    ...
    let result = db_handler.execute(query).await?;
    println!(
        "Edge case batch delete affected {} rows",
        result.rows_affected
    );
    ...
    let count_result = db_handler
        .query("SELECT COUNT(*) FROM batch_delete_edge")
        .await?;
    let count: i32 =
        String::from_utf8(count_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
    assert_eq!(count, 5);
