...
async fn test_pg_attribute_enum_types() {
    ...
    client
        .simple_query(
            "CREATE TABLE test_enum_table (id INTEGER PRIMARY KEY, current_status status)",
        )
        .await
    ...
    let enum_info = client
        .query("SELECT type_oid, type_name FROM __pgsqlite_enum_types", &[])
        .await
        .unwrap();
    ...
    assert_eq!(
        table_oid_rows.len(),
        1,
        "Should find test_enum_table in pg_class"
    );
    ...
    if rows.is_empty() {
        ...
        let any_attrs = client
            .query("SELECT COUNT(*) FROM pg_catalog.pg_attribute", &[])
            .await
            .unwrap();
    ...
    let labels: Vec<String> = enum_rows
        .iter()
...
async fn test_pg_enum_filtering() {
    ...
    client
        .simple_query("CREATE TYPE size AS ENUM ('small', 'medium', 'large', 'extra_large')")
        .await
    ...
    let type_names: Vec<String> = type_rows
        .iter()
        .map(|row| row.get::<_, &str>(1).to_string())
        .collect();

    assert!(
        type_names.contains(&"color".to_string()),
        "Should find color enum"
    );
    assert!(
        type_names.contains(&"size".to_string()),
        "Should find size enum"
    );

    // Filter to just our test types
    let our_types: Vec<_> = type_rows
        .iter()
        .filter(|row| {
            let name: &str = row.get(1);
    ...
    let labels: Vec<String> = color_values
        .iter()
        .map(|row| row.get::<_, &str>(0).to_string())
        .collect();
