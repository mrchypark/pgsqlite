...
async fn test_batch_insert_with_datetime_functions() {
    ...
    assert!(
        result.is_ok(),
        "Should handle datetime functions in batch INSERT"
    );
...
async fn test_single_insert_with_datetime_functions() {
    ...
    let count = client
        .query_one("SELECT COUNT(*) FROM single_datetime_test", &[])
        .await
        .unwrap();
    let count_val: i64 = count.get(0);
...
async fn test_mixed_datetime_values_and_functions() {
    ...
    assert!(
        result.is_ok(),
        "Should handle mixed datetime values and functions"
    );

    // Verify all rows were inserted
    let count = client
        .query_one("SELECT COUNT(*) FROM mixed_datetime_test", &[])
        .await
        .unwrap();
    let count_val: i64 = count.get(0);
