use crate::translator::metadata::{ColumnTypeHint, TranslationMetadata};
...
impl RowToJsonTranslator {
    ...
    fn translate_subquery_pattern(
        query: &str,
        metadata: &mut TranslationMetadata,
    ) -> Option<String> {
        use regex::Regex;
            ...
            && let Some(captures) = re.captures(query)
        {
            ...
            if let Some(columns) = Self::extract_columns_from_select(subquery) {
                ...
                let json_fields: Vec<String> = columns
                    .iter()
                ...
                let result = query.replace(&format!("row_to_json({alias})"), &json_construction);
...
mod tests {
    ...
    fn test_contains_row_to_json_call() {
        assert!(RowToJsonTranslator::contains_row_to_json_call(
    ...
    fn test_extract_columns_from_select() {
        let columns =
            RowToJsonTranslator::extract_columns_from_select("SELECT name, age FROM users");
        assert_eq!(columns, Some(vec!["name".to_string(), "age".to_string()]));

        let columns = RowToJsonTranslator::extract_columns_from_select(
            "SELECT name AS user_name, age FROM users",
        );
        assert_eq!(
            columns,
            Some(vec!["user_name".to_string(), "age".to_string()])
        );
    }
    ...
    fn test_translate_subquery_pattern() {
        ...
        assert!(
            result.is_some(),
            "Expected translation result for query: {query}"
        );
