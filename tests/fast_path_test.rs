...
async fn test_fast_path_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "100 INSERTs took: {:?} ({:.3}ms per insert)",
        insert_duration,
        insert_duration.as_secs_f64() * 1000.0 / 100.0
    );
    ...
    println!(
        "100 SELECTs took: {:?} ({:.3}ms per select)",
        select_duration,
        select_duration.as_secs_f64() * 1000.0 / 100.0
    );
    ...
    for i in 0..100 {
        db_handler
            .execute(&format!(
                "INSERT INTO products (name, price) VALUES ('product{i}', {i}.99)"
            ))
            .await?;
    }
    let decimal_insert_duration = start.elapsed();
    println!(
        "100 DECIMAL INSERTs took: {:?} ({:.3}ms per insert)",
        decimal_insert_duration,
        decimal_insert_duration.as_secs_f64() * 1000.0 / 100.0
    );
    ...
    if insert_duration >= decimal_insert_duration {
        println!(
            "WARNING: Fast path INSERT ({insert_duration:?}) was not faster than decimal path ({decimal_insert_duration:?})"
        );
    }
...
async fn test_fast_path_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create tables
    db_handler
        .execute("CREATE TABLE simple (id INTEGER, name TEXT)")
        .await?;
    db_handler
        .execute("CREATE TABLE with_decimal (id INTEGER, price DECIMAL)")
        .await?;
