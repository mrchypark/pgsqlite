...
static INSERT_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?si)INSERT\s+INTO\s+(\w+)\s*\(([^)]+)\)\s*VALUES\s*(.+?)(?:\s+RETURNING\s+|;\s*$|$)",
    )
    .unwrap()
});
...
static INSERT_SELECT_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?si)INSERT\s+INTO\s+(\w+)\s*\(([^)]+)\)\s+SELECT\s+(.+)").unwrap());

// Pattern to match INSERT INTO table SELECT ... (without column list)
static INSERT_SELECT_NO_COLUMNS_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?si)INSERT\s+INTO\s+(\w+)\s+SELECT\s+(.+)").unwrap());
...
impl InsertTranslator {
    ...
    pub fn needs_translation(query: &str) -> bool {
        ...
        let has_sqlalchemy_values = query.contains("FROM (VALUES")
            && query.contains(") AS ")
    ...
    pub async fn translate_query(query: &str, db: &DbHandler) -> Result<String, String> {
        ...
        if let Some(caps) = INSERT_PATTERN.captures(query) {
            ...
            let converted_values =
                Self::convert_values_clause(values_str, &columns, &column_types)?;

            // Check if there's a RETURNING clause
            let returning_clause = if let Some(idx) = query.to_ascii_uppercase().find(" RETURNING ")
            {
                &query[idx..]
            } else {
                ""
            ...
            let converted_values =
                Self::convert_values_clause(values_str, &columns_refs, &column_types)?;
            ...
            let final_select = if Self::is_sqlalchemy_values_pattern(select_clause) {
                ...
                Self::translate_select_clause(select_clause, &columns, &column_types)?
            };

            debug!(
                "INSERT SELECT translation produced SELECT (len={})",
                final_select.len()
            );
            ...
            let columns_refs: Vec<&str> = columns.iter().map(|s| s.as_str()).collect();
            let converted_select =
                Self::translate_select_clause(select_clause, &columns_refs, &column_types)?;
    ...
    async fn get_all_columns_and_types(
        db: &DbHandler,
        table_name: &str,
                        ...
                        if paren_depth == 0 {
                            ...
                            if row_content.starts_with('(') && row_content.ends_with(')') {
                                ...
                                if values.len() != columns.len() {
                                    ...
                                    return Err(format!(
                                        "Column count mismatch in row {}: {} columns but {} values",
                                        row_num,
                                        columns.len(),
                                        values.len()
                                    ));
                                ...
                                for (i, value) in values.iter().enumerate() {
                                    ...
                                    let converted = if let Some(pg_type) =
                                        column_types.get(&column_name.to_lowercase())
                                    {
    ...
    fn convert_value(value: &str, pg_type: &str) -> Result<String, String> {
        ...
        match pg_type.to_lowercase().as_str() {
            "date" => match ValueConverter::convert_date_to_unix(unquoted) {
                Ok(days) => Ok(days),
                Err(e) => Err(format!(
                    "Invalid date value '{unquoted}': {e}. Expected format: YYYY-MM-DD"
                )),
            },
            "time" => match ValueConverter::convert_time_to_seconds(unquoted) {
                Ok(micros) => Ok(micros),
                Err(e) => Err(format!(
                    "Invalid time value '{unquoted}': {e}. Expected format: HH:MM:SS[.ffffff]"
                )),
            },
            "timestamp" => match ValueConverter::convert_timestamp_to_unix(unquoted) {
                Ok(micros) => Ok(micros),
                Err(e) => Err(format!(
                    "Invalid timestamp value '{unquoted}': {e}. Expected format: YYYY-MM-DD HH:MM:SS[.ffffff]"
                )),
            },
    ...
    fn translate_select_clause(
        ...
        for (i, expr) in expressions.iter().enumerate() {
            let column_name = columns[i];
            let converted_expr =
                if let Some(pg_type) = column_types.get(&column_name.to_lowercase()) {
    ...
    fn convert_sqlalchemy_values_to_union(
        ...
        debug!(
            "Parsed {} type casts from SQLAlchemy SELECT",
            type_casts.len()
        );
        ...
        let as_pos = values_end_search
            .find(") AS")
            .ok_or("End of VALUES not found")?;
        ...
        if let Some(order_pos) = select_clause.find(" ORDER BY ") {
            let order_end = select_clause[order_pos..]
                .find(" RETURNING ")
                .map(|p| order_pos + p)
    ...
    fn parse_sqlalchemy_type_casts(
    ...
    fn convert_select_expression(expr: &str, pg_type: &str) -> Result<String, String> {
        ...
        let needs_datetime_conversion = matches!(
            pg_type.to_lowercase().as_str(),
    ...
    fn convert_datetime_literal(literal: &str, pg_type: &str) -> Result<String, String> {
        match pg_type.to_lowercase().as_str() {
            "date" => match ValueConverter::convert_date_to_unix(literal) {
                Ok(days) => Ok(days),
                Err(e) => Err(format!("Invalid date value '{literal}': {e}")),
            },
            "time" => match ValueConverter::convert_time_to_seconds(literal) {
                Ok(micros) => Ok(micros),
                Err(e) => Err(format!("Invalid time value '{literal}': {e}")),
            },
            "timestamp" => match ValueConverter::convert_timestamp_to_unix(literal) {
                Ok(micros) => Ok(micros),
...
mod tests {
    ...
    fn test_parse_values() {
        let values =
            InsertTranslator::parse_values("1, 'hello', '2024-01-15', 'it''s fine'").unwrap();
    ...
    fn test_needs_translation() {
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (date_col) VALUES ('2024-01-15')"
        ));
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (time_col) VALUES ('14:30:00')"
        ));
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (arr_col) VALUES ('{1,2,3}')"
        ));
    }
    ...
    (NULL, NULL, NULL);"#;
        ...
        assert!(
            INSERT_PATTERN.is_match(query),
            "Regex should match multi-line INSERT"
        );
    ...
    fn test_needs_translation_array_types() {
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (arr_col) VALUES (ARRAY[1,2,3])"
        ));
        assert!(!InsertTranslator::needs_translation(
            "INSERT INTO test (id) VALUES (1)"
        ));
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (date_col) SELECT '2024-01-15' FROM source"
        ));
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test SELECT '2024-01-15', NOW() FROM source"
        ));
        assert!(InsertTranslator::needs_translation(
            "INSERT INTO test (time_col) SELECT CURRENT_TIME FROM source"
        ));
        assert!(!InsertTranslator::needs_translation(
            "INSERT INTO test SELECT id, name FROM source"
        ));
    }
    ...
    fn test_parse_select_expressions() {
        ...
        let expressions =
            InsertTranslator::parse_select_expressions("id, name, '2024-01-15'").unwrap();
        assert_eq!(expressions, vec!["id", "name", "'2024-01-15'"]);

        // Test expressions with function calls
        let expressions =
            InsertTranslator::parse_select_expressions("id, UPPER(name), NOW()").unwrap();
        assert_eq!(expressions, vec!["id", "UPPER(name)", "NOW()"]);

        // Test complex expressions with nested commas
        let expressions = InsertTranslator::parse_select_expressions(
            "id, COALESCE(date_col, '2024-01-01'), name",
        )
        .unwrap();
        assert_eq!(
            expressions,
            vec!["id", "COALESCE(date_col, '2024-01-01')", "name"]
        );
        ...
        assert_eq!(
            expressions,
            vec!["p0::VARCHAR", "p1::TEXT", "p2::TIMESTAMP WITHOUT TIME ZONE"]
        );
    }
    ...
    fn test_convert_select_expression() {
        ...
        let result = InsertTranslator::convert_select_expression(
