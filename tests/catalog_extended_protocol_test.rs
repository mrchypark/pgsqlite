...
async fn test_catalog_extended_protocol() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        // Create test table
        db_handler
            .execute("CREATE TABLE test_table1 (id INTEGER PRIMARY KEY, name TEXT)")
            .await
            .unwrap();
        ...
        use pgsqlite::protocol::{
            AuthenticationMessage, BackendMessage, FrontendMessage, PostgresCodec,
            TransactionStatus,
        };
                ...
                Some(Ok(msg)) => {
                    ...
                    match msg {
                        ...
                        FrontendMessage::Describe { typ, name } => {
                            ...
                            framed
                                .send(BackendMessage::ParameterDescription(vec![]))
                                .await
                                .unwrap();
                        ...
                        FrontendMessage::Execute { max_rows, .. } => {
                            ...
                            // Send data row
                            let row = vec![Some(b"test_table1".to_vec()), Some(b"r".to_vec())];
