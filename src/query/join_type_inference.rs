...
static COLUMN_WITH_TABLE_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)(\w+)\.(\w+)").unwrap());
    ...
    for cap in column_pattern.captures_iter(query) {
        ...
        if let Some(alias) = cap.get(3) {
            ...
            if alias_str == format!("{table}_{column}") {
                ...
                mapping.insert(alias_str.to_string(), table.clone());
                info!(
                    "JOIN mapping: SQLAlchemy pattern detected '{}' for {}.{}",
                    alias_str, table, column
                );
            }
        }
...
mod tests {
    ...
    fn test_column_to_table_mapping() {
        ...
        assert_eq!(
            mapping.get("order_items_unit_price"),
