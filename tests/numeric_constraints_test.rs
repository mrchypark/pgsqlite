...
fn test_numeric_constraint_validator() {
    ...
    assert!(
        validator
            .validate_value("test", "amount", "99999999.99")
            .is_ok()
    );
    ...
    assert!(
        validator
            .validate_value("test", "amount", "99999999.999")
            .is_err()
    );
    ...
    assert!(
        validator
            .validate_value("test", "amount", "123.456")
            .is_err()
    );
}
...
fn test_numeric_formatting() {
    ...
    assert_eq!(
        validator.format_value("test", "four_dec", "123"),
        "123.0000"
    );
    assert_eq!(
        validator.format_value("test", "four_dec", "123.456789"),
        "123.4568"
    ); // Rounded
...
fn test_numeric_triggers() {
    ...
    conn.execute("INSERT INTO products (price) VALUES (99.99)", [])
        .unwrap();
    conn.execute("INSERT INTO products (price) VALUES (0.01)", [])
        .unwrap();
    ...
    // Test updates
    conn.execute("UPDATE products SET price = 88.88 WHERE id = 1", [])
        .unwrap();
