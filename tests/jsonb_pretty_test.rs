...
async fn test_jsonb_pretty_simple_object_integration() {
    ...
    let rows = client
        .simple_query(r#"SELECT jsonb_pretty('{"name":"John","age":30,"active":true}') AS result"#)
        .await
        .unwrap();
...
async fn test_jsonb_pretty_edge_cases_integration() {
    ...
    // Test empty object
    let rows = client
        .simple_query("SELECT jsonb_pretty('{}') AS result")
        .await
        .unwrap();
    ...
    // Test simple string
    let rows = client
        .simple_query(r#"SELECT jsonb_pretty('"hello world"') AS result"#)
        .await
        .unwrap();
    ...
    // Test number
    let rows = client
        .simple_query("SELECT jsonb_pretty('42') AS result")
        .await
        .unwrap();
    ...
    // Test null
    let rows = client
        .simple_query("SELECT jsonb_pretty('null') AS result")
        .await
        .unwrap();
    ...
    let rows = client
        .simple_query("SELECT jsonb_pretty('{not valid json}') AS result")
        .await
        .unwrap();
