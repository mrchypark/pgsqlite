...
impl JsonEachTranslator {
    ...
    pub fn contains_json_each(sql: &str) -> bool {
        ...
        sql_lower.contains("json_each(")
            || sql_lower.contains("jsonb_each(")
            || sql_lower.contains("json_each_text(")
            || sql_lower.contains("jsonb_each_text(")
    }
    ...
    pub fn translate_json_each(sql: &str) -> Result<String, PgSqliteError> {
        ...
        let json_each_text_from_regex =
            Regex::new(r"\bFROM\s+json_each_text\(([^)]+)\)\s+AS\s+(\w+)").unwrap();
...
mod tests {
    ...
    fn test_contains_json_each() {
        assert!(JsonEachTranslator::contains_json_each(
            "SELECT json_each(data) FROM table"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "FROM json_each(data) AS t"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "SELECT jsonb_each(data) FROM table"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "FROM jsonb_each(data) AS t"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "SELECT json_each_text(data) FROM table"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "FROM json_each_text(data) AS t"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "SELECT jsonb_each_text(data) FROM table"
        ));
        assert!(JsonEachTranslator::contains_json_each(
            "FROM jsonb_each_text(data) AS t"
        ));
        assert!(!JsonEachTranslator::contains_json_each(
    ...
    fn test_json_each_text_from_clause() {
        let sql =
            "SELECT key, value FROM json_each_text('{\"a\": 1, \"b\": true, \"c\": null}') AS t";
        let result = JsonEachTranslator::translate_json_each(sql).unwrap();
