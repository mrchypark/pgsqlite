...
impl NumericFormatTranslator {
    ...
    pub fn needs_translation(query: &str) -> bool {
        query.contains("::text")
            || query.to_uppercase().contains("CAST") && query.to_uppercase().contains("AS TEXT")
    }
    ...
    fn load_all_numeric_constraints(
        conn: &Connection,
    ) -> Result<HashMap<String, (i32, i32)>, rusqlite::Error> {
    ...
    /// Process ::text casts
    fn process_double_colon_casts(
        query: &str,
        constraints: &HashMap<String, (i32, i32)>,
    ) -> String {
        static CAST_REGEX: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)?)\s*::\s*text\b").unwrap());
    ...
    fn process_cast_syntax(query: &str, constraints: &HashMap<String, (i32, i32)>) -> String {
        static CAST_REGEX: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(?i)CAST\s*\(\s*(\w+(?:\.\w+)?)\s+AS\s+TEXT\s*\)").unwrap());
...
mod tests {
    ...
    fn test_numeric_format_translation() {
        ...
        assert_eq!(
            translated,
            "SELECT numeric_format(amount, 10, 2), numeric_format(tax, 5, 3) FROM prices"
        );
        ...
        assert_eq!(
            translated,
            "SELECT numeric_format(amount, 10, 2) FROM prices"
        );
        ...
        assert_eq!(
            translated,
            "SELECT numeric_format(prices.amount, 10, 2) FROM prices"
        );
