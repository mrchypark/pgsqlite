...
async fn test_array_literals() {
    ...
    let row = client
        .query_one(
            "SELECT int_array, text_array FROM array_data WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    // Test empty arrays
    let row = client
        .query_one("SELECT int_array FROM array_data WHERE id = 3", &[])
        .await
        .unwrap();
...
async fn test_array_operations() {
    ...
    for (op, desc) in unsupported_ops {
        match client
            .query(&format!("SELECT {op} FROM array_ops WHERE id = 1"), &[])
            .await
        {
...
async fn test_array_with_nulls() {
    ...
    // Test NULL array
    let row = client
        .query_one("SELECT data FROM nullable_arrays WHERE id = 3", &[])
        .await
        .unwrap();
