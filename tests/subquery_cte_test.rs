...
fn test_simple_subquery_in_select() {
    ...
    let sql =
        "SELECT name, (SELECT SUM(total) FROM orders WHERE customer_id = c.id) as total_orders
               FROM customers c";
    let result = rewrite_query(&conn, sql).unwrap();
...
fn test_derived_table_subquery() {
    ...
    assert!(
        inner_result.contains("decimal_from_text"),
        "Inner query should wrap AVG argument"
    );
    ...
    assert!(
        result.contains("decimal_gt"),
        "WHERE clause should use decimal_gt for avg_order > 100"
    );
}
...
fn test_expression_type_resolver_with_derived_table() {
    ...
    let avg_price_type = derived_cols
        .iter()
