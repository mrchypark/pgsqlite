...
async fn test_enum_cast_extended_protocol() {
    ...
    let stmt = client
        .prepare("INSERT INTO items (id, name, status) VALUES ($1, $2, $3::status)")
        .await
        .expect("Failed to prepare statement");

    client
        .execute(&stmt, &[&1i32, &"Item 1", &"active"])
        .await
        .expect("Failed to insert with parameterized cast");

    // Test parameterized SELECT with cast
    let stmt = client
        .prepare("SELECT $1::status as casted")
        .await
        .expect("Failed to prepare SELECT");

    let row = client
        .query_one(&stmt, &[&"pending"])
        .await
    ...
    let result = client.execute(&stmt, &[&2i32, &"Item 2", &"invalid"]).await;
