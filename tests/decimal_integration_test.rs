...
impl TestContext {
    async fn execute(
        &self,
        query: &str,
    ) -> Result<pgsqlite::session::DbResponse, pgsqlite::PgSqliteError> {
        self.db_handler
            .execute_with_session(query, &self.session_id)
            .await
    }
    ...
    async fn query(
        ...
        self.db_handler
            .query_with_session(query, &self.session_id)
            .await
    }
}
...
async fn test_decimal_arithmetic_execution() {
    ...
    let result = ctx
        .query("SELECT balance + 100 FROM accounts")
        .await
        .unwrap();
    ...
    // Test subtraction
    let result = ctx
        .query("SELECT credit_limit - balance FROM accounts")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 2);
    ...
    // Test multiplication
    let result = ctx
        .query("SELECT balance * 1.05 FROM accounts")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 2);
...
async fn test_decimal_comparisons() {
    ...
    let result = ctx
        .query("SELECT * FROM accounts WHERE balance = '200.00'")
        .await
        .unwrap();
    ...
    let result = ctx
        .query("SELECT * FROM accounts WHERE CAST(balance AS REAL) < 250")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 2);
    ...
    let result = ctx
        .query("SELECT * FROM accounts WHERE CAST(credit_limit AS REAL) > 750")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 2);
}
...
async fn test_decimal_aggregates() {
    ...
    // Test SUM
    let result = ctx
        .query("SELECT SUM(amount) FROM transactions WHERE account_id = 1")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
    ...
    // Test AVG
    let result = ctx
        .query("SELECT AVG(fee) FROM transactions")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
    ...
    // Test MIN/MAX
    let result = ctx
        .query("SELECT MIN(amount), MAX(amount) FROM transactions")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
}
...
async fn test_update_with_decimal_operations() {
    ...
    let result = ctx
        .execute("UPDATE accounts SET balance = balance * 1.1 WHERE name = 'Test'")
        .await
        .unwrap();
    ...
    // Verify the update
    let result = ctx
        .query("SELECT balance FROM accounts WHERE name = 'Test'")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
...
async fn test_decimal_precision_preservation() {
    ...
    let result = ctx
        .query("SELECT amount * fee / 100 FROM transactions")
        .await
        .unwrap();
...
async fn test_mixed_type_operations() {
    ...
    // Integer + Decimal
    let result = ctx
        .query("SELECT int_val + decimal_val FROM mixed_types")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
    ...
    // Decimal * Integer
    let result = ctx
        .query("SELECT decimal_val * int_val FROM mixed_types")
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 1);
}
...
async fn test_null_handling() {
    ...
    let result = ctx
        .query("SELECT balance + 100, credit_limit - balance FROM accounts WHERE name = 'NullTest'")
        .await
        .unwrap();
...
async fn test_error_handling() {
    ...
    let _result = ctx
        .query("SELECT balance / credit_limit FROM accounts WHERE name = 'Zero'")
        .await;
