...
async fn test_arithmetic_aliasing_simple_protocol() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute(
                "INSERT INTO products (id, price, quantity, tax_rate) VALUES (1, 99.99, 10, 0.08)",
            )
            .await
            .unwrap();
        db_handler
            .execute(
                "INSERT INTO products (id, price, quantity, tax_rate) VALUES (2, 149.50, 5, 0.08)",
            )
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT price * 1.1 AS price_with_markup FROM products WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT price + 10 adjusted_price FROM products WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_arithmetic_aliasing_extended_protocol() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        // With shared memory database, we can use execute() directly
        db_handler
            .execute("CREATE TABLE items (id INTEGER PRIMARY KEY, cost REAL, markup REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO items (id, cost, markup) VALUES (1, 50.0, 1.5)")
            .await
            .unwrap();
    ...
    let stmt = client
        .prepare("SELECT cost * markup AS selling_price FROM items WHERE id = $1")
        .await
        .unwrap();
    ...
    let stmt = client
        .prepare("SELECT cost * markup + 5 AS final_price FROM items WHERE id = $1")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[&1i32]).await.unwrap();
...
async fn test_arithmetic_no_alias() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        // With shared memory database, we can use execute() directly
        db_handler
            .execute("CREATE TABLE test_values (id INTEGER PRIMARY KEY, num REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO test_values (id, num) VALUES (1, 42.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT num * 2.0 AS result FROM test_values WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_arithmetic_integer_columns() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute(
                "CREATE TABLE inventory (id INTEGER PRIMARY KEY, quantity INTEGER, price INTEGER)",
            )
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO inventory VALUES (1, 10, 25)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT quantity * price AS total_value FROM inventory WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
