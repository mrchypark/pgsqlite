...
async fn test_insert_performance_comparison() {
    ...
    conn.execute(
        "CREATE TABLE baseline_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        [],
    )
    ...
    db.execute_with_session(
        "CREATE TABLE fast_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    println!(
        "  Overhead vs SQLite: {:.1}x",
        fast_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );

    // Test 3: pgsqlite with decimal table (slow path)
    println!("\n3. pgsqlite - Decimal table (slow path):");
    db.execute_with_session(
        "CREATE TABLE decimal_test (id INTEGER PRIMARY KEY, price DECIMAL(10,2), name TEXT)",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    println!(
        "  Overhead vs SQLite: {:.1}x",
        decimal_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    println!(
        "  Overhead vs fast path: {:.1}x",
        decimal_avg.as_secs_f64() / fast_avg.as_secs_f64()
    );
    ...
    println!(
        "  Overhead vs SQLite: {:.1}x",
        param_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    ...
    println!(
        "  Overhead vs SQLite: {:.1}x",
        pool_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    ...
    println!(
        "Fast path (no decimal): {:?} per INSERT ({:.1}x overhead)",
        fast_avg,
        fast_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    println!(
        "Slow path (decimal):    {:?} per INSERT ({:.1}x overhead)",
        decimal_avg,
        decimal_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    println!(
        "Parameterized:          {:?} per INSERT ({:.1}x overhead)",
        param_avg,
        param_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    println!(
        "Statement pool:         {:?} per INSERT ({:.1}x overhead)",
        pool_avg,
        pool_avg.as_secs_f64() / sqlite_avg.as_secs_f64()
    );
    ...
    println!(
        "\nMutex + protocol overhead: ~{:.1}µs per operation",
        mutex_overhead * 1_000_000.0
    );
