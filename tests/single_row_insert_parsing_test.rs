...
async fn test_single_row_insert_with_semicolon() {
    ...
    assert!(
        result.is_ok(),
        "Should handle INSERT with trailing semicolon"
    );

    // Verify row was inserted
    let row = client
        .query_one(
            "SELECT text_col, date_col FROM test_insert WHERE text_col = 'Test 1'",
            &[],
        )
        .await
        .unwrap();
    ...
    assert!(
        result2.is_ok(),
        "Should handle INSERT without trailing semicolon"
    );
