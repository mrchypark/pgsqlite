...
async fn test_basic_queries(client: &tokio_postgres::Client) {
    ...
    let rows = client
        .query("SELECT * FROM demo_table WHERE value > 500 LIMIT 100", &[])
        .await
    ...
    let start = Instant::now();
    let rows = client
        .query("SELECT * FROM demo_table ORDER BY value DESC LIMIT 50", &[])
        .await
        .expect("Failed to execute query");
    ...
    let rows = client
        .query(
            "SELECT category, COUNT(*), AVG(value) FROM demo_table GROUP BY category",
            &[],
        )
        .await
        .expect("Failed to execute query");
...
async fn test_memory_efficiency(client: &tokio_postgres::Client) {
    ...
    let start = Instant::now();
    let all_rows = client
        .query("SELECT * FROM demo_table ORDER BY id", &[])
        .await
    ...
    println!(
        "  Full fetch:    {:?} (~{} KB memory)",
        full_fetch_time,
        total_records * 50 / 1024
    );
    println!(
        "  Chunked fetch: {:?} (~{} KB peak memory)",
        total_chunk_time,
        chunk_size * 50 / 1024
    );
    println!(
...
async fn test_prepared_statements(client: &tokio_postgres::Client) {
    ...
    for i in 0..test_iterations {
        ...
        let _rows = client
            .query(&query, &[])
            .await
            .expect("Failed to execute dynamic query");
    }
    ...
    let stmt = client
        .prepare("SELECT * FROM demo_table WHERE category = $1 LIMIT 5")
        .await
    ...
    println!(
        "  Dynamic queries:  {:?} ({:.2}ms avg)",
        dynamic_time,
        dynamic_time.as_millis() as f64 / test_iterations as f64
    );
    println!(
        "  Prepared queries: {:?} ({:.2}ms avg)",
        prepared_time,
        prepared_time.as_millis() as f64 / test_iterations as f64
    );

    if prepared_time < dynamic_time {
        println!(
            "  🚀 Prepared statements are {:.1}x faster",
            dynamic_time.as_secs_f64() / prepared_time.as_secs_f64()
        );
    } else {
        println!(
            "  ⚠️  Dynamic queries were {:.1}x faster",
...
async fn connect_with_retry() -> (
    ...
    for attempt in 1..=10 {
        ...
        match tokio_postgres::connect("host=localhost port=5433 user=postgres dbname=test", NoTls)
            .await
        {
