...
async fn test_batch_update_with_values() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE batch_users (id INTEGER PRIMARY KEY, name TEXT, age INTEGER)")
        .await?;
    ...
    for i in 1..=5 {
        db_handler
            .execute(&format!(
                "INSERT INTO batch_users (id, name, age) VALUES ({}, 'User{}', {})",
                i,
                i,
                i * 10
            ))
    ...
    let select_result = db_handler
        .query("SELECT id, name, age FROM batch_users ORDER BY id")
        .await?;
...
async fn test_batch_update_single_column() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, price DECIMAL(10,2))")
        .await?;

    // Insert test data
    db_handler
        .execute("INSERT INTO products (id, name, price) VALUES (1, 'Widget', 10.99)")
        .await?;
    db_handler
        .execute("INSERT INTO products (id, name, price) VALUES (2, 'Gadget', 25.50)")
        .await?;
    db_handler
        .execute("INSERT INTO products (id, name, price) VALUES (3, 'Tool', 15.75)")
        .await?;
    ...
    let result = db_handler.execute(query).await?;
    println!(
        "Single column batch update affected {} rows",
    ...
    for (i, row) in select_result.rows.iter().enumerate() {
        ...
        assert!(
            (price - expected_prices[i]).abs() < 0.01,
            "Price mismatch at row {}: expected {}, got {}",
            i,
            expected_prices[i],
            price
        );
...
async fn test_batch_update_with_quotes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE quotes_test (id INTEGER PRIMARY KEY, description TEXT)")
        .await?;

    // Insert test data
    db_handler
        .execute("INSERT INTO quotes_test (id, description) VALUES (1, 'Simple text')")
        .await?;
    db_handler
        .execute("INSERT INTO quotes_test (id, description) VALUES (2, 'Another text')")
        .await?;
    ...
    // Verify the updates
    let select_result = db_handler
        .query("SELECT id, description FROM quotes_test ORDER BY id")
        .await?;

    let expected_descriptions = ["Text with, comma", "Text with 'apostrophe'"];

    for (i, row) in select_result.rows.iter().enumerate() {
        let description = String::from_utf8(row[1].as_ref().unwrap().clone())?;
        assert_eq!(
            description, expected_descriptions[i],
...
async fn test_batch_update_no_alias() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE simple_table (id INTEGER PRIMARY KEY, value INTEGER)")
        .await?;

    // Insert test data
    db_handler
        .execute("INSERT INTO simple_table VALUES (1, 100), (2, 200), (3, 300)")
        .await?;
    ...
    let result = db_handler.execute(query).await?;
    println!(
        "No alias batch update affected {} rows",
        result.rows_affected
    );

    // Verify the updates
    let select_result = db_handler
        .query("SELECT id, value FROM simple_table ORDER BY id")
        .await?;
...
async fn test_batch_update_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE perf_test (id INTEGER PRIMARY KEY, value INTEGER)")
        .await?;

    // Insert test data
    for i in 1..=1000 {
        db_handler
            .execute(&format!("INSERT INTO perf_test VALUES ({}, {})", i, i * 10))
            .await?;
    }
    ...
    let verify_result = db_handler
        .query("SELECT value FROM perf_test WHERE id IN (1, 50, 100)")
        .await?;
...
async fn test_regular_update_still_works() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create test table
    db_handler
        .execute("CREATE TABLE regular_test (id INTEGER PRIMARY KEY, name TEXT)")
        .await?;

    // Insert test data
    db_handler
        .execute("INSERT INTO regular_test VALUES (1, 'Original')")
        .await?;
    ...
    // Verify the update
    let select_result = db_handler
        .query("SELECT name FROM regular_test WHERE id = 1")
        .await?;
