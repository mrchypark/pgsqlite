...
async fn setup_realistic_dataset(client: &Client) {
    ...
    for batch in 0..100 {
        ...
        let query = format!(
            "INSERT INTO customers (email, name, total_orders) VALUES {}",
            values.join(", ")
        );
        client
...
async fn benchmark_data_export_scenario(client: &Client) {
    ...
    let all_customers = client
        .query(export_query, &[])
        .await
        .expect("Failed to export all customers");
    ...
    loop {
        ...
        let chunk_customers = client
            .query(&chunk_query, &[])
            .await
            .expect("Failed to fetch chunk");

        if chunk_customers.is_empty() {
            break;
        }
        ...
        if chunk_customers.len() < chunk_size {
            break;
        }
    }
    ...
    println!(
        "  📦 Full Export:      {:?} ({} customers, ~{:.2}MB memory)",
        full_export_time,
        total_customers,
        (total_customers * 300) as f64 / 1_000_000.0
    );
    println!(
        "  🚀 Streaming Export: {:?} ({} customers, ~{:.2}MB peak memory)",
        streaming_export_time,
        exported_count,
        (chunk_size * 300) as f64 / 1_000_000.0
    );
    println!(
        "  💾 Memory reduction: {:.1}%",
    ...
    if streaming_export_time < full_export_time {
        println!(
            "  🎯 Streaming is {:.1}x faster overall",
            full_export_time.as_secs_f64() / streaming_export_time.as_secs_f64()
        );
    } else {
        println!(
            "  ⚠️  Streaming is {:.1}x slower but uses {:.1}% less memory",
...
async fn benchmark_pagination_scenario(client: &Client) {
    ...
    println!(
        "📋 Simulating {total_pages} API requests for paginated product listings (page size: {page_size})"
    );
    ...
    let stmt = client
        ...
        .await
        .expect("Failed to prepare paginated query");
    ...
    for page in 0..total_pages {
        ...
        let rows = client
            ...
            .await
            .expect("Failed to fetch page with portal");
        let page_time = page_start.elapsed();
    ...
    if portal_time < traditional_time {
        println!(
            "  🎯 Portal approach is {:.1}x faster",
            traditional_time.as_secs_f64() / portal_time.as_secs_f64()
        );
    } else {
        println!(
            "  ⚠️  Traditional approach is {:.1}x faster",
    ...
    println!(
        "  📊 Average per page: Traditional {:.2}ms, Portal {:.2}ms",
        traditional_avg as f64 / 1000.0,
        portal_avg as f64 / 1000.0
    );
}
...
async fn benchmark_report_generation_scenario(client: &Client) {
    ...
    println!(
        "  🚀 Streaming Report: {streaming_report_time:?} ({processed_rows} rows, {chunk_count} chunks)"
    );
    println!(
        "  💾 Memory reduction: {:.1}%",
    ...
    println!(
        "  ⚡ Throughput: Full {full_throughput:.0} rows/sec, Streaming {streaming_throughput:.0} rows/sec"
    );
}
...
async fn benchmark_etl_streaming_scenario(client: &Client) {
    ...
    if !insert_values.is_empty() {
        ...
        client
            .execute(&insert_query, &insert_params)
            .await
    ...
    client
        .execute("DELETE FROM order_summary", &[])
        .await
        .expect("Failed to clean summary");
    ...
    loop {
        ...
        let mut chunk_extracted_values: Vec<(
            i32,
            i64,
            rust_decimal::Decimal,
            rust_decimal::Decimal,
        ...
        let chunk_insert = format!(
            "INSERT INTO order_summary VALUES {}",
            chunk_values.join(", ")
        );
        client
            .execute(&chunk_insert, &chunk_params)
            .await
    ...
    println!(
        "  📊 Traditional ETL: {:?} ({} records)",
        traditional_etl_time,
        all_data.len()
    );
    println!(
        "  🚀 Streaming ETL:   {streaming_etl_time:?} ({processed_records} records, {chunk_count} chunks)"
    );
    println!(
        "  💾 Memory reduction: {:.1}%",
    ...
    println!(
        "  ⚡ Throughput: Traditional {traditional_throughput:.0} records/sec, Streaming {streaming_throughput:.0} records/sec"
    );
}
...
async fn benchmark_multitenant_scenario() {
    ...
    println!(
        "👥 Simulating {tenant_count} tenants each running {queries_per_tenant} queries concurrently"
    );
            ...
            for query_id in 0..queries_per_tenant {
                ...
                let result = match query_id % 4 {
                    ...
                    2 => {
                        ...
                        let rows = client
                            .query(
                                "SELECT * FROM products WHERE price > $1 LIMIT 15",
    ...
    for (tenant_id, results, tenant_time) in &all_results {
        ...
        println!(
            "  Tenant {}: {} queries, {} rows, {:?} total ({:.1}μs avg/query)",
    ...
    println!(
        "  🎯 Throughput: {:.0} queries/sec, {:.0} rows/sec",
    ...
    println!(
        "  🎪 Portal efficiency: {avg_queries_per_second:.0} concurrent queries/sec across {tenant_count} tenants"
    );
}
...
async fn connect_to_server() -> (
    ...
    tokio_postgres::connect("host=localhost port=5433 user=postgres dbname=test", NoTls)
        .await
