...
impl QueryAnalyzer {
    ...
    pub fn analyze(query: &str) -> TranslationFlags {
        ...
        if query.contains("::") || query_lower.contains("cast(") {
            ...
            if query_lower.contains("::numeric")
                || query_lower.contains("::decimal")
                || query_lower.contains("::double precision")
                || query_lower.contains("::real")
                || query_lower.contains("::float")
                || query_lower.contains(" as numeric")
                || query_lower.contains(" as decimal")
                || query_lower.contains(" as double precision")
                || query_lower.contains(" as real")
        ...
        if query.contains("->")
            ...
            || query.contains("?|")
            || query.contains("?&")
            || query_lower.contains("json")
            || query_lower.contains("jsonb")
        {
            ...
            || query.contains(" all(")
        {
            ...
            if query_lower.contains("array_")
                || query_lower.contains("unnest")
                || query.contains("ARRAY[")
                || query.contains("array[")
                || query.contains(" ANY(")
                || query.contains(" any(")
...
mod tests {
    ...
    fn test_multiple_flags() {
        let flags =
            QueryAnalyzer::analyze("SELECT id::int, data->>'name', ARRAY[1,2,3] FROM users");
        assert!(flags.contains(TranslationFlags::CAST));
        assert!(flags.contains(TranslationFlags::JSON));
        assert!(flags.contains(TranslationFlags::ARRAY));
    }
    ...
    fn test_insert_array_detection() {
        let flags =
            QueryAnalyzer::analyze("INSERT INTO test_arrays (int_array) VALUES ('{1,2,3}')");
        assert!(flags.contains(TranslationFlags::INSERT_DATETIME));
    }
}
