...
impl EnumValidator {
    ...
    pub fn validate_insert(conn: &Connection, query: &str) -> Result<(), PgSqliteError> {
        // Parse INSERT statement to extract table and values
        let insert_regex =
            Regex::new(r"(?i)INSERT\s+INTO\s+(\w+)\s*\(([^)]+)\)\s*VALUES\s*\(([^)]+)\)")
                    ...
                    if let Ok(Some(enum_type)) = EnumMetadata::get_enum_type(conn, &pg_type) {
                        ...
                        if !EnumMetadata::is_valid_enum_value(conn, enum_type.type_oid, value)? {
                            return Err(PgSqliteError::Protocol(format!(
                                "invalid input value for enum {pg_type}: \"{value}\""
                            )));
                        }
                    }
                }
    ...
    pub fn validate_update(conn: &Connection, query: &str) -> Result<(), PgSqliteError> {
        // Parse UPDATE statement
        let update_regex = Regex::new(r"(?i)UPDATE\s+(\w+)\s+SET\s+(.+?)(?:\s+WHERE|$)")
            .map_err(|e| PgSqliteError::Protocol(format!("Regex error: {e}")))?;
        ...
        if let Some(captures) = update_regex.captures(query) {
            ...
            let assignments: Vec<&str> = set_clause.split(',').map(|a| a.trim()).collect();
