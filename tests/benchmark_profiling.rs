...
async fn benchmark_with_profiling() {
    ...
    // Insert schema info
    conn.execute(
        "INSERT INTO __pgsqlite_schema VALUES ('profile_test', 'id', 'int4')",
        [],
    )
    .unwrap();
    conn.execute(
        "INSERT INTO __pgsqlite_schema VALUES ('profile_test', 'name', 'text')",
        [],
    )
    .unwrap();
    conn.execute(
        "INSERT INTO __pgsqlite_schema VALUES ('profile_test', 'value', 'int4')",
        [],
    )
    .unwrap();
    ...
    let mut stmt = conn
        .prepare("INSERT INTO profile_test (id, name, value) VALUES (?1, ?2, ?3)")
        .unwrap();
    for i in 0..NUM_ROWS {
        stmt.execute(params![i as i32, format!("name_{}", i), i as i32])
            .unwrap();
    }
    drop(stmt);
    drop(conn);
    ...
    let test_queries = vec![
        ...
        (
            "Simple INSERT",
            "INSERT INTO profile_test (id, name, value) VALUES (10001, 'test', 100)",
        ),
        (
            "Simple UPDATE",
            "UPDATE profile_test SET value = 200 WHERE id = 1",
        ),
    ...
    let mut stmt = conn
        ...
        .unwrap();
    for i in 0..1000 {
        stmt.execute(params![{ i }, format!("name_{}", i), { i }])
            .unwrap();
    }
    drop(stmt);
...
async fn read_until_ready(stream: &mut TcpStream) -> usize {
    ...
    loop {
        ...
        while pos < n {
            ...
            let msg_len =
                i32::from_be_bytes([buf[pos], buf[pos + 1], buf[pos + 2], buf[pos + 3]]) as usize;
            pos += 4;
