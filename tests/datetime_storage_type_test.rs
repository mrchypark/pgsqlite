...
async fn test_pgsqlite_schema_metadata() {
    ...
    for row in &schema_rows {
        ...
        match column_name.as_str() {
            "date_col" => assert!(
                pg_type.eq_ignore_ascii_case("date"),
                "Expected date, got {pg_type}"
            ),
            "time_col" => assert!(
                pg_type.eq_ignore_ascii_case("time"),
                "Expected time, got {pg_type}"
            ),
            "timestamp_col" => assert!(
                pg_type.eq_ignore_ascii_case("timestamp"),
                "Expected timestamp, got {pg_type}"
            ),
            "timestamptz_col" => assert!(
                pg_type.eq_ignore_ascii_case("timestamptz"),
                "Expected timestamptz, got {pg_type}"
            ),
            "timetz_col" => assert!(
                pg_type.eq_ignore_ascii_case("timetz"),
                "Expected timetz, got {pg_type}"
            ),
            "interval_col" => assert!(
                pg_type.eq_ignore_ascii_case("interval"),
                "Expected interval, got {pg_type}"
            ),
    ...
    assert_eq!(
        schema_rows.len(),
        6,
        "Should have metadata for 6 datetime columns"
    );
...
async fn test_datetime_edge_cases() {
    ...
    client
        ...
        .await
        .unwrap_err(); // Should fail due to multiple primary keys
...
async fn test_datetime_storage_known_limitations() {
    ...
    if !text_literal_rows.is_empty() {
        ...
        assert_eq!(
            date_type, "text",
            "Known limitation: text literals stored as TEXT"
        );
    }
