...
async fn test_current_date_function() {
    ...
    let row = client
        .query_one("SELECT CURRENT_DATE as today", &[])
        .await
        .unwrap();
...
async fn test_extract_function() {
    ...
    let rows = client
        .query(
            &format!(
                "SELECT extract('year', {timestamp_micros}) as year,
...
async fn test_date_trunc_function() {
    ...
    let rows = client
        .query(
            &format!(
                "SELECT date_trunc('hour', {timestamp_micros}) as hour_trunc,
...
async fn test_interval_arithmetic() {
    ...
    let rows = client
        .query(
            &format!(
                "SELECT {timestamp_micros} + INTERVAL '1 day' as tomorrow,
