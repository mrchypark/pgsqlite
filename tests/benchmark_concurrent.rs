...
async fn benchmark_mixed_workload_80_20() {
    ...
    assert!(
        ...
        "Should handle 100+ ops in mixed workload"
    );
    assert!(
        metrics.read_success_rate > 0.95,
        "Read success rate should be >95%"
    );
    assert!(
        metrics.write_success_rate > 0.90,
...
async fn benchmark_mixed_workload_50_50() {
    ...
    assert!(
        metrics.total_operations > 100,
        "Should handle 100+ ops in balanced workload"
    );
    assert!(
        metrics.read_success_rate > 0.90,
        "Read success rate should be >90%"
    );
    assert!(
        metrics.write_success_rate > 0.85,
        "Write success rate should be >85%"
    );
...
async fn benchmark_mixed_workload_20_80() {
    ...
    assert!(
        metrics.total_operations > 100,
        "Should handle 100+ ops in write-heavy workload"
    );
    assert!(
        metrics.read_success_rate > 0.90,
        "Read success rate should be >90%"
    ...
    assert!(
        metrics.write_success_rate > 0.80,
        "Write success rate should be >80%"
    );
...
async fn run_mixed_workload(
        ...
        let handle = tokio::spawn(async move {
            ...
            while !*stop_flag.lock().await {
                ...
                if rand::random::<f64>() < read_ratio {
                    ...
                    match db
                        .query(&format!("SELECT value FROM benchmark_data WHERE id = {id}"))
                        .await
                    {
                        Ok(_) => read_count += 1,
                        Err(_) => read_errors += 1,
    ...
    WorkloadMetrics {
        ...
        p95_latency_ms: all_latencies
            .get(p95_idx)
            .map(|d| d.as_secs_f64() * 1000.0)
            .unwrap_or(0.0),
        p99_latency_ms: all_latencies
            .get(p99_idx)
            .map(|d| d.as_secs_f64() * 1000.0)
            .unwrap_or(0.0),
    ...
    for task_id in 0..task_count {
        ...
        let handle = tokio::spawn(async move {
            ...
            while !*stop_flag.lock().await {
                ...
                match db
                    .query(&format!(
                        "SELECT value, description FROM benchmark_data WHERE id = {id}"
                    ))
                    .await
                {
                    Ok(_) => read_count += 1,
...
async fn execute_transfer_transaction(
    ...
    let from_result = db
        .query(&format!(
            "SELECT value FROM benchmark_data WHERE id = {from_id}"
        ))
        .await?;
    let to_result = db
        .query(&format!(
            "SELECT value FROM benchmark_data WHERE id = {to_id}"
        ))
        .await?;
    ...
    let from_balance: i64 =
        String::from_utf8(from_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
    let to_balance: i64 =
        String::from_utf8(to_result.rows[0][0].as_ref().unwrap().clone())?.parse()?;
...
fn print_metrics(metrics: &WorkloadMetrics) {
    ...
    println!(
        "  Read success rate: {:.1}%",
        metrics.read_success_rate * 100.0
    );
    println!(
        "  Write success rate: {:.1}%",
        metrics.write_success_rate * 100.0
    );
