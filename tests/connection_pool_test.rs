...
async fn test_db_handler_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db_handler
        .execute_with_session(
            "CREATE TABLE test_basic (id INTEGER PRIMARY KEY, name TEXT)",
            &session_id,
        )
        .await?;
    db_handler
        .execute_with_session(
            "INSERT INTO test_basic (name) VALUES ('test1')",
            &session_id,
        )
        .await?;

    let result = db_handler
        .query_with_session("SELECT * FROM test_basic", &session_id)
        .await?;
...
async fn test_db_handler_transactions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db_handler
        .execute_with_session("INSERT INTO test_tx (value) VALUES (100)", &session_id)
        .await?;
    db_handler
        .execute_with_session("INSERT INTO test_tx (value) VALUES (200)", &session_id)
        .await?;
    db_handler.commit_with_session(&session_id).await?;

    let result = db_handler
        .query_with_session("SELECT COUNT(*) FROM test_tx", &session_id)
        .await?;
    assert_eq!(result.rows.len(), 1);
    ...
    db_handler.begin_with_session(&session_id).await?;
    db_handler
        .execute_with_session("INSERT INTO test_tx (value) VALUES (300)", &session_id)
        .await?;
...
async fn test_db_handler_concurrent_access() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db_handler
        .create_session_connection(setup_session_id)
        .await?;
    ...
    for i in 0..10 {
        db_handler
            .execute_with_session(
                &format!("INSERT INTO test_concurrent (thread_id) VALUES ({i})"),
                &setup_session_id,
            )
            .await
    ...
    for i in 0..num_tasks {
        ...
        let handle = tokio::spawn(async move {
            ...
            db.create_session_connection(task_session_id)
                .await
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            ...
                            if retry_count >= max_retries {
                                panic!(
                                    "Read failed in task {i}, iteration {j} after {retry_count} retries: {e:?}"
                                );
                            }
    ...
    assert_eq!(
        results.len(),
        num_tasks,
        "Not all concurrent read tasks completed"
    );
    ...
    for i in 0..write_batches {
        for j in 0..writes_per_batch {
            ...
            let exec_result = db_handler
                .execute_with_session(
                    &format!("INSERT INTO test_concurrent (thread_id) VALUES ({value})"),
                    &setup_session_id,
                )
                .await
                .unwrap_or_else(|e| {
                    panic!("Failed to insert in batch {i}, item {j} (value {value}): {e:?}")
                });
            if exec_result.rows_affected != 1 {
                panic!(
                    "INSERT affected {} rows instead of 1 for value {}",
                    exec_result.rows_affected, value
                );
            }
        }
    ...
    let count_result = db_handler
        .query_with_session(
            "SELECT COUNT(thread_id) FROM test_concurrent",
            &setup_session_id,
        )
        .await
