...
impl ExtendedFastPath {
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        let rusqlite_params = match Self::convert_parameters_cached(
            query,
            bound_values,
            param_formats,
            param_types,
        ...
        match query_type {
            QueryType::Select => {
                ...
                match Self::execute_select_with_params(
                    framed,
                    db,
                    session,
                    portal_name,
            ...
            QueryType::Insert | QueryType::Update | QueryType::Delete => {
                match Self::execute_dml_with_params(
                    framed,
                    db,
                    session,
                    query,
            ...
            match param_type {
                ...
                t if t == PgType::Date.to_oid() => {
                    ...
                    match crate::types::ValueConverter::convert_date_to_unix(text) {
                        Ok(days_str) => {
                            let days = days_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol(format!("Invalid date days: {days_str}"))
                            })?;
                ...
                t if t == PgType::Time.to_oid() => {
                    ...
                    match crate::types::ValueConverter::convert_time_to_seconds(text) {
                        Ok(micros_str) => {
                            let micros = micros_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol(format!(
                                    "Invalid time microseconds: {micros_str}"
                                ))
                            })?;
                ...
                t if t == PgType::Timestamp.to_oid() => {
                    ...
                    match crate::types::ValueConverter::convert_timestamp_to_unix(text) {
                        Ok(micros_str) => {
                            let micros = micros_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol(format!(
                                    "Invalid timestamp microseconds: {micros_str}"
                                ))
                            })?;
                            Ok(rusqlite::types::Value::Integer(micros))
                        }
                ...
                t if t == PgType::Timestamptz.to_oid()
                    || t == PgType::Timetz.to_oid()
                    || t == PgType::Interval.to_oid() =>
                {
                ...
                t if t == PgType::Money.to_oid()
                    || t == PgType::Macaddr.to_oid()
                    || t == PgType::Macaddr8.to_oid()
                    || t == PgType::Inet.to_oid()
                    || t == PgType::Cidr.to_oid()
                    || t == PgType::Int4range.to_oid()
                    || t == PgType::Int8range.to_oid()
                    || t == PgType::Numrange.to_oid()
                    || t == PgType::Bit.to_oid()
            ...
            match param_type {
                    ...
                    match bytes.len() {
                        ...
                        8 => {
                            ...
                            if val >= i16::MIN as i64 && val <= i16::MAX as i64 {
                                ...
                                Err(PgSqliteError::Protocol(format!(
                                    "INT8 value {} too large for INT2",
                                    val
                                )))
                            }
                        }
                        _ => Err(PgSqliteError::Protocol(format!(
                            "Invalid INT2 binary format: {} bytes",
                            bytes.len()
                        ))),
                    }
                }
                    ...
                    match bytes.len() {
                        ...
                        8 => {
                            ...
                            if val >= i32::MIN as i64 && val <= i32::MAX as i64 {
                                ...
                                Err(PgSqliteError::Protocol(format!(
                                    "INT8 value {} too large for INT4",
                                    val
                                )))
                            }
                        }
                        _ => Err(PgSqliteError::Protocol(format!(
                            "Invalid INT4 binary format: {} bytes",
                            bytes.len()
                        ))),
                    }
                }
                t if t == PgType::Int8.to_oid() => {
                    ...
                    match bytes.len() {
                        ...
                        _ => Err(PgSqliteError::Protocol(format!(
                            "Invalid INT8 binary format: {} bytes",
                            bytes.len()
                        ))),
                    }
                }
                t if t == PgType::Float4.to_oid() => {
                    ...
                    if bytes.len() == 4 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid FLOAT4 binary format".to_string(),
                        ))
                    }
                }
                t if t == PgType::Float8.to_oid() => {
                    ...
                    if bytes.len() == 8 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid FLOAT8 binary format".to_string(),
                        ))
                    }
                }
                t if t == PgType::Numeric.to_oid() => {
                    ...
                    match DecimalHandler::decode_numeric(bytes) {
                        ...
                        Err(e) => Err(PgSqliteError::Protocol(format!(
                ...
                t if t == PgType::Date.to_oid() => {
                    ...
                    if bytes.len() == 4 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid DATE binary format".to_string(),
                        ))
                    }
                }
                t if t == PgType::Time.to_oid() || t == PgType::Timetz.to_oid() => {
                    ...
                    if bytes.len() == 8 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid TIME binary format".to_string(),
                        ))
                    }
                }
                t if t == PgType::Timestamp.to_oid() || t == PgType::Timestamptz.to_oid() => {
                    ...
                    if bytes.len() == 8 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid TIMESTAMP binary format".to_string(),
                        ))
                    }
                }
                t if t == PgType::Interval.to_oid() => {
                    ...
                    if bytes.len() == 16 {
                        ...
                        Err(PgSqliteError::Protocol(
                            "Invalid INTERVAL binary format".to_string(),
                        ))
                    }
                }
                    ...
                    || t == PgType::Varbit.to_oid() =>
                {
                    ...
                    Err(PgSqliteError::Protocol(format!(
                        "Binary format not implemented for type {param_type}"
                    )))
                }
                0 => {
                    ...
                    if bytes.len() == 8 {
                        ...
                        if (0..4_102_444_800).contains(&seconds) {
                            // 2100-01-01
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        let response = match db
            .try_execute_fast_path_with_params(query, &params, &session.id)
            .await
        {
            Ok(Some(resp)) => resp,
            Ok(None) => {
        ...
        let send_row_desc = {
            ...
            let looks_suspicious = if !is_empty && !stmt.field_descriptions.is_empty() {
                ...
                let non_param_types: Vec<_> = stmt
                    .field_descriptions
                    .iter()
                    .filter(|fd| {
                        !fd.name.starts_with('$') && fd.name != "?column?" && fd.name != "NULL"
                    })
                    .map(|fd| fd.type_oid)
                    .collect();
            ...
            if looks_suspicious {
                if query.contains("orders") && query.contains("customer_id") {
                    ...
                    info!(
                        "Fast path: Field descriptions: {:?}",
                        stmt.field_descriptions
                    );
                }
                debug!(
                    "Fast path: Existing field descriptions look suspicious (all TEXT), will override"
                );
            }
        ...
        if send_row_desc {
            ...
            debug!(
                "Fast path: Need to send RowDescription for {} columns",
            ...
            for (i, col_name) in response.columns.iter().enumerate() {
                ...
                let type_oid = if col_name.starts_with('$')
                    || col_name == "?column?"
                    || col_name == "NULL"
                {
                    ...
                    if col_name.contains('_') {
                        ...
                        if let Some(underscore_pos) = col_name.find('_') {
                                ...
                                .await
                            {
                                ...
                                debug!(
                                    "Fast path: Inferred type for '{}' from schema ({}_{}) -> {} (OID {})",
                                    col_name,
                                    potential_table,
                                    potential_column,
                                    pg_type_str,
                                ...
                                debug!(
                                    "Fast path: No schema type found for {}.{}",
                                    potential_table, potential_column
                                );
                            }
                        }
                    }
                    ...
                    if inferred_type == PgType::Text.to_oid() {
                        ...
                        if let Some(table_name) = Self::extract_table_from_query(query) {
                                ...
                                .await
                            {
                                ...
                                debug!(
                                    "Fast path: Inferred type for '{}' from table '{}' -> {} (OID {})",
                                    col_name, table_name, pg_type_str, new_type
                                );
                                inferred_type = new_type;
                            } else {
                                debug!(
                                    "Fast path: No type found for column '{}' in table '{}'",
                                    col_name, table_name
                                );
                            }
                        } else {
                    ...
                    if inferred_type == PgType::Text.to_oid() {
                        debug!(
                            "Fast path: No type found for '{}', defaulting to TEXT",
                            col_name
                        );
                    }
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if query.contains("RETURNING") && !response.columns.is_empty() {
            // Send RowDescription
            let fields: Vec<crate::protocol::FieldDescription> = response
                .columns
                .iter()
                .enumerate()
                .map(|(i, col_name)| {
