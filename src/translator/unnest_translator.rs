...
static UNNEST_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bunnest\s*\(\s*([^)]+)\s*\)").unwrap());
...
static UNNEST_WITH_ORDINALITY_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?i)\bFROM\s+unnest\s*\(\s*([^)]+)\s*\)\s+WITH\s+ORDINALITY(?:\s+(?:AS\s+)?(\w+))?",
    )
    .unwrap()
});
...
impl UnnestTranslator {
    ...
    fn translate_from_clause_with_ordinality(sql: &str) -> Result<String, PgSqliteError> {
        ...
        for (original, replacement) in replacements {
            result = result.replace(&original, &replacement);
            debug!(
                "Translated FROM unnest WITH ORDINALITY: {} -> {}",
...
mod tests {
    ...
    fn test_contains_unnest() {
        assert!(UnnestTranslator::contains_unnest(
