...
impl EnumTriggers {
    ...
    pub fn create_enum_validation_triggers(
        ...
        let insert_trigger_name = format!("__pgsqlite_{}_{}_{}_insert_check", 
        ...
        let insert_trigger_sql = format!(
            ...
            BEFORE INSERT ON "{}"
            FOR EACH ROW
            WHEN NEW."{}" IS NOT NULL AND NOT EXISTS (
                SELECT 1 FROM __pgsqlite_enum_values ev
                JOIN __pgsqlite_enum_types et ON ev.type_oid = et.type_oid
                WHERE et.type_name = '{}' AND ev.label = NEW."{}"
            )
            BEGIN
                SELECT RAISE(ABORT, 'invalid input value for enum {}: "' || NEW."{}" || '"');
            END"#,
            insert_trigger_name, table_name, column_name, 
            enum_type, column_name, enum_type, column_name
        );
        ...
        let update_trigger_name = format!("__pgsqlite_{}_{}_{}_update_check", 
            table_name, column_name, enum_type);
        ...
        let update_trigger_sql = format!(
            ...
            BEGIN
                SELECT RAISE(ABORT, 'invalid input value for enum {}: "' || NEW."{}" || '"');
            END"#,
            update_trigger_name, column_name, table_name, column_name, 
            enum_type, column_name, enum_type, column_name
        );
