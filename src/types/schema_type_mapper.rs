...
impl SchemaTypeMapper {
    ...
    pub fn get_type_from_schema(
        ...
        if let Ok(Some(pg_type)) =
            crate::metadata::TypeMetadata::get_pg_type(conn, table_name, column_name)
        {
    ...
    pub fn pg_type_string_to_oid(pg_type: &str) -> i32 {
        ...
        match pg_type {
            ...
            "integer" | "INTEGER" | "int4" | "INT4" | "int" | "INT" => {
    ...
    pub fn get_aggregate_return_type_with_query(
        ...
        if upper.starts_with("DECIMAL_ADD(")
            || upper.starts_with("DECIMAL_SUB(")
            || upper.starts_with("DECIMAL_MUL(")
            || upper.starts_with("DECIMAL_DIV(")
        ...
        if upper.starts_with("JSON_GROUP_ARRAY(")
            || upper.starts_with("JSON_ARRAY(")
            || upper.starts_with("JSON_OBJECT(")
            || upper.starts_with("JSON_EXTRACT(")
        {
            return Some(PgType::Text.to_oid()); // text
        }

        // JSON aggregate functions that return text (for compatibility)
        if upper.starts_with("JSON_AGG(")
            || upper.starts_with("JSON_OBJECT_AGG(")
            || upper.starts_with("JSONB_AGG(")
            || upper.starts_with("JSONB_OBJECT_AGG(")
        ...
        if let Some(column_name) =
            crate::types::QueryContextAnalyzer::extract_column_from_aggregation(function_name)
        {
            ...
            if let Some(fixed_type) =
                crate::types::aggregate_type_fixer::fix_aggregate_type_for_decimal(
                    function_name,
                    query,
                )
            {
        ...
        if let Some(fixed_type) =
            crate::types::aggregate_type_fixer::fix_aggregate_type_for_decimal(function_name, query)
        {
            return Some(fixed_type);
        }
        ...
        if upper.starts_with("ARRAY_LENGTH(")
            || upper.starts_with("ARRAY_UPPER(")
            || upper.starts_with("ARRAY_LOWER(")
            || upper.starts_with("ARRAY_NDIMS(")
        {
            return Some(PgType::Int4.to_oid()); // int4
        }

        if upper.starts_with("ARRAY_APPEND(")
            || upper.starts_with("ARRAY_PREPEND(")
            || upper.starts_with("ARRAY_CAT(")
            || upper.starts_with("ARRAY_REMOVE(")
            || upper.starts_with("ARRAY_REPLACE(")
            || upper.starts_with("ARRAY_SLICE(")
        ...
        if upper.starts_with("ARRAY_CONTAINS(")
            || upper.starts_with("ARRAY_CONTAINED(")
