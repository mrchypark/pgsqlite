...
static TIMEZONE_OFFSET_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^([-+])(\d{2}):?(\d{2})$").unwrap());
...
impl ValueConverter {
    ...
    fn convert_timestamptz_to_unix(value: &str) -> Result<String, String> {
        ...
        let (datetime_str, offset_seconds) =
            if let Some(caps) = TIMESTAMPTZ_REGEX.captures(value.trim()) {
    ...
    fn convert_interval_to_seconds(value: &str) -> Result<String, String> {
        ...
        if let Some(caps) = INTERVAL_REGEX.captures(trimmed) {
            ...
            let total_micros =
                (days * 86400 + hours * 3600 + minutes * 60 + seconds) * 1_000_000 + fraction;
...
mod tests {
    ...
    fn test_timestamp_conversion() {
        ...
        let result =
            ValueConverter::convert_timestamp_to_unix("2024-01-15 14:30:45.123456").unwrap();
        let micros = result.parse::<i64>().unwrap();
    ...
    fn test_interval_conversion() {
        ...
        assert_eq!(
            ValueConverter::convert_interval_to_seconds("3600000000").unwrap(),
            "3600000000"
        );

        // Test HH:MM:SS format
        assert_eq!(
            ValueConverter::convert_interval_to_seconds("01:30:00").unwrap(),
            "5400000000"
        ); // 1.5 hours in microseconds
        ...
        assert_eq!(
            ValueConverter::convert_seconds_to_interval("95400000000").unwrap(),
            "1 day 02:30:00"
        );
        assert_eq!(
            ValueConverter::convert_seconds_to_interval("5400500000").unwrap(),
            "01:30:00.500000"
        );
    }
}
