...
async fn benchmark_batch_insert_performance() {
    ...
    for &batch_size in BATCH_SIZES {
        ...
        println!(
            "Testing batch size {} ({} batches{})",
            batch_size,
            num_batches,
            if remainder > 0 {
                format!(" + {remainder} rows")
            } else {
                String::new()
            }
        );
    ...
    for batch in 0..(TOTAL_ROWS / 100) {
        let mut query =
            String::from("INSERT INTO batch_test (id, name, value, description) VALUES ");
        for i in 0..100 {
            ...
            query.push_str(&format!(
                "({row_id}, 'test_{row_id}', {row_id}, 'Description for row {row_id}')"
            ));
        }
    ...
    for (batch_size, elapsed, _per_row, rows_per_sec) in &results {
        ...
        println!(
            "  Batch size {batch_size:4}: {elapsed:?} ({rows_per_sec:6.0} rows/sec, {speedup:.1}x speedup vs single)"
        );
    }
    ...
    let (optimal_size, optimal_time, _, optimal_rate) = results
        .iter()
