...
async fn test_returning_clause() {
    ...
    let result = client
        .simple_query(&format!(
            "DELETE FROM users WHERE id = {bob_id} RETURNING name, email"
        ))
        .await
        .unwrap();
    ...
    let result = client
        .simple_query("DELETE FROM users WHERE name = 'NonExistent' RETURNING *")
        .await
        .unwrap();
    ...
    assert!(
        !has_rows,
        "Should not return any rows for non-existent user"
    );
