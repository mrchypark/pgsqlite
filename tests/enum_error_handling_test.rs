...
async fn test_invalid_enum_value_error() {
    ...
    client
        .simple_query("CREATE TABLE person (name TEXT, current_mood mood)")
        .await
        .unwrap();
    ...
    assert!(
        error_str.contains("invalid input value for enum mood: \"angry\""),
...
async fn test_cannot_drop_type_with_dependencies() {
    ...
    client
        .simple_query("CREATE TABLE items (id INTEGER, item_status status)")
        .await
        .unwrap();
    ...
    let error_str = error.to_string();
    assert!(
        error_str.contains("cannot drop type status because other objects depend on it")
    ...
    client
        .simple_query("DROP TYPE status CASCADE")
        .await
        .unwrap();
}
...
async fn test_alter_type_that_does_not_exist() {
    ...
    let result = client
        .simple_query("ALTER TYPE nonexistent ADD VALUE 'new_value'")
        .await;
...
async fn test_if_exists_clause() {
    ...
    client
        .simple_query("DROP TYPE IF EXISTS nonexistent_type")
        .await
        .unwrap();

    // Create and drop with IF EXISTS using simple_query for DDL
    client
        .simple_query("CREATE TYPE test_type AS ENUM ('a', 'b')")
        .await
        .unwrap();

    client
        .simple_query("DROP TYPE IF EXISTS test_type")
        .await
        .unwrap();
