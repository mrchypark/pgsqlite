...
async fn test_enum_type_oid_in_schema() {
    ...
    client
        .simple_query("CREATE TABLE people (id INTEGER PRIMARY KEY, name TEXT, mood mood)")
        .await
        .expect("Failed to create table");

    // Insert a row
    client
        .simple_query("INSERT INTO people (id, name, mood) VALUES (1, 'Alice', 'happy')")
        .await
    ...
    let table_check = client
        ...
        .await
        .expect("Failed to query pg_class");

    assert_eq!(
        table_check.len(),
        1,
        "Table 'people' should exist in pg_class"
    );
    ...
    let all_rows = client
        ...
        .await
        .expect("Failed to query pg_attribute");
    ...
    let rows = client
        .query(
            &format!(
                "SELECT attname, atttypid::text FROM pg_attribute WHERE attrelid = {table_oid}"
            ),
            &[],
        )
        .await
        .expect("Failed to query pg_attribute with WHERE");

    eprintln!(
        "pg_attribute (WHERE attrelid = {}) returned {} rows",
        table_oid,
        rows.len()
    );
    ...
    if rows.len() >= 3 {
        ...
        assert!(
            type_oid >= 10000,
            "ENUM type should have custom OID >= 10000, got {type_oid}"
        );
    } else {
    ...
    let results = client
        .simple_query("SELECT id, name, mood FROM people WHERE id = 1")
        .await
...
async fn test_enum_with_text_protocol() {
    ...
    // Create a table
    client
        .simple_query("CREATE TABLE tasks (id INTEGER PRIMARY KEY, status status)")
        .await
        .expect("Failed to create table");

    // Use prepared statement but query with text values only
    let stmt = client
        .prepare("INSERT INTO tasks (id, status) VALUES ($1, $2)")
        .await
        .expect("Failed to prepare statement");

    // Insert with text protocol (default)
    client
        .execute(&stmt, &[&1i32, &"active"])
        .await
        .expect("Failed to insert with text protocol");

    // Query back
    let rows = client
        .query("SELECT id, status::text FROM tasks WHERE id = 1", &[])
        .await
