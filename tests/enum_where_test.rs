...
async fn test_enum_where_equality() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE status AS ENUM ('pending', 'active', 'inactive', 'deleted')")
        .await
    ...
    // Insert test data
    client
        .simple_query("INSERT INTO items (id, name, status) VALUES (1, 'Item 1', 'active')")
        .await
        .expect("Failed to insert row 1");
    client
        .simple_query("INSERT INTO items (id, name, status) VALUES (2, 'Item 2', 'pending')")
        .await
        .expect("Failed to insert row 2");
    client
        .simple_query("INSERT INTO items (id, name, status) VALUES (3, 'Item 3', 'inactive')")
        .await
        .expect("Failed to insert row 3");
    client
        .simple_query("INSERT INTO items (id, name, status) VALUES (4, 'Item 4', 'deleted')")
        .await
        .expect("Failed to insert row 4");

    // Test equality comparison
    let results = client
        .simple_query("SELECT id, name, status FROM items WHERE status = 'active'")
        .await
    ...
    let results = client
        .simple_query("SELECT id, name, status FROM items WHERE status != 'deleted'")
        .await
        .expect("Failed to query with != WHERE clause");
...
async fn test_enum_where_in_list() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE priority AS ENUM ('low', 'medium', 'high', 'critical')")
        .await
        .expect("Failed to create ENUM type");
    ...
    for result in &results {
        if let tokio_postgres::SimpleQueryMessage::Row(row) = result {
            data_rows.push((row.get("id").unwrap(), row.get("priority").unwrap()));
        }
    }
    ...
    // Test NOT IN clause
    let results = client
        .simple_query("SELECT COUNT(*) as cnt FROM tasks WHERE priority NOT IN ('low')")
        .await
...
async fn test_enum_where_ordering() {
    ...
    client
        .simple_query("CREATE TYPE severity AS ENUM ('info', 'warning', 'error', 'fatal')")
        .await
        .expect("Failed to create ENUM type");

    // Create a table
    client
        .simple_query("CREATE TABLE logs (id INTEGER PRIMARY KEY, message TEXT, severity severity)")
        .await
        .expect("Failed to create table");
...
async fn test_enum_where_null_handling() {
    ...
    // Test IS NULL
    let results = client
        .simple_query("SELECT id FROM posts WHERE state IS NULL ORDER BY id")
        .await
    ...
    // Test IS NOT NULL
    let results = client
        .simple_query("SELECT COUNT(*) as cnt FROM posts WHERE state IS NOT NULL")
        .await
