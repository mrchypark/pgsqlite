...
fn test_migration_history() {
    ...
    let conn = runner.into_connection();
    let migrations: Vec<(i32, String, String)> = conn
        .prepare("SELECT version, name, status FROM __pgsqlite_migrations ORDER BY version")
        .unwrap()
    ...
    assert_eq!(
        migrations[1],
        (2, "enum_type_support".to_string(), "completed".to_string())
    );
    assert_eq!(
        migrations[2],
        (
            3,
            "datetime_timezone_support".to_string(),
            "completed".to_string()
        )
    );
    assert_eq!(
        ...
        (
            4,
            "datetime_integer_storage".to_string(),
            "completed".to_string()
        )
    );
    assert_eq!(
        migrations[4],
        (5, "pg_catalog_tables".to_string(), "completed".to_string())
    );
    assert_eq!(
        migrations[5],
        (
            6,
            "varchar_constraints".to_string(),
            "completed".to_string()
        )
    );
    assert_eq!(
        ...
        (
            7,
            "numeric_constraints".to_string(),
            "completed".to_string()
        )
    );
    assert_eq!(
        migrations[7],
        (8, "array_support".to_string(), "completed".to_string())
    );
    assert_eq!(
        migrations[8],
        (9, "fts_support".to_string(), "completed".to_string())
    );
    assert_eq!(
        migrations[9],
        (
            10,
            "typcategory_support".to_string(),
            "completed".to_string()
        )
    );
    assert_eq!(
