...
async fn test_datetime_conversion_success() {
    ...
    client
        .simple_query("INSERT INTO dt_test VALUES (1, '2024-01-15', '14:30:00')")
        .await
        .unwrap();

    // Verify storage is INTEGER
    let storage_check = client
        .simple_query("SELECT typeof(date_col), typeof(time_col) FROM dt_test WHERE id = 1")
        .await
        .unwrap();

    if let Some(msg) = storage_check
        .into_iter()
    ...
    let retrieve = client
        .simple_query("SELECT date_col, time_col FROM dt_test WHERE id = 1")
        .await
        .unwrap();

    if let Some(msg) = retrieve
        .into_iter()
        .find(|m| matches!(m, tokio_postgres::SimpleQueryMessage::Row(_)))
        && let tokio_postgres::SimpleQueryMessage::Row(data) = msg
    {
