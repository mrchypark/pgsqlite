...
use crate::cache::{
    CachedParameterInfo, GLOBAL_PARAMETER_CACHE, GLOBAL_ROW_DESCRIPTION_CACHE, RowDescriptionKey,
};
use crate::catalog::CatalogInterceptor;
use crate::protocol::{BackendMessage, FieldDescription};
use crate::query::ParameterParser;
use crate::session::{DbHandler, GLOBAL_QUERY_CACHE, Portal, PreparedStatement, SessionState};
use crate::translator::{CastTranslator, JsonTranslator, ReturningTranslator};
...
impl ExtendedQueryHandler {
    ...
    {
        info!(
            "PARSE: Starting parse for statement '{}', query: {}",
            name, query
        );
        ...
        if !name.is_empty() {
            ...
            if let Some(existing) = statements.get(&name) {
                ...
                if existing.query == query && existing.param_types == param_types {
                    ...
                    info!(
                        "PARSE: Using cached statement '{}' with {} field_descriptions",
                        name,
                        existing.field_descriptions.len()
                    );
                    drop(statements);
                    framed
        ...
        cleaned_query =
            crate::query::executor::expand_user_sql_functions(db, session, &cleaned_query).await?;
        ...
        let mut extracted_param_types =
            vec![0i32; ParameterParser::count_parameters(&cleaned_query)];
            ...
            for (index, param_name) in python_params.iter().enumerate() {
                ...
                if let Some(cast_start) = cleaned_query.find(&param_pattern) {
                    ...
                    if let Some(type_name) =
                        ParameterParser::extract_cast_type_name(&cleaned_query, type_start)
                    {
                        let type_oid = ParameterParser::cast_type_to_oid(type_name);
                        if type_oid != 0 {
        ...
        if extracted_param_types.iter().any(|&t| t != 0) {
            ...
            if let Some(cached_info) = GLOBAL_PARAMETER_CACHE.get(&query) {
                ...
                debug!(
                    "Using cached parameter types for query: {:?}",
                    actual_param_types
                );
            } else {
                ...
                if let Some(cached) = GLOBAL_QUERY_CACHE.get(&query) {
                    ...
                    debug!(
                        "Using cached parameter types from query cache: {:?}",
                        actual_param_types
                    );
                    ...
                    let (analyzed_types, original_types_opt, table_name, column_names) =
                        if query_starts_with_ignore_case(&query, "INSERT") {
                            match Self::analyze_insert_params(&query, db).await {
                                Ok((types, orig_types)) => {
                                    debug!(
                                        "Analyzed INSERT parameter types: {:?} (original: {:?})",
                                        types, orig_types
                                    );

                                    // Extract table and columns for caching
                                    let (table, cols) =
                                        crate::types::QueryContextAnalyzer::get_insert_column_info(
                                            &query,
                                        )
                    ...
                    GLOBAL_PARAMETER_CACHE.insert(
                        ...
                        CachedParameterInfo {
                            ...
                            original_types: original_types_opt
                                .unwrap_or_else(|| actual_param_types.clone()),
                            table_name,
                            column_names,
                            created_at: std::time::Instant::now(),
                        },
                    ...
                    ) && let Some(statement) = parsed.first()
                    {
                        ...
                        GLOBAL_QUERY_CACHE.insert(
                            ...
                            crate::cache::CachedQuery {
                                ...
                                normalized_query: crate::cache::QueryCache::normalize_query(
                                    &cleaned_query,
                                ),
                            },
                        );
        ...
        let mut translated_for_analysis =
            if crate::translator::CastTranslator::needs_translation(&cleaned_query) {
                db.with_session_connection(&session.id, |conn| {
                    Ok(crate::translator::CastTranslator::translate_query(
                        &cleaned_query,
                        Some(conn),
                    ))
                })
        ...
        if crate::translator::NumericFormatTranslator::needs_translation(&translated_for_analysis) {
            translated_for_analysis = db
                .with_session_connection(&session.id, |conn| {
                    Ok(crate::translator::NumericFormatTranslator::translate_query(
        ...
        #[cfg(feature = "unified_processor")]
        if crate::translator::DateTimeTranslator::needs_translation(&cleaned_query) {
            let (_, metadata) =
                crate::translator::DateTimeTranslator::translate_with_metadata(&cleaned_query);
        ...
        if crate::translator::DateTimeTranslator::needs_translation(&translated_for_analysis) {
            let (translated, metadata) =
                crate::translator::DateTimeTranslator::translate_with_metadata(
                    &translated_for_analysis,
                );
        ...
        #[cfg(not(feature = "unified_processor"))] // Skip when using unified processor
        {
            ...
            translated_for_analysis =
                CatalogFunctionTranslator::translate(&translated_for_analysis);
            translated_for_analysis =
                PgTableIsVisibleTranslator::translate(&translated_for_analysis);
        }
        ...
        #[cfg(not(feature = "unified_processor"))] // Skip when using unified processor
        {
            ...
            let (translated, metadata) =
                RowToJsonTranslator::translate_row_to_json(&translated_for_analysis);
            if translated != translated_for_analysis {
        ...
        if crate::translator::ArithmeticAnalyzer::needs_analysis(&translated_for_analysis) {
            let arithmetic_metadata =
                crate::translator::ArithmeticAnalyzer::analyze_query(&translated_for_analysis);
            translation_metadata.merge(arithmetic_metadata);
            debug!(
                "Found {} arithmetic type hints",
        ...
        info!(
            "PARSE: Analyzing query '{}' for field descriptions",
            translated_for_analysis
        );
        ...
        let field_descriptions = if query_is_select_like(&cleaned_query) {
            ...
            if cleaned_query.contains("pg_catalog")
                || cleaned_query.contains("pg_type")
                || cleaned_query.contains("pg_class")
                || cleaned_query.contains("pg_attribute")
                || cleaned_query.contains("pg_namespace")
                || cleaned_query.contains("pg_enum")
                ...
                || cleaned_query.contains("pg_stat_io")
            {
                ...
                let test_response = db
                    .query_with_session_cached(&test_query, &session.id, cached_conn.as_ref())
                    .await;

                match test_response {
                    Ok(response) => {
                        info!(
                            "Test query returned {} columns: {:?}",
                        ...
                        if let Some(ref table) = table_name {
                            ...
                            for col_name in &response.columns {
                                ...
                                {
                                    info!(
                                        "PARSE: Found schema type for column '{}' in table '{}': {}",
                                        col_name, table, pg_type
                                    );
                                        ...
                                        && let Some(src_col) = captures.get(2)
                                    {
                                        ...
                                        {
                                            info!(
                                                "Found type for aliased column '{}' from query pattern '{}.{}' in table '{}': {}",
                                                col_name,
                                                src_table_name,
                                                src_col_name,
                                                table,
                                    ...
                                    if let Some(hint) = translation_metadata.get_hint(col_name) {
                                        ...
                                        if let Some(ref source_col) = hint.source_column {
                                            ...
                                            {
                                                info!(
                                                    "Found source column type for datetime expression '{}' -> '{}': {}",
                                                    col_name, source_col, source_type
                                                );
                                                schema_types.insert(col_name.clone(), source_type);
                                            } else if let Some(suggested_type) =
                                                &hint.suggested_type
                                            {
                                                info!(
                                                    "Using suggested type for datetime expression '{}': {:?}",
                                                    col_name, suggested_type
                                                );
                                                // Convert PgType to the string format used in schema
                                                let type_string = match suggested_type {
                                                    crate::types::PgType::Float8 => {
                                                        "DOUBLE PRECISION"
                                                    }
                                                    ...
                                                    crate::types::PgType::Timestamptz => {
                                                        "TIMESTAMPTZ"
                                                    }
                                            ...
                                            info!(
                                                "Found type hint from translation for '{}': {:?}",
                                                col_name, suggested_type
                                            );
                                            // Convert PgType to the string format used in schema
                                            let type_string = match suggested_type {
                                            ...
                                            schema_types
                                                .insert(col_name.clone(), type_string.to_string());
                                        }
                                    } else {
                                        // Try to find source table and column if this is an alias
                                        info!(
                                            "Attempting to extract source for alias: '{}' from query: {}",
                                            col_name, cleaned_query
                                        );
                                        if let Some((source_table, source_col)) =
                                            Self::extract_source_table_column_for_alias(
                                                &cleaned_query,
                                                col_name,
                                            )
                                        {
                                            info!(
                                                "Successfully resolved alias '{}' -> table '{}', column '{}'",
                                            ...
                                            {
                                                info!(
                                                    "Found schema type for alias '{}' -> source column '{}.{}': {}",
                                                    col_name, source_table, source_col, pg_type
                                                );
                                                schema_types.insert(col_name.clone(), pg_type);
                                            } else {
                                                info!(
                                                    "No schema type found for '{}.{}'",
                                                    source_table, source_col
                                                );
                                            }
                                        } else {
                                            info!(
                                                "Could not extract source table/column for alias '{}' from query",
                                                col_name
                                            );
                                        }
                                    }
                        ...
                        for (i, col_name) in response.columns.iter().enumerate() {
                            ...
                            let inferred_type = {
                                ...
                                // For parameter columns (NULL from SELECT $1), try to match with parameters
                                    ...
                                    if is_simple_param_select {
                                        ...
                                        info!(
                                            "Simple parameter SELECT detected, column {} likely corresponds to parameter {}",
                                            i,
                                            i + 1
                                        );

                                        // Check actual_param_types which includes inferred types
                                        if !actual_param_types.is_empty()
                                            && i < actual_param_types.len()
                                        {
                                            let param_type = actual_param_types[i];
                                            if param_type != 0
                                                && param_type != PgType::Text.to_oid()
                                            {
                                                info!(
                                                    "Using actual param type {} for column {}",
                                                    param_type, i
                                                );
                                                param_type
                                                ...
                                                && i < param_types.len()
                                            {
                                                ...
                                                if param_type != 0 {
                                                    info!(
                                                        "Using provided param type {} for column {}",
                                                        param_type, i
                                                    );
                                                    param_type
                                                } else {
                                                    info!(
                                                        "No specific param type for column {}, defaulting to TEXT",
                                                        i
                                                    );
                                                    PgType::Text.to_oid()
                                                }
                                ...
                                } else if let Some(hint) = translation_metadata.get_hint(col_name)
                                {
                                    ...
                                    if hint.expression_type
                                        == Some(
                                            crate::translator::ExpressionType::ArithmeticOnFloat,
                                        )
                                    {
                                        debug!(
                                            "Arithmetic expression '{}' detected with ArithmeticOnFloat hint, returning FLOAT8",
                                            col_name
                                        );
                                        ...
                                        debug!(
                                            "Using type hint from translation metadata for '{}': {:?}",
                                            col_name, suggested_type
                                        );
                                        ...
                                        if let Some(source_col) = &hint.source_column {
                                            ...
                                            let (source_table, source_column) = if source_col
                                                .contains('.')
                                            {
                                                let parts: Vec<&str> =
                                                    source_col.split('.').collect();
                                                (parts[0], parts[1])
                                            } else if let Some(ref table) = table_name {
                                                (table.as_str(), source_col.as_str())
                                            } else {
                                                info!(
                                                    "Arithmetic expression '{}' has no table context and source column '{}' has no table prefix",
                                                    col_name, source_col
                                                );
                                                ("", source_col.as_str()) // Will likely fail, but try anyway
                                            };

                                            if let Ok(Some(source_type_str)) = db
                                                .get_schema_type_with_session(
                                                    &session.id,
                                                    source_table,
                                                    source_column,
                                                ...
                                                .await
                                            {
                                                ...
                                                info!(
                                                    "Resolved arithmetic expression '{}' from source '{}' (type {} -> OID {}) -> {:?}",
                                                    col_name,
                                                    source_col,
                                                    source_type_str,
                                                    source_type_oid,
                                                ...
                                                info!(
                                                    "Could not resolve source column type for arithmetic expression '{}'",
                                                    col_name
                                                );
                                        ...
                                        info!(
                                            "No type hint found in translation metadata for '{}'",
                                            col_name
                                        );
                            ...
                            if let Some(pg_type) = schema_types.get(col_name) {
                                ...
                                info!(
                                    "PARSE: Column '{}': using schema type '{}' -> OID {}",
                                    col_name, pg_type, type_oid
                                );
                            ...
                            {
                                info!(
                                    "Column '{}' identified with type OID {} from aggregate detection",
                                    col_name, oid
                                );
                            ...
                            if cleaned_query.contains("decimal_mul")
                                || cleaned_query.contains("decimal_add")
                                || cleaned_query.contains("decimal_sub")
                                || cleaned_query.contains("decimal_div")
                            {
                                // This query uses decimal arithmetic functions
                                // Check if this column might be the result
                                if col_name.contains("total")
                                    || col_name.contains("sum")
                                    || col_name.contains("price")
                                    || col_name.contains("amount")
                                    || col_name == "?column?"
                                {
                                    info!(
                                        "Column '{}' appears to be result of decimal arithmetic",
                                        col_name
                                    );
                            ...
                            if !response.rows.is_empty() {
                                if let Some(value) = response.rows[0].get(i) {
                                    ...
                                    let inferred_type = if table_name
                                        .as_ref()
                                        .is_some_and(|t| t == "__pgsqlite_metadata")
                                        && col_name == "value"
                                    {
                                        info!(
                                            "PARSE: Column 'value' in __pgsqlite_metadata: forcing TEXT type"
                                        );
                                        PgType::Text.to_oid()
                                    } else {
                                ...
                                info!(
                                    "Column '{}': no data rows, attempting schema-based type inference",
                                    col_name
                                );
                                    ...
                                    )
                                {
                                        ...
                                        .await
                                    {
                                        Ok(Some(pg_type_str)) => {
                                            ...
                                            info!(
                                                "Column '{}': resolved type from schema '{}.{}' -> {} (OID {})",
                                                col_name,
                                                source_table,
                                                source_col,
                                                pg_type_str,
                                        ...
                                        Ok(None) => {
                                            info!(
                                                "Column '{}': no schema type found for '{}.{}', defaulting to text",
                                                col_name, source_table, source_col
                                            );
                                    ...
                                    info!(
                                        "Column '{}': could not extract source table.column, analyzing query structure",
                                        col_name
                                    );
                                        ...
                                        && let Some(underscore_pos) = col_name.find('_')
                                    {
                                            ...
                                            .await
                                        {
                                            Ok(Some(pg_type_str)) => {
                                                let type_oid = crate::types::SchemaTypeMapper::pg_type_string_to_oid(&pg_type_str);
                                                info!(
                                                    "Column '{}': resolved type from table_column pattern '{}_{}'-> {} (OID {})",
                                                    col_name,
                                                    potential_table,
                                                    potential_column,
                                                    pg_type_str,
                                    ...
                                    if !type_found {
                                        ...
                                        if let Some(table_name) =
                                            extract_table_name_from_select(&cleaned_query)
                                        {
                                            info!(
                                                "Column '{}': extracted table '{}' from FROM clause, assuming column exists",
                                                col_name, table_name
                                            );
                                            ...
                                            info!(
                                                "Column '{}': could not extract table name from query, defaulting to text",
                                                col_name
                                            );
                                            inferred_types.push(PgType::Text.to_oid());
                                        }
                        ...
                        info!(
                            "Parsed {} field descriptions from query with inferred types",
                            fields.len()
                        );
                        fields
                    }
    ...
    fn extract_source_table_column_for_alias(query: &str, alias: &str) -> Option<(String, String)> {
        ...
        info!(
            "extract_source_table_column_for_alias: Looking for alias '{}' in query",
            alias
        );
        ...
        if let Some(as_pos) = query_upper.find(&as_pattern) {
            ...
            if let Some(comma_pos) = trimmed.rfind(',') {
                ...
                info!(
                    "Found comma at position {}, expr_start = {}",
                    comma_pos, expr_start
                );
            } else if let Some(select_pos) = query_upper.find(select_upper) {
                expr_start = select_pos + select_upper.len();
                info!(
                    "Found SELECT at position {}, expr_start = {}",
                    select_pos, expr_start
                );
            }
            ...
            if expr_start >= trimmed.len() {
                info!(
                    "expr_start {} >= trimmed.len() {}, using 0",
                    expr_start,
                    trimmed.len()
                );
                expr_start = 0;
            ...
            if let Some(dot_pos) = expr.rfind('.') {
                ...
                {
                    info!(
                        "Successfully parsed alias '{}' -> table '{}', column '{}'",
                        alias, table_part, column_part
                    );
            ...
            // For simple column references without table prefix
                ...
                if let Some(table_name) = extract_table_name_from_select(query) {
                    info!(
                        "Successfully parsed alias '{}' -> table '{}' (from FROM clause), column '{}'",
                        alias, table_name, expr
                    );
            ...
            info!(
                "Failed to parse expression '{}' - not a valid table.column or simple column",
                expr
            );
        } else {
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if !is_simple_query {
            ...
            // Check if this statement used Python-style parameters and reorder values if needed
            {
                ...
                if let Some(param_names) = python_mappings.get(&statement) {
                    info!(
                        "Statement '{}' used Python parameters: {:?}",
                        statement, param_names
                    );
                    ...
                    info!(
                        "Python parameter mapping found, values should already be in correct order"
                    );
                }
            }
        }
        ...
        let stmt = if statement.is_empty() {
            ...
            statements.get("").or_else(|| statements.get(&statement))
        } else {
            statements.get(&statement)
        }
        .ok_or_else(|| {
            info!(
                "Statement lookup failed for '{}', available statements: {:?}",
                statement,
        ...
        let needs_inference =
            stmt.param_types.is_empty() || stmt.param_types.iter().all(|&t| t == 0);
        ...
        if needs_inference && !values.is_empty() {
            ...
            for (i, val) in values.iter().enumerate() {
                ...
                info!(
                    "  Param {}: inferred type OID {} from value (format={})",
                    i + 1,
                    inferred_type,
                    format
                );
        ...
        for (i, val) in values.iter().enumerate() {
            ...
            if let Some(v) = val {
                info!(
                    "  Param {}: {} bytes, expected type OID {}, format {} ({})",
                    i + 1,
                    v.len(),
                    expected_type,
                    format,
                ...
                info!(
                    "  Param {}: NULL, expected type OID {}, format {} ({})",
                    i + 1,
                    expected_type,
                    format,
                    if format == 1 { "binary" } else { "text" }
        ...
        session
            .portal_manager
            .create_portal(portal.clone(), portal_obj.clone())?;
        ...
        // Send BindComplete
        framed
            .send(BackendMessage::BindComplete)
            .await
            .map_err(PgSqliteError::Io)?;
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        translated_query =
            crate::query::executor::expand_user_sql_functions(db, session, &translated_query)
                .await?;
        translated_query =
            ...
            && (result_formats.is_empty() || result_formats[0] == 0)
        {
                ...
                if let Some(stmt) = statements.get(&statement_name) {
                    if !stmt.field_descriptions.is_empty() {
                        ...
                        if let Some(select_end) = select_end {
                            ...
                            info!(
                                "Ultra-fast path: Parsing {} columns from SELECT clause",
                                columns.len()
                            );
                            for col_expr in columns {
                                info!(
                                    "Ultra-fast path: Processing column expression: '{}'",
                                    col_expr
                                );
                                ...
                                let col_name =
                                    if let Some(as_pos) = col_expr.to_uppercase().rfind(" AS ") {
                                ...
                                if let Some((source_table, source_col)) =
                                    Self::extract_source_table_column_for_alias(&query, col_name)
                                {
                                    if let Ok(Some(pg_type_str)) = db
                                        .get_schema_type_with_session(
                                            &session.id,
                                            &source_table,
                                            &source_col,
                                        ...
                                        .await
                                    {
                                        ...
                                        info!(
                                            "Ultra-fast path: Pre-execution type inference for '{}' from '{}.{}' -> {} (OID {})",
                                            col_name,
                                            source_table,
                                            source_col,
                                            pg_type_str,
                                    ...
                                    if let Ok(Some(pg_type_str)) = db
                                        .get_schema_type_with_session(&session.id, table, col_expr)
                                        .await
                                    {
                                        let type_oid =
                                            crate::types::SchemaTypeMapper::pg_type_string_to_oid(
                                        ...
                                        info!(
                                            "Ultra-fast path: Pre-execution type inference for '{}' from table '{}' -> {} (OID {})",
                                ...
                                if !found_type {
                                    ...
                                    info!(
                                        "Ultra-fast path: Could not infer type for '{}', defaulting to TEXT",
                                        col_name
                                    );
                                    inferred_types.push(PgType::Text.to_oid());
                                }
                            ...
                            info!(
                                "Ultra-fast path: Could not find FROM clause in query, cannot parse columns"
                            );
                        }
            ...
            let converted_values: Vec<Option<Vec<u8>>> = bound_values
                .iter()
                .enumerate()
                .map(|(i, value)| {
                    match value {
                                        ...
                                        || t == PgType::Timestamptz.to_oid() =>
                                    {
                                        ...
                                        if bytes.len() == 8 {
                                            ...
                                            let unix_microseconds =
                                                pg_microseconds + 946_684_800_000_000;
            ...
            match db
                .execute_with_params(query_to_execute, &converted_values, &session.id)
                .await
            {
                Ok(response) => {
                    ...
                    for row in response.rows {
                        ...
                        for (i, field_type) in field_types.iter().enumerate() {
                                ...
                                && let Ok(s) = std::str::from_utf8(value)
                            {
                                ...
                                if *field_type == PgType::Timestamp.to_oid()
                                    || *field_type == PgType::Timestamptz.to_oid()
                                {
                                    info!(
                                        "  Column {}: TIMESTAMP type OID {}, raw value: '{}'",
                                        i, field_type, s
                                    );
                                } else if *field_type == PgType::Text.to_oid()
                                    && s.parse::<i64>().is_ok()
                                {
                    ...
                    if max_rows > 0 && row_count >= max_rows as usize {
                        ...
                        framed
                            .send(BackendMessage::PortalSuspended)
                            .await
                            .map_err(PgSqliteError::Io)?;
                    }
        ...
        let validation_error = if query_starts_with_ignore_case(query_to_use, "INSERT") {
            if let Some(table_name) = Self::extract_table_name_from_insert(query_to_use) {
                ...
                let validation_query = Self::substitute_parameters(
                    query_to_use,
                    &bound_values,
                    &param_formats,
                    &param_types,
                ...
                match db
                    .with_session_connection(&session.id, |conn| {
                        match NumericValidator::validate_insert(
                            conn,
                            &validation_query,
                            &table_name,
                        ) {
                ...
                {
                    Ok(()) => None,
                    Err(PgSqliteError::Sqlite(rusqlite::Error::SqliteFailure(_, Some(msg))))
            ...
            if let Some(table_name) = Self::extract_table_name_from_update(query_to_use) {
                ...
                match db
                    .with_session_connection(&session.id, |conn| {
                        match NumericValidator::validate_update(
                            conn,
                            &validation_query,
                            &table_name,
                        ) {
        ...
        if final_query.contains("pg_catalog")
            ...
            || final_query.contains("pg_constraint")
            || final_query.contains("pg_index")
            || final_query.contains("pg_depend")
        {
            info!(
                "Detected catalog query in extended protocol: {}",
                final_query
            );
        }
            ...
            || query_starts_with_ignore_case(&final_query, "ROLLBACK")
        {
            ...
            crate::query::SetHandler::handle_set_command_extended(
                framed,
                session,
                &final_query,
                skip_row_desc,
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if typ == b'S' {
            ...
            let is_catalog_query = query.contains("pg_catalog")
                || query.contains("pg_type")
                || query.contains("pg_namespace")
                || query.contains("pg_class")
                || query.contains("pg_attribute")
                || query.contains("pg_constraint")
            ...
            if !stmt.field_descriptions.is_empty() {
                info!(
                    "Sending RowDescription with {} fields in Describe",
                    stmt.field_descriptions.len()
                );
                ...
                if is_catalog_query
                    || query.contains("pg_attribute")
                    || query.contains("a.attnotnull")
                    || query.contains("a.atthasdef")
                {
                ...
                for (i, fd) in corrected_fields.iter().enumerate() {
                    info!(
                        "Field {}: name='{}', type_oid={}, table_oid={}",
                        i, fd.name, fd.type_oid, fd.table_oid
                    );
                }
                framed
                    .send(BackendMessage::RowDescription(corrected_fields))
                    .await
                    .map_err(PgSqliteError::Io)?;
                ...
                info!(
                    "Catalog query detected in Describe, generating field descriptions for: {}",
                    query
                );
                    ...
                    if let Some(sqlparser::ast::Statement::Query(query_stmt)) = parsed.first() {
                        if let sqlparser::ast::SetExpr::Select(select) = &*query_stmt.body {
                            ...
                            if is_select_star {
                                ...
                                if let Some(predefined_fields) = crate::catalog::column_metadata::select_star_field_descriptions_for_catalog_query(query) {
                ...
                if !field_descriptions.is_empty() {
                    info!(
                        "Sending RowDescription with {} catalog fields in Describe",
                    ...
                    if let Some(stmt_mut) = statements_mut.get_mut(&name) {
                        ...
                        info!(
                            "Updated statement '{}' with {} catalog field descriptions",
                            name,
                            field_descriptions.len()
                        );
                    }
            ...
            let statements = session.prepared_statements.read().await;
            let stmt = statements.get(&portal.statement_name).ok_or_else(|| {
                PgSqliteError::Protocol(format!("Unknown statement: {}", portal.statement_name))
            })?;
            ...
            if !stmt.field_descriptions.is_empty() {
                ...
                if let Some(ref inferred_types) = portal.inferred_param_types {
                    info!(
                        "Describe portal: inferred types available: {:?}",
                        inferred_types
                    );
                    ...
                    for (col_idx, field) in fields.iter_mut().enumerate() {
                            ...
                            || field.name.starts_with('$')
                        {
                            ...
                            if let Some(&inferred_type) = inferred_types.get(param_idx) {
                                info!(
                                    "Updating column '{}' at index {} (param {}) type from {} to {}",
                                    field.name,
                                    col_idx,
                                    param_idx + 1,
                                    field.type_oid,
                                ...
                                info!(
                                    "No inferred type for column '{}' at index {} (param {})",
                                    field.name,
                                    col_idx,
                                    param_idx + 1
                                );
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        // Send CloseComplete
        framed
            .send(BackendMessage::CloseComplete)
            .await
            .map_err(PgSqliteError::Io)?;
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if let Ok(Some(response)) = db
            .try_execute_fast_path_with_params(query, &rusqlite_params, &session.id)
            .await
        {
            if response.columns.is_empty() {
                // DML operation - send command complete
                let tag = match fast_query.operation {
                    crate::query::FastPathOperation::Insert => {
                        format!("INSERT 0 {}", response.rows_affected)
                    }
                    crate::query::FastPathOperation::Update => {
                        format!("UPDATE {}", response.rows_affected)
                    }
                    crate::query::FastPathOperation::Delete => {
        ...
        if let Ok(response) =
            Self::try_statement_pool_execution(db, session, query, &rusqlite_params, fast_query)
                .await
        {
            if response.columns.is_empty() {
                // DML operation
    ...
    async fn try_statement_pool_execution(
        ...
        let byte_params: Vec<Option<Vec<u8>>> = params
            .iter()
            .map(|v| match v {
        ...
        match fast_query.operation {
            crate::query::FastPathOperation::Select => db
                .query_with_statement_pool_params(query, &byte_params, &session.id)
                .await
                .map_err(|e| PgSqliteError::Protocol(e.to_string())),
            _ => db
                .execute_with_statement_pool_params(query, &byte_params, &session.id)
                .await
                .map_err(|e| PgSqliteError::Protocol(e.to_string())),
        }
    }
    ...
    fn convert_parameter_to_value(
        ...
        if format == 0 {
            ...
            match param_type {
                t if t == PgType::Bool.to_oid() => Ok(rusqlite::types::Value::Integer(
                    if text == "t" || text == "true" { 1 } else { 0 },
                )), // BOOL
                t if t == PgType::Int8.to_oid() => Ok(rusqlite::types::Value::Integer(
                    text.parse::<i64>()
                        .map_err(|_| PgSqliteError::Protocol("Invalid int8".to_string()))?,
                )), // INT8
                t if t == PgType::Int4.to_oid() => Ok(rusqlite::types::Value::Integer(
                    text.parse::<i64>()
                        .map_err(|_| PgSqliteError::Protocol("Invalid int4".to_string()))?,
                )), // INT4
                t if t == PgType::Int2.to_oid() => Ok(rusqlite::types::Value::Integer(
                    text.parse::<i64>()
                        .map_err(|_| PgSqliteError::Protocol("Invalid int2".to_string()))?,
                )), // INT2
                t if t == PgType::Float4.to_oid() => Ok(rusqlite::types::Value::Real(
                    text.parse::<f64>()
                        .map_err(|_| PgSqliteError::Protocol("Invalid float4".to_string()))?,
                )), // FLOAT4
                t if t == PgType::Float8.to_oid() => Ok(rusqlite::types::Value::Real(
                    text.parse::<f64>()
                        .map_err(|_| PgSqliteError::Protocol("Invalid float8".to_string()))?,
                )), // FLOAT8
                t if t == PgType::Date.to_oid() => {
                    // DATE - convert to days since epoch
                    match crate::types::ValueConverter::convert_date_to_unix(text) {
                        Ok(days_str) => Ok(rusqlite::types::Value::Integer(
                            days_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol("Invalid date days".to_string())
                            })?,
                        )),
                ...
                t if t == PgType::Time.to_oid() => {
                    ...
                    match crate::types::ValueConverter::convert_time_to_seconds(text) {
                        Ok(micros_str) => Ok(rusqlite::types::Value::Integer(
                            micros_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol("Invalid time microseconds".to_string())
                            })?,
                        )),
                ...
                t if t == PgType::Timestamp.to_oid() => {
                    ...
                    match crate::types::ValueConverter::convert_timestamp_to_unix(text) {
                        Ok(micros_str) => Ok(rusqlite::types::Value::Integer(
                            micros_str.parse::<i64>().map_err(|_| {
                                PgSqliteError::Protocol(
                                    "Invalid timestamp microseconds".to_string(),
                                )
                            })?,
                        )),
            ...
            Err(PgSqliteError::Protocol(
                "Binary format not supported in fast path".to_string(),
            ))
        }
    }
    ...
    {
        debug!(
            "send_data_rows_only called with {} rows",
            response.rows.len()
        );
    ...
    {
        debug!(
            "send_select_response called with {} columns: {:?}",
            response.columns.len(),
            response.columns
        );
        ...
        let needs_conversion = field_types
            .map(|types| {
                let has_timestamp = types.iter().any(
                    |&t| {
                        t == PgType::Timestamp.to_oid()
                            || t == PgType::Timestamptz.to_oid()
                            || t == PgType::Text.to_oid()
                    }, // TEXT columns might contain timestamps
                );
                info!(
                    "send_select_response: field_types={:?}, needs_conversion={}",
                    types, has_timestamp
                );
                has_timestamp
            })
            ...
            if let Some(types) = field_types {
                ...
                for row in response.rows {
                    ...
                    for (i, cell) in row.iter().enumerate() {
                        ...
                        if type_oid == PgType::Timestamp.to_oid()
                            || type_oid == PgType::Timestamptz.to_oid()
                        {
                            ...
                            if let Some(bytes) = cell {
                                if let Ok(s) = std::str::from_utf8(bytes) {
                                    ...
                                    if let Ok(micros) = s.parse::<i64>() {
                                        ...
                                        if micros > 100_000_000_000
                                            && micros < 4_102_444_800_000_000
                                        {
                                            // This is likely a datetime value stored as INTEGER microseconds
                                            use crate::types::datetime_utils::format_microseconds_to_timestamp;
                                            let formatted =
                                                format_microseconds_to_timestamp(micros);
                                            info!(
                                                "Converting TEXT column timestamp value {} to formatted: {}",
                                                micros, formatted
                                            );
                                            converted_row.push(Some(formatted.into_bytes()));
                                        } else {
        ...
        framed
            .send(BackendMessage::CommandComplete {
                tag: format!("SELECT {}", response.rows_affected),
            })
            .await?;
    ...
    fn substitute_parameters(
        ...
        for (i, value) in values.iter().enumerate() {
            ...
            info!(
                "Processing parameter {}: format={}, type_oid={}, bytes_len={}",
                i + 1,
                format,
                param_type,
                    ...
                    if format == 1 {
                        ...
                        match param_type {
                            ...
                            t if t == PgType::Int4.to_oid() => {
                                ...
                                if bytes.len() == 4 {
                                    ...
                                    let value = i16::from_be_bytes([bytes[0], bytes[1]]);
                                    info!(
                                        "Decoded binary int16 (as int4) parameter {}: {}",
                                        i + 1,
                                        value
                                    );
                                    value.to_string()
                            ...
                            t if t == PgType::Money.to_oid() => {
                                ...
                                if bytes.len() == 8 {
                                    ...
                                    info!(
                                        "Decoded binary money parameter {}: {} cents -> {}",
                                        i + 1,
                                        cents,
                                        formatted
                                    );
                            ...
                            t if t == PgType::Text.to_oid() || t == PgType::Varchar.to_oid() => {
                                ...
                                match String::from_utf8(bytes.clone()) {
                                    ...
                                    Err(_) => {
                                        ...
                                        info!(
                                            "Failed to decode as UTF-8, treating as blob. Hex: {}",
                                            hex::encode(bytes)
                                        );
                                        format!("X'{}'", hex::encode(bytes))
                                    }
                                ...
                                || t == PgType::Timestamptz.to_oid() =>
                            {
                                ...
                                if bytes.len() == 8 {
                                    ...
                                    info!(
                                        "Decoded binary timestamp parameter {}: {} PG microseconds = {} Unix microseconds",
                                        i + 1,
                                        pg_micros,
                                        unix_micros
                                    );
                                    ...
                                    if query.contains("FROM (VALUES")
                                        && query.contains("SELECT CAST(p0")
                                    {
                                                ...
                                                .map(|dt| dt.naive_utc())
                                        {
                                            ...
                                            info!(
                                                "VALUES clause detected - formatting timestamp as: {}",
                                                formatted
                                            );
                                            format!("'{formatted}'")
                                        } else {
                            ...
                            t if t == PgType::Time.to_oid() || t == PgType::Timetz.to_oid() => {
                                ...
                                if bytes.len() == 8 {
                                    ...
                                    info!(
                                        "Decoded binary time parameter {}: {} microseconds",
                                        i + 1,
                                        micros
                                    );
                                    micros.to_string()
                            ...
                            0 => {
                                ...
                                if bytes.len() == 1 && (bytes[0] == 0 || bytes[0] == 1) {
                                    ...
                                    info!(
                                        "Inferred float64 parameter {} (unknown type): {}",
                                        i + 1,
                                        value
                                    );
                                    value.to_string()
                                    ...
                                    info!(
                                        "Unknown binary parameter type OID 0 for parameter {}, bytes: {}",
                                        i + 1,
                                        hex::encode(bytes)
                                    );
                                    format!("X'{}'", hex::encode(bytes))
                            ...
                            _ => {
                                ...
                                info!(
                                    "Unknown binary parameter type OID {} for parameter {}, bytes: {}",
                                    param_type,
                                    i + 1,
                                    hex::encode(bytes)
                                );
                        ...
                        match String::from_utf8(bytes.clone()) {
                            Ok(s) => {
                                ...
                                match param_type {
                                    t if t == PgType::Int4.to_oid()
                                        || t == PgType::Int8.to_oid()
                                        || t == PgType::Int2.to_oid()
                                        || t == PgType::Float4.to_oid()
                                        || t == PgType::Float8.to_oid() =>
                                    {
                                    ...
                                    t if t == PgType::Time.to_oid()
                                        || t == PgType::Timetz.to_oid() =>
                                    {
                                        // TIME types - convert to seconds since midnight
                                        match crate::types::ValueConverter::convert_time_to_seconds(
                                            &s,
                                        ) {
                                            Ok(seconds) => seconds,
                                            Err(e) => {
        ...
        let result =
            ParameterParser::substitute_parameters(query, &string_values).map_err(|e| {
                PgSqliteError::InvalidParameter(format!("Parameter substitution error: {e}"))
            })?;
                ...
                if let Some(paren_pos) = table_part.find('(') {
                    ...
                    if !all_values.is_empty() {
                        ...
                        let new_query = format!(
                            "INSERT INTO {table_name} {columns} VALUES {values_str}{returning_clause}"
                        );
    ...
    fn get_catalog_column_type(column_name: &str, query: &str) -> i32 {
            ...
            || ["not_null", "has_default", "is_not_null", "has_def"].contains(&column_name)
        {
            match column_name {
                ...
                "attname" | "attacl" | "attoptions" | "attfdwoptions" | "attmissingval" => {
                    PgType::Text.to_oid()
                }
                "attstattarget" | "attndims" | "attcacheoff" | "atttypmod" | "attinhcount" => {
                    PgType::Int4.to_oid()
                }
                "attlen" | "attnum" => PgType::Int2.to_oid(),
                "attbyval" | "attnotnull" | "atthasdef" | "atthasmissing" | "attisdropped"
                | "attislocal" | "not_null" | "has_default" | "is_not_null" | "has_def" => {
                    PgType::Bool.to_oid()
                }
                "attalign" | "attstorage" | "attcompression" | "attidentity" | "attgenerated" => {
                    PgType::Char.to_oid()
                }
                _ => PgType::Text.to_oid(),
            }
            ...
            match column_name {
                ...
                | "relfilenode" | "reltablespace" | "reltoastrelid" | "relrewrite" => OID_TYPE,
            ...
            match column_name {
                ...
                | "indisreplident" => PgType::Bool.to_oid(),
            ...
            match column_name {
                ...
                "condeferrable" | "condeferred" | "convalidated" | "conislocal"
            ...
            match column_name {
                ...
                "datfrozenxid" | "datminmxid" | "datcollate" | "datctype" | "datlocale"
                | "daticurules" | "datcollversion" | "datacl" => PgType::Text.to_oid(),
                _ => PgType::Text.to_oid(),
            }
                    ...
                    if format == 1 {
                        ...
                        match type_oid {
                            ...
                            t if t == PgType::BoolArray.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    if let Some(binary_data) =
                                        crate::protocol::binary::BinaryEncoder::encode_value(
                            ...
                            t if t == PgType::Int4Array.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    info!(
                                        "DEBUG: Calling encode_value for INT4Array with value: '{}'",
                                        s
                                    );
                                    let encoded_result =
                                        crate::protocol::binary::BinaryEncoder::encode_value(
                                    ...
                                    if let Some(ref binary_data) = encoded_result {
                                        info!(
                                            "DEBUG: Successfully encoded INT4Array to {} bytes",
                                            binary_data.len()
                                        );
                                        Some(binary_data.clone())
                                    } else {
                                        info!(
                                            "DEBUG: Failed to encode INT4Array, falling back to text"
                                        );
                                        Some(bytes.clone())
                                    }
                                } else {
                            ...
                            t if t == PgType::TextArray.to_oid()
                                || t == PgType::VarcharArray.to_oid()
                                || t == PgType::CharArray.to_oid() =>
                            {
                            ...
                            t if t == PgType::Uuid.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    if let Ok(uuid_bytes) =
                                        crate::types::uuid::UuidHandler::uuid_to_bytes(&s)
                                    {
                                        Some(uuid_bytes)
                                    } else {
                                        Some(bytes.clone())
                                ...
                                || t == PgType::Timestamptz.to_oid() =>
                            {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    ...
                                    if let Ok(unix_micros) = s.parse::<i64>() {
                                        ...
                                        Some(buf)
                                    } else if let Some(micros) =
                                        Self::timestamp_to_pg_microseconds(&s)
                                    {
                                        let mut buf = vec![0u8; 8];
                                        BigEndian::write_i64(&mut buf, micros);
                                        Some(buf)
                            ...
                            t if t == PgType::Numeric.to_oid() => {
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    ...
                                    if let Ok(decimal) = rust_decimal::Decimal::from_str(&s) {
                                        ...
                                        Some(
                                            crate::protocol::binary::BinaryEncoder::encode_numeric(
                                                &decimal,
                                            ),
                                        )
                                    } else {
                                        debug!(
                                            "Failed to parse NUMERIC value '{}', keeping as text",
                                            s
                                        );
                                        Some(bytes.clone())
                                    }
                            ...
                            t if t == PgType::Int4range.to_oid() => {
                                // int4range
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    if let Some(range_bytes) =
                                        Self::encode_range(&s, PgType::Int4.to_oid())
                                    {
                                        Some(range_bytes)
                                    } else {
                                        // If parsing fails, keep as text
                            ...
                            t if t == PgType::Int8range.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    if let Some(range_bytes) =
                                        Self::encode_range(&s, PgType::Int8.to_oid())
                                    {
                                        Some(range_bytes)
                                    } else {
                                        // If parsing fails, keep as text
                            ...
                            t if t == PgType::Numrange.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    if let Some(range_bytes) =
                                        Self::encode_range(&s, PgType::Numeric.to_oid())
                                    {
                                        Some(range_bytes)
                                    } else {
                                        // If parsing fails, keep as text
                            ...
                            t if t == PgType::Text.to_oid()
                                || t == PgType::Varchar.to_oid()
                                || t == PgType::Char.to_oid() =>
                            {
                            ...
                            t if t == PgType::Text.to_oid() => {
                                ...
                                if let Ok(s) = String::from_utf8(bytes.clone()) {
                                    ...
                                    if let Ok(micros) = s.parse::<i64>() {
                                        ...
                                        {
                                            info!(
                                                "Value {} is in timestamp range, converting...",
                                                micros
                                            );
                                            ...
                                            let formatted =
                                                format_microseconds_to_timestamp(micros);
                                            info!(
                                                "Converting presumed timestamp value {} to formatted timestamp: {}",
                                                micros, formatted
                                            );
                                            Some(formatted.into_bytes())
                                        } else {
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        info!(
            "execute_select: Checking if query is catalog query: {}",
            query
        );
        if query.contains("int_array_with_nulls") {
            info!(
                "DEBUG: execute_select called with array null query: {}",
                query
            );
        }
        let response = if let Some(catalog_result) =
            CatalogInterceptor::intercept_query(query, db.clone(), Some(session.clone())).await
        {
        ...
        info!(
            "EXECUTE: send_row_desc = {} for query: {}",
            send_row_desc, query
        );
        if send_row_desc {
            // Extract table name from query to look up schema
            ...
            let fields = if let Some(cached_fields) = GLOBAL_ROW_DESCRIPTION_CACHE.get(&cache_key) {
                ...
                if let Some(ref table) = table_name {
                    for col_name in &response.columns {
                            ...
                            .await
                        {
                            ...
                            // Look for pattern like "table.column AS alias" in the SELECT clause
                            let pattern =
                                format!(r"(?i)(\w+)\.(\w+)\s+AS\s+{}", regex::escape(col_name));
                            if let Ok(re) = regex::Regex::new(&pattern)
                                && let Some(captures) = re.captures(query)
                                && let Some(src_table) = captures.get(1)
                                && let Some(src_col) = captures.get(2)
                            {
                                ...
                                {
                                    info!(
                                        "Found type for aliased column '{}' from query pattern '{}.{}': {}",
                                        col_name, src_table_name, src_col_name, pg_type
                                    );
                                    schema_types.insert(col_name.clone(), pg_type);
                                }
                            }
                ...
                for (i, col_name) in response.columns.iter().enumerate() {
                    let col_lower = col_name.to_lowercase();
                    if col_lower.contains("max(")
                        || col_lower.contains("min(")
                        || col_lower.contains("sum(")
                        || col_lower.contains("avg(")
                    {
                ...
                let max_regex =
                    regex::Regex::new(r"\(\s*SELECT\s+MAX\s*\(\s*(\w+)\s*\)\s+FROM\s+(\w+)\s*\)")
                        .ok();
                let min_regex =
                    regex::Regex::new(r"\(\s*SELECT\s+MIN\s*\(\s*(\w+)\s*\)\s+FROM\s+(\w+)\s*\)")
                        .ok();
                        ...
                        if let Some(col_match) = captures.get(1) {
                            ...
                            if let Some(ref table) = lookup_table {
                                    ...
                                    .await
                                {
                                    ...
                                    info!(
                                        "Scalar subquery MAX({}) from table {} has type {} (OID {})",
                        ...
                        if let Some(col_match) = captures.get(1) {
                                    ...
                                    .await
                            {
                                let type_oid =
                                    crate::types::SchemaTypeMapper::pg_type_string_to_oid(&pg_type);
                                info!(
                                    "Scalar subquery MIN({}) from table {} has type {} (OID {})",
                                    col_name_inner, table, pg_type, type_oid
                                );
                                aggregate_types.insert(idx, type_oid);
                                continue;
                ...
                let should_correct = query.contains("pg_attribute")
                    || (query.contains("a.attnotnull") || query.contains("a.atthasdef"));
                info!(
                    "EXECUTE_SELECT: Boolean type correction check - should_correct={}, query contains pg_attribute={}, query contains a.attnotnull={}",
                    should_correct,
                    query.contains("pg_attribute"),
                    query.contains("a.attnotnull")
                );
            ...
            info!(
                "Sending RowDescription with {} fields during Execute with inferred types",
                fields.len()
            );
            framed
                .send(BackendMessage::RowDescription(fields))
        ...
        let (result_formats, field_types) = {
            ...
            let mut field_types: Vec<i32> = if stmt.field_descriptions.is_empty() {
                ...
                for (i, col_name) in response.columns.iter().enumerate() {
                    ...
                    {
                        info!(
                            "Column '{}' is aggregate function with type OID {} (field_types)",
                            col_name, oid
                        );
                    ...
                    if let Some(ref table) = table_name {
                        info!(
                            "EXECUTE: Looking up schema for column '{}' in table '{}'",
                            col_name, table
                        );
                        // Try to extract source table.column from alias
                        if let Some((source_table, source_col)) =
                            Self::extract_source_table_column_for_alias(&portal.query, col_name)
                        {
                            if let Ok(Some(pg_type_str)) = db
                                .get_schema_type_with_session(
                                    &session.id,
                                ...
                                .await
                            {
                                ...
                                info!(
                                    "Column '{}': resolved type from schema '{}.{}' -> {} (OID {}) in execute_select",
                            ...
                            if parts.len() == 2 {
                                ...
                                info!(
                                    "Column '{}': Attempting to resolve type from '{}.{}'",
                                    col_name, table_part, column_part
                                );
                                if let Ok(Some(pg_type_str)) = db
                                    .get_schema_type_with_session(
                                        &session.id,
                                    ...
                                    .await
                                {
                                    ...
                                    info!(
                                        "Column '{}': Could not find type for '{}.{}'",
                                        col_name, table_part, column_part
                                    );
                                }
                            } else {
                                info!(
                                    "Column '{}': Contains dot but not in expected format",
                                    col_name
                                );
                            }
                        } else if let Ok(Some(pg_type_str)) = db
                            .get_schema_type_with_session(&session.id, table, col_name)
                            .await
                        {
                            let type_oid =
                                crate::types::SchemaTypeMapper::pg_type_string_to_oid(&pg_type_str);
                            info!(
                                "Column '{}': resolved type from schema '{}.{}' -> {} (OID {}) in execute_select",
                    ...
                    if !found_type {
                        ...
                        let type_oid = if !response.rows.is_empty() {
                            if let Some(value) = response.rows[0].get(i) {
                                ...
                                {
                                    info!(
                                        "EXECUTE: Column 'value' in __pgsqlite_metadata: forcing TEXT type"
                                    );
                                    25 // TEXT
                                } else {
                                    let inferred =
                        ...
                        info!(
                            "Column '{}' inferred as type OID {} (field_types)",
                            col_name, type_oid
                        );
                        field_types.push(type_oid);
                    }
        ...
        let has_portal_state = session
            ...
            .is_some();
        let (rows_to_send, sent_count, total_rows) =
        ...
        for (row_idx, row) in rows_to_send.into_iter().enumerate() {
            ...
            if query.contains("int_array_with_nulls") {
                ...
                info!(
                    "DEBUG: field_types length: {}, contents: {:?}",
                    field_types.len(),
                    field_types
                );
                info!(
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if ReturningTranslator::has_returning_clause(query) {
            debug!(
                "Extended protocol: Query has RETURNING clause, using execute_dml_with_returning: {}",
                query
            );
        ...
        debug!(
            "Extended protocol: Executing DML query without RETURNING: {}",
            query
        );
        let cached_conn = Self::get_or_cache_connection(session, db).await;
        let response = db
            .execute_with_session_cached(query, &session.id, cached_conn.as_ref())
            .await?;
    ...
    async fn build_returning_field_descriptions(
        ...
        for (i, col_name) in columns.iter().enumerate() {
            ...
            let type_oid = if returning_clause == "*" || col_name == &col_name.to_lowercase() {
                ...
                if let Ok(Some(pg_type_str)) = db
                    .get_schema_type_with_session(&session.id, table_name, col_name)
                    .await
                {
            ...
            for (i, cell) in row.iter().enumerate() {
                    ...
                    && let Ok(value_str) = String::from_utf8(data.clone())
                {
                        ...
                        && let Ok(micros) = value_str.parse::<i64>()
                    {
                        // Convert microseconds to formatted timestamp
                        let formatted =
                            crate::types::datetime_utils::format_microseconds_to_timestamp(micros);
                        converted_row.push(Some(formatted.into_bytes()));
                        continue;
                    }
                }
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if query_starts_with_ignore_case(&base_query, "INSERT") {
            ...
            let table_name = ReturningTranslator::extract_table_from_insert(&base_query)
                .ok_or_else(|| {
                    PgSqliteError::Protocol("Failed to extract table name".to_string())
                })?;

            // Execute the INSERT
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            let response = db
                .execute_with_session_cached(&base_query, &session.id, cached_conn.as_ref())
                .await?;
            ...
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            let last_rowid_response = db
                .query_with_session_cached(last_rowid_query, &session.id, cached_conn.as_ref())
                .await?;

            let last_rowid: i64 = if !last_rowid_response.rows.is_empty()
                && !last_rowid_response.rows[0].is_empty()
            {
            ...
            let returning_query = if response.rows_affected > 1 && last_rowid > 0 {
                ...
                debug!(
                    "Multi-row INSERT RETURNING: fetching rows from rowid {} to {}",
                ...
                debug!(
                    "Single-row INSERT RETURNING: fetching row with rowid {}",
                    last_rowid
                );
                format!(
            ...
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            let returning_response = db
                .query_with_session_cached(&returning_query, &session.id, cached_conn.as_ref())
                .await?;
            ...
            let rowid_query = format!("SELECT rowid FROM {table_name} {where_clause}");
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            let rowid_response = db
                .query_with_session_cached(&rowid_query, &session.id, cached_conn.as_ref())
                .await?;
            let rowids: Vec<String> = rowid_response
                .rows
                .iter()
            ...
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            let captured_rows = db
                .query_with_session_cached(&capture_query, &session.id, cached_conn.as_ref())
                .await?;
            ...
            let columns_without_rowid: Vec<String> = captured_rows
                .columns
                .iter()
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        if EnumDdlHandler::is_enum_ddl(query) {
            ...
            let enum_error = PgSqliteError::Protocol(
                ...
                Please create ENUMs before establishing connections."
                    .to_string(),
            );
        ...
        if query_starts_with_ignore_case(query, "CREATE")
            ...
            && crate::query::executor::try_handle_create_or_replace_sql_function(db, session, query)
                .await?
        {
            framed
                .send(BackendMessage::CommandComplete {
        ...
        if query_starts_with_ignore_case(query, "CREATE TABLE") {
            ...
            let (sqlite_sql, type_mappings, enum_columns, array_columns) = db
                .with_session_connection(&session.id, |conn| {
                    let result =
                        crate::translator::CreateTableTranslator::translate_with_connection_full(
                            query,
                            Some(conn),
                        )
                        .map_err(|e| {
                ...
                .await
                .map_err(|e| {
                    PgSqliteError::Protocol(format!("Failed to translate CREATE TABLE: {e}"))
                })?;

            // Execute the translated CREATE TABLE
            let cached_conn = Self::get_or_cache_connection(session, db).await;
            db.execute_with_session_cached(&sqlite_sql, &session.id, cached_conn.as_ref())
                .await?;
            ...
            if !type_mappings.is_empty() {
                ...
                if let Some(table_name) = extract_table_name_from_create(query) {
                        ...
                        if parts.len() == 2 && parts[0] == table_name {
                            ...
                            if let Some(modifier) = type_mapping.type_modifier {
                                ...
                                if pg_type_lower == "numeric" || pg_type_lower == "decimal" {
                                    ...
                                    let cached_conn =
                                        Self::get_or_cache_connection(session, db).await;
                                    match db
                                        .execute_with_session_cached(
                                            &constraint_query,
                                            &session.id,
                    ...
                    debug!(
                        "Stored type mappings for table {} (extended query protocol)",
                        table_name
                    );
                    ...
                    if !enum_columns.is_empty() {
                        db.with_session_connection(&session.id, |conn| {
                            for (column_name, enum_type) in &enum_columns {
                                // Record enum usage
                                crate::metadata::EnumTriggers::record_enum_usage(
                                    conn,
                                    &table_name,
                                    column_name,
                                    enum_type,
                                ...
                                crate::metadata::EnumTriggers::create_enum_validation_triggers(
                                    conn,
                                    &table_name,
                                    column_name,
                                    enum_type,
                        ...
                        .await
                        .map_err(|e| {
                            PgSqliteError::Protocol(format!("Failed to create ENUM triggers: {e}"))
                        })?;
                    }
            ...
            if let Some(table_name) = extract_table_name_from_create(query) {
                db.with_session_connection(&session.id, |conn| {
                    ...
                    info!(
                        "Extended: About to populate constraints for table: {}",
                        table_name
                    );
                    if let Err(e) =
                        crate::catalog::constraint_populator::populate_constraints_for_table(
                            conn,
                            &table_name,
                        )
                    {
        ...
        let translated_query =
            if query.to_lowercase().contains("json") || query.to_lowercase().contains("jsonb") {
    ...
    async fn analyze_insert_params(
        query: &str,
        db: &Arc<DbHandler>,
    ) -> Result<(Vec<i32>, Vec<i32>), PgSqliteError> {
        // Use QueryContextAnalyzer to extract table and column info
        let (table_name, columns) =
            crate::types::QueryContextAnalyzer::get_insert_column_info(query).ok_or_else(|| {
                PgSqliteError::Protocol("Failed to parse INSERT query".to_string())
            })?;

        info!(
            "Analyzing INSERT for table '{}' with columns: {:?}",
            table_name, columns
        );
        ...
        for column in &columns {
            if let Some(col_info) = table_schema.column_map.get(&column.to_lowercase()) {
                ...
                if param_oid != col_info.pg_oid {
                    info!(
                        "Mapped parameter type for {}.{}: {} (OID {}) -> TEXT (OID 25) for binary protocol compatibility",
                        table_name, column, col_info.pg_type, col_info.pg_oid
                    );
                } else {
                    info!(
                        "Found cached type for {}.{}: {} (OID {})",
                        table_name, column, col_info.pg_type, col_info.pg_oid
                    );
                }
            } else {
                ...
                info!(
                    "Column {}.{} not found in schema, defaulting to text",
                    table_name, column
                );
            }
        }
    ...
    async fn analyze_select_params(
        query: &str,
        db: &Arc<DbHandler>,
        session: &Arc<SessionState>,
    ) -> Result<Vec<i32>, PgSqliteError> {
        ...
        for i in 1..=99 {
                ...
                && let Some(type_match) = captures.get(1)
            {
                ...
                info!(
                    "Found explicit cast for parameter {}: {} (OID {})",
                    i, cast_type, oid
                );
                found_type = true;
            }
            ...
            let table_name = if let Some(name) = extract_table_name_from_select(query) {
                ...
                param_types.push(25);
                info!(
                    "Could not extract table name for parameter {}, defaulting to text",
                    i
                );
                continue;
            };
            ...
            for pattern in &patterns {
                    ...
                    && let Some(column_match) = captures.get(1)
                {
                    ...
                    if let Ok(Some(pg_type)) = db
                        .get_schema_type_with_session(&session.id, &table_name, column)
                        .await
                    {
                        let oid = crate::types::SchemaTypeMapper::pg_type_string_to_oid(&pg_type);
                        param_types.push(oid);
                        info!(
                            "Found type for parameter {} from column {}: {} (OID {})",
                    ...
                    if let Ok(response) = db.query(&schema_query).await {
                        for row in &response.rows {
                            ...
                            {
                                let pg_type = crate::types::SchemaTypeMapper::sqlite_type_to_pg_oid(
                                    &sqlite_type,
                                );
                                param_types.push(pg_type);
                                info!(
                                    "Mapped SQLite type for parameter {} from column {}: {} -> PG OID {}",
            ...
            if !found_type {
                ...
                param_types.push(25);
                info!(
                    "Could not determine type for parameter {}, defaulting to text",
                    i
                );
            }
        }
    ...
    fn analyze_column_casts(query: &str) -> std::collections::HashMap<usize, String> {
            ...
            if let Some(pos) = query_upper.find("FROM") {
                ...
                let before_ok = pos == 0
                    || query_upper
                        ...
                        .unwrap_or(true);
                let after_ok = pos + 4 >= query_upper.len()
                    || query_upper
                        .chars()
                        .nth(pos + 4)
                        .map(|c| c.is_whitespace())
        ...
        for ch in select_list.chars() {
            match ch {
                ...
                ',' if paren_depth == 0 => {
                    ...
                    debug!(
                        "analyze_column_casts: Processing expression {}: '{}'",
                        column_idx, current_expr
                    );
                    if let Some(cast_type) = Self::extract_cast_from_expression(&current_expr) {
                        debug!(
                            "analyze_column_casts: Found cast: column {} -> {}",
                            column_idx, cast_type
                        );
        ...
        if !current_expr.trim().is_empty() {
            debug!(
                "analyze_column_casts: Processing last expression: '{}'",
                current_expr
            );
            if let Some(cast_type) = Self::extract_cast_from_expression(&current_expr) {
                debug!(
                    "analyze_column_casts: Found cast in last expression: column {} -> {}",
                    column_idx, cast_type
                );
                cast_map.insert(column_idx, cast_type);
            }
