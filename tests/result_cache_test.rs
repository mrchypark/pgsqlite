...
async fn test_result_cache_for_identical_queries() {
    let test_server = setup_test_server_with_init(|db| {
        Box::pin(async move {
            // Create a test table
            db.execute("CREATE TABLE cache_test (id INTEGER PRIMARY KEY, name TEXT, value REAL)")
                .await?;

            // Insert test data
            for i in 1..=100 {
                let query = format!(
                    "INSERT INTO cache_test VALUES ({}, 'name_{}', {})",
                    i,
                    i,
                    i as f64 * 1.5
                );
...
async fn test_result_cache_invalidation_on_ddl() {
    let test_server = setup_test_server_with_init(|db| {
        Box::pin(async move {
            ...
            db.execute("CREATE TABLE ddl_test (id INTEGER PRIMARY KEY, data TEXT)")
                .await?;
            db.execute("INSERT INTO ddl_test VALUES (1, 'original')")
                .await?;
            Ok(())
        })
    })
    ...
    client
        .execute(
            "ALTER TABLE ddl_test ADD COLUMN extra INTEGER DEFAULT 0",
            &[],
        )
        .await
        .unwrap();
    ...
    // Insert new data
    client
        .execute("INSERT INTO ddl_test (id, data) VALUES (2, 'new')", &[])
        .await
        .unwrap();
...
async fn test_result_cache_not_used_for_dml() {
    let test_server = setup_test_server_with_init(|db| {
        Box::pin(async move {
            db.execute("CREATE TABLE dml_test (id INTEGER PRIMARY KEY, value INTEGER)")
                .await?;
            Ok(())
        })
    })
    ...
    for i in 1..=5 {
        let value = i * 10;
        let affected = client
            .execute(&format!("INSERT INTO dml_test VALUES ({i}, {value})"), &[])
            .await
            .unwrap();
    ...
    let rows = client
        .query("SELECT COUNT(*) FROM dml_test", &[])
        .await
        .unwrap();
...
async fn test_result_cache_with_different_queries() {
    let test_server = setup_test_server_with_init(|db| {
        Box::pin(async move {
            db.execute("CREATE TABLE diff_test (id INTEGER, category TEXT, amount REAL)")
                .await?;
