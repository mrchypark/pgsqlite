use pgsqlite::query::fast_path::{FastPathOperation, can_use_fast_path_enhanced};
use uuid::Uuid;
    ...
    for query in test_cases {
        ...
        match can_use_fast_path_enhanced(query) {
            Some(fast_query) => {
                ...
                println!(
                    "✅ Fast path detected for INSERT: table={}",
...
async fn test_insert_performance_improvement() {
    ...
    for i in 0..100 {
        ...
        db.execute_with_session(&query, &session_id)
            .await
    ...
    let result = db
        .query_with_session("SELECT COUNT(*) FROM test_table", &session_id)
        .await
...
async fn test_insert_with_decimal_columns_fallback() {
    ...
    db.execute_with_session(
        "CREATE TABLE decimal_table (id INTEGER, price DECIMAL(10,2), name TEXT)",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    for i in 0..100 {
        let query =
            format!("INSERT INTO decimal_table (id, price, name) VALUES ({i}, {i}.99, 'test{i}')");
        db.execute_with_session(&query, &session_id)
            .await
            .expect("Failed to execute INSERT");
    }
    ...
    // Verify data was inserted
    let result = db
        .query_with_session("SELECT COUNT(*) FROM decimal_table", &session_id)
        .await
        .expect("Failed to count rows");
    assert_eq!(result.rows.len(), 1);
...
async fn test_insert_bottleneck_analysis() {
    ...
    db.execute_with_session(
        "CREATE TABLE perf_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    db.execute_with_session(
        "INSERT INTO perf_test (name, value) VALUES ('warmup', 1)",
        &session_id,
    )
    .await
    ...
    println!(
        "Schema cache lookup (1000x): {:?}, avg: {:?}",
        schema_lookup_time,
        schema_lookup_time / 1000
    );
    ...
    db.execute_with_session(
        "INSERT INTO perf_test (name, value) VALUES ('single', 100)",
        &session_id,
    )
    .await
    .expect("Failed to execute INSERT");
    ...
    conn.execute(
        "CREATE TABLE direct_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        [],
    )
    .expect("Failed to create table");
    ...
    for i in 0..100 {
        conn.execute(
            "INSERT INTO direct_test (name, value) VALUES (?1, ?2)",
            rusqlite::params![format!("direct{}", i), i],
        )
        .expect("Failed to execute INSERT");
    }
    let direct_time = start.elapsed();
    println!(
        "100 direct SQLite INSERTs: {:?}, avg: {:?}",
        direct_time,
        direct_time / 100
    );
    ...
    println!(
        "100 statement pool INSERTs: {:?}, avg: {:?}",
        pool_time,
        pool_time / 100
    );
