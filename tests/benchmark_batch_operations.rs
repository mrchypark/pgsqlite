...
async fn benchmark_batch_operations() {
    ...
    for i in 0..OPERATIONS {
        let query = format!(
            "UPDATE batch_ops_test SET value = {} WHERE id = {}",
            i * 20,
            i
        );
    ...
    println!(
        "  Per operation: {:.3}ms",
        single_update_time.as_secs_f64() * 1000.0 / OPERATIONS as f64
    );
    println!(
        "  Operations/sec: {:.0}",
        OPERATIONS as f64 / single_update_time.as_secs_f64()
    );
    ...
    for i in TOTAL_ROWS..(TOTAL_ROWS + OPERATIONS) {
        let query = format!(
            "INSERT INTO batch_ops_test (id, name, value) VALUES ({}, 'temp_{}', {})",
            i,
            i,
            i * 10
        );
    ...
    println!(
        "  Per operation: {:.3}ms",
        single_delete_time.as_secs_f64() * 1000.0 / OPERATIONS as f64
    );
    println!(
        "  Operations/sec: {:.0}",
        OPERATIONS as f64 / single_delete_time.as_secs_f64()
    );
    ...
    for batch_size in batch_sizes {
        ...
        println!(
            "  Per batch: {:.3}ms",
            batch_update_time.as_secs_f64() * 1000.0 / num_batches as f64
        );
        println!(
            "  Per row: {:.3}ms",
            batch_update_time.as_secs_f64() * 1000.0 / OPERATIONS as f64
        );
    ...
    for batch_size in [10, 50, 100] {
        ...
        for i in 0..OPERATIONS {
            ...
            let query = format!(
                "INSERT INTO batch_ops_test (id, name, value) VALUES ({}, 'batch_{}', {})",
                id,
                id,
                id * 10
            );
        ...
        for batch in 0..num_batches {
            ...
            let ids: Vec<String> = (start_id..start_id + batch_size)
                .map(|id| id.to_string())
                .collect();
            let query = format!("DELETE FROM batch_ops_test WHERE id IN ({})", ids.join(","));
            send_query(&mut stream, &query).await;
            read_until_ready(&mut stream).await;
        }
        ...
        println!(
            "  Per batch: {:.3}ms",
            batch_delete_time.as_secs_f64() * 1000.0 / num_batches as f64
        );
        println!(
            "  Per row: {:.3}ms",
            batch_delete_time.as_secs_f64() * 1000.0 / OPERATIONS as f64
        );
        println!("  Speedup vs single: {speedup:.1}x");
    }
    ...
    for batch in 0..(OPERATIONS / 100) {
        ...
        conn.execute(
            &format!(
                "UPDATE batch_ops_test SET value = value + 200 WHERE id IN ({})",
                ids.join(",")
            ),
            [],
        )
    ...
    println!(
        "SQLite {} batch UPDATEs (100 per batch): {:?}",
        OPERATIONS / 100,
        sqlite_batch_update_time
    );

    // Calculate overheads
    println!("\n--- Performance Summary ---");
    println!(
        "Single UPDATE overhead vs SQLite: {:.1}x",
