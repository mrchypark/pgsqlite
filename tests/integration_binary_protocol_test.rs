...
async fn test_basic_binary_protocol() {
    ...
    let rows = client
        .query("SELECT * FROM binary_basic_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_comprehensive_binary_protocol() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            vec![
                (
                    "bool_val",
                    &false as &(dyn tokio_postgres::types::ToSql + Sync),
                ),
                ("int2_val", &-32768i16),
    ...
    let rows = client
        .query("SELECT * FROM binary_comprehensive_test ORDER BY id", &[])
        .await
        .unwrap();
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        println!(
            "  Row {test_id}: bool={bool_val}, int4={int4_val}, float8={float8_val:.6}, text='{text_val}'"
        );
    ...
    let final_count = client
        .query_one("SELECT COUNT(*) FROM binary_comprehensive_test", &[])
        .await
        .unwrap();
...
async fn test_binary_numeric_precision() {
    ...
    let test_cases = [
        ...
        (
            Decimal::from_str("12345678901234.12345678").unwrap(),
    ...
    let rows = client
        .query("SELECT * FROM numeric_precision_test ORDER BY id", &[])
        .await
        .unwrap();
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        if small_decimal.scale() <= 2 {
            assert_eq!(
                small_decimal.to_string(),
                test_cases[i].0.round_dp(2).to_string()
            );
        }
