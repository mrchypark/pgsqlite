...
async fn test_numeric_constraint_enforcement() {
    ...
    client
        .execute("INSERT INTO products VALUES (2, 0.01, 0, 0.000)", &[])
        .await
        .unwrap();

    // Test precision overflow - should fail
    let result = client
        .execute("INSERT INTO products VALUES (3, 99999999.999, 1, 0.5)", &[])
        .await;
    ...
    let result = client
        .execute("INSERT INTO products VALUES (4, 100.999, 1, 0.5)", &[])
        .await;
    assert!(result.is_err());

    // Test quantity with decimals when scale=0 - should fail
    let result = client
        .execute("INSERT INTO products VALUES (5, 100.00, 123.45, 0.5)", &[])
        .await;
    assert!(result.is_err());

    // Test tax_rate > 1 (precision 3, scale 3 means max 0.999) - should fail
    let result = client
        .execute("INSERT INTO products VALUES (6, 100.00, 10, 1.000)", &[])
        .await;
    assert!(result.is_err());
...
async fn test_numeric_constraint_updates() {
    ...
    client
        .execute("INSERT INTO accounts VALUES (1, 1000.00)", &[])
        .await
        .unwrap();

    // Valid update
    client
        .execute("UPDATE accounts SET balance = 2000.50 WHERE id = 1", &[])
        .await
        .unwrap();

    // Invalid update - too many decimal places
    let result = client
        .execute("UPDATE accounts SET balance = 2000.555 WHERE id = 1", &[])
        .await;
    assert!(result.is_err());
...
async fn test_numeric_null_handling() {
    ...
    client
        .execute("INSERT INTO nullable_test VALUES (1, 100.00, NULL)", &[])
        .await
        .unwrap();
...
async fn test_numeric_multi_row_insert() {
    ...
    let count = client
        .query_one("SELECT COUNT(*) FROM batch_test", &[])
        .await
        .unwrap();
...
async fn test_numeric_edge_cases() {
    ...
    client
        .execute("INSERT INTO edge_cases (id, tiny) VALUES (1, 9)", &[])
        .await
        .unwrap();
    let result = client
        .execute("INSERT INTO edge_cases (id, tiny) VALUES (2, 10)", &[])
        .await;
    assert!(result.is_err());

    // Test all-scale constraint (max 0.999)
    client
        .execute("UPDATE edge_cases SET all_scale = 0.999 WHERE id = 1", &[])
        .await
        .unwrap();
    let result = client
        .execute(
            "INSERT INTO edge_cases (id, all_scale) VALUES (3, 1.0)",
            &[],
        )
        .await;
    assert!(result.is_err());
