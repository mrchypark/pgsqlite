...
mod simple_concurrent_test {
    ...
    async fn setup_test_server_and_data()
    -> (tokio::task::JoinHandle<()>, u16, tokio_postgres::Client) {
        // Start test server
        ...
        let server_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                tokio::spawn(async move {
                    if let Err(e) =
                        pgsqlite::handle_test_connection_with_pool(stream, addr, db_handler).await
                    {
        ...
        let (client, connection) =
            tokio_postgres::connect(&format!("host=127.0.0.1 port={} dbname=test", port), NoTls)
                .await
        ...
        for thread_id in 0..thread_count {
            ...
            let handle = task::spawn(async move {
                ...
                for i in 0..iterations_per_thread {
                    ...
                    let rows = client
                        .query("SELECT * FROM benchmark_test WHERE id = $1", &[&id])
                        .await
                        .expect("Query failed");
    ...
    async fn benchmark_scaling_analysis() {
        ...
        for thread_count in thread_counts {
            ...
            println!(
                "{} threads: {:.1} qps, {:.2}x scaling, {:.1}% efficiency",
