...
impl FtsTranslator {
        ...
        if let Some(caps) = FTS_MATCH_REGEX.captures(query) {
            ...
            let (table_name, column_name) = if column_ref.contains('.') {
                ...
                let actual_table = self
                    .resolve_table_alias(query, alias_or_table)
    ...
    fn get_fts_table_name(
        ...
        match stmt.query_row([table_name, column_name], |row| row.get::<_, String>(0)) {
    ...
    fn resolve_table_alias(&self, query: &str, alias: &str) -> Option<String> {
        ...
        let alias_regex =
            regex::RegexBuilder::new(&format!(r"(?i)\bFROM\s+(\w+)\s+{}\b", regex::escape(alias)))
                .build()
                .ok()?;
        alias_regex
            .captures(query)
...
mod tests {
    ...
    fn test_convert_plain_to_fts5() {
        ...
        assert_eq!(
            translator
                .convert_plain_to_fts5("'quick brown fox'")
                .unwrap(),
    ...
    fn test_convert_phrase_to_fts5() {
        ...
        assert_eq!(
            translator
                .convert_phrase_to_fts5("'quick brown fox'")
                .unwrap(),
            "\"quick brown fox\""
        );
    }
    ...
    fn test_resolve_table_alias() {
        ...
        let query2 =
            "SELECT id, title FROM documents WHERE search_vector @@ to_tsquery('english', 'fox')";
