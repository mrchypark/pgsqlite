...
fn test_decimal_ordering_integration() {
    ...
    let mut stmt = conn
        .prepare("SELECT item, amount FROM prices ORDER BY CAST(amount AS REAL)")
        .unwrap();
    ...
    let mut stmt_desc = conn
        .prepare("SELECT item, amount FROM prices ORDER BY CAST(amount AS REAL) DESC")
        .unwrap();
    let results_desc: Vec<(String, String)> = stmt_desc
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))
        .unwrap()
        .collect::<Result<Vec<_>, _>>()
        .unwrap();
