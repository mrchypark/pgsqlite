...
static SIMPLE_DELETE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)^\s*DELETE\s+FROM\s+\w+\s+(WHERE\s+\w+\s*=\s*('[^']*'|\d+))?\s*;?\s*$")
        .unwrap()
});
...
pub fn is_ultra_simple_query(query: &str) -> bool {
    ...
    if query.to_uppercase().starts_with("INSERT") {
           ...
           query.contains("ARRAY[")
        {
            ...
            debug!(
                "INSERT query detected with special patterns - NOT ultra-simple: {}",
                query
            );
            return false;
        }
...
mod tests {
    ...
    fn test_ultra_simple_detection() {
        ...
        assert!(is_ultra_simple_query(
            "UPDATE users SET name = 'test', age = 25"
        ));
        assert!(is_ultra_simple_query(
            "UPDATE users SET price = 99.99, quantity = 5 WHERE id = 1"
        ));
        ...
        assert!(!is_ultra_simple_query(
            "SELECT value FROM unnest('[1,2,3]') AS t"
        ));
        assert!(!is_ultra_simple_query(
            "SELECT value FROM UNNEST('[1,2,3]') AS t"
        ));
    ...
    fn test_table_extraction() {
        assert_eq!(
            extract_simple_table_name("SELECT * FROM users"),
            Some("users".to_string())
        );
        assert_eq!(
            extract_simple_table_name("INSERT INTO products (name) VALUES ('test')"),
            Some("products".to_string())
        );
        assert_eq!(
            extract_simple_table_name("UPDATE customers SET name = 'test'"),
            Some("customers".to_string())
        );
        assert_eq!(
            extract_simple_table_name("DELETE FROM orders"),
    ...
    fn test_array_insert_detection() {
        ...
        assert!(!is_ultra_simple_query(
            "INSERT INTO test_arrays (int_array) VALUES ('{1,2,3}')"
        ));
        assert!(!is_ultra_simple_query(
            "INSERT INTO test_arrays (int_array, text_array) VALUES ('{1,2,3}', '{\"a\",\"b\"}')"
        ));
    ...
    fn test_fast_path_simple_query() {
        ...
        assert!(is_fast_path_simple_query(
        ...
        assert!(!is_fast_path_simple_query(
            "INSERT INTO logs (time) VALUES ('14:30:00')"
        ));
        assert!(!is_fast_path_simple_query(
        ...
        assert!(!is_fast_path_simple_query(
            "INSERT INTO users (name) VALUES ('test') RETURNING id::text"
        ));
        assert!(!is_fast_path_simple_query(
            "INSERT INTO users (name) VALUES ('test') RETURNING upper(name)"
        ));
        assert!(!is_fast_path_simple_query(
            "UPDATE users SET price = 10 WHERE id = 1 RETURNING price * 2"
        ));
        assert!(!is_fast_path_simple_query(
            "DELETE FROM users WHERE id = 1 RETURNING now()"
        ));
    ...
    fn test_returning_optimization() {
        ...
        assert!(is_fast_path_simple_query(
            "UPDATE users SET name = 'test' RETURNING *"
        ));
        assert!(is_fast_path_simple_query(
            "DELETE FROM users WHERE id = 1 RETURNING id"
        ));
        assert!(is_fast_path_simple_query(
            "INSERT INTO test VALUES (1) RETURNING col1, col2, col3"
        ));

        // Complex RETURNING should not be fast path
        assert!(!is_fast_path_simple_query(
            "INSERT INTO users VALUES (1) RETURNING id + 1"
        ));
        assert!(!is_fast_path_simple_query(
            "INSERT INTO users VALUES (1) RETURNING upper(name)"
        ));
        assert!(!is_fast_path_simple_query(
            "INSERT INTO users VALUES (1) RETURNING id::text"
        ));
    }
}
