...
async fn test_multirow_insert_returning() {
    ...
    client
        .execute("CREATE TABLE temp_users (name TEXT, email TEXT)", &[])
        .await
        .unwrap();
    ...
    assert_eq!(
        select_returned_rows.len(),
        2,
        "Should return 2 rows from INSERT SELECT"
    );
...
async fn test_sqlalchemy_style_insert_returning() {
    ...
    assert_eq!(
        returned_rows.len(),
        2,
        "Should return 2 rows from SQLAlchemy-style INSERT"
    );
    assert_eq!(
        returned_rows[0].0, returned_rows[0].1,
        "id and id__1 should match"
    );
    assert_eq!(
        returned_rows[1].0, returned_rows[1].1,
        "id and id__1 should match"
    );
