...
async fn test_single_numeric_constraint() {
    ...
    client
        .execute("INSERT INTO products VALUES (1, 99999999.99)", &[])
        .await
        .unwrap();
    ...
    let result = client
        .execute("INSERT INTO products VALUES (2, 99999999.999)", &[])
        .await;
    ...
    let result2 = client
        .execute("INSERT INTO products VALUES (3, 999999999.99)", &[])
        .await;

    match &result2 {
        Ok(_) => println!(
            "✗ ERROR: Value 999999999.99 was accepted but should have been rejected (too many total digits)"
        ),
        Err(e) => {
    ...
    let rows = client
        .query("SELECT id, price::text FROM products ORDER BY id", &[])
        .await
        .unwrap();
    println!("\nValues in table:");
    for row in rows {
