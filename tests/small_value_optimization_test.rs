...
async fn test_small_value_optimization_performance() {
    ...
    client
        .execute("DROP TABLE IF EXISTS small_value_test", &[])
        .await
        .ok();
    client
        .execute(
    ...
    for _ in 0..5 {
        ...
        let rows: Vec<_> = client
            .query("SELECT * FROM small_value_test WHERE id < 50", &[])
            .await
    ...
    let all_rows: Vec<_> = client
        .query("SELECT * FROM small_value_test", &[])
        .await
...
async fn test_small_value_correctness() {
    ...
    client
        .execute("DROP TABLE IF EXISTS small_value_correct", &[])
        .await
        .ok();
    client
        .execute(
    ...
    let row = client
        .query_one("SELECT * FROM small_value_correct", &[])
        .await
        .expect("Failed to query");
...
async fn test_small_value_binary_protocol() {
    ...
    client
        .execute("DROP TABLE IF EXISTS small_value_binary", &[])
        .await
        .ok();
    client
        .execute(
    ...
    let stmt = client
        .prepare("SELECT * FROM small_value_binary WHERE id = $1")
        .await
