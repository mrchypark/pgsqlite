...
impl SchemaDriftDetector {
    ...
    fn get_tracked_tables(conn: &Connection) -> Result<Vec<String>, rusqlite::Error> {
        let mut stmt =
            conn.prepare("SELECT DISTINCT table_name FROM __pgsqlite_schema ORDER BY table_name")?;
    ...
    fn check_table_drift(
        conn: &Connection,
        table_name: &str,
    ) -> Result<Option<TableDrift>, SchemaDriftError> {
    ...
    fn normalize_sqlite_type(type_str: &str) -> String {
        ...
        for token in base_type.split_whitespace() {
            if matches!(
                token,
                "PRIMARY"
                    | "KEY"
        ...
        let base_type = if type_tokens.is_empty() {
            base_type
        } else {
            type_tokens.join(" ")
        };
