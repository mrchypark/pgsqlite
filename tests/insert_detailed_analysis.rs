...
async fn test_insert_detailed_timing() {
    ...
    db.execute_with_session(
        "CREATE TABLE test_insert (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        &session_id,
    )
    .await
    ...
    // Warm up
    db.execute_with_session(
        "INSERT INTO test_insert (name, value) VALUES ('warmup', 1)",
        &session_id,
    )
    .await
    ...
    let test_cases = vec![
        (
            "Simple INSERT",
            "INSERT INTO test_insert (name, value) VALUES ('test1', 100)",
        ),
        (
            "INSERT with single quotes",
            "INSERT INTO test_insert (name, value) VALUES ('test''s', 200)",
        ),
        (
            "INSERT with numbers",
            "INSERT INTO test_insert (name, value) VALUES ('test123', 300)",
        ),
        (
            "INSERT with longer values",
            "INSERT INTO test_insert (name, value) VALUES ('this is a much longer test string that might affect performance', 400)",
        ),
    ];
    ...
    println!(
        "50 INSERTs without transaction: {:?}, avg: {:?}",
        no_txn_time,
        no_txn_time / 50
    );
    ...
    let start = Instant::now();
    db.begin_with_session(&session_id)
        .await
    ...
    db.commit_with_session(&session_id)
        .await
        .expect("Failed to commit transaction");
    let with_txn_time = start.elapsed();
    println!(
        "50 INSERTs with transaction: {:?}, avg: {:?}",
        with_txn_time,
        with_txn_time / 50
    );
    ...
    let start = Instant::now();
    db.execute_with_session(
        "CREATE TABLE no_decimal (id INTEGER, name TEXT)",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    let start = Instant::now();
    db.execute_with_session(
        "CREATE TABLE with_decimal (id INTEGER, price DECIMAL(10,2))",
        &session_id,
    )
    .await
    .expect("Failed to create table");
    ...
    println!(
        "20 INSERTs without decimal: {:?}, avg: {:?}",
        insert_no_decimal,
        insert_no_decimal / 20
    );
    println!(
        "20 INSERTs with decimal: {:?}, avg: {:?}",
        insert_with_decimal,
        insert_with_decimal / 20
    );
