...
fn test_create_and_retrieve_enum() {
    ...
    // Retrieve by OID
    let enum_type_by_oid = EnumMetadata::get_enum_type_by_oid(&conn, type_oid)
        .unwrap()
        .unwrap();
...
fn test_enum_values() {
    ...
    let active_value = EnumMetadata::get_enum_value(&conn, type_oid, "active")
        .unwrap()
        .unwrap();
    ...
    // Get by OID
    let value_by_oid = EnumMetadata::get_enum_value_by_oid(&conn, active_value.value_oid)
        .unwrap()
        .unwrap();
...
fn test_add_enum_value() {
    ...
    let type_oid =
        EnumMetadata::create_enum_type(&mut conn, "priority", &["low", "high"], None).unwrap();
