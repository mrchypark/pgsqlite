...
static ARRAY_AGG_DISTINCT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)array_agg\s*\(\s*DISTINCT\s+([^)]+)\s*\)").unwrap());

static ARRAY_AGG_ORDER_BY_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)array_agg\s*\(\s*([^)]+?)\s+ORDER\s+BY\s+([^)]+)\s*\)").unwrap());
...
impl ArrayAggTranslator {
    ...
    fn translate_distinct_order_by(sql: &str) -> Result<String, PgSqliteError> {
        ...
        for (original, replacement) in replacements {
            ...
            debug!(
                "Translated DISTINCT ORDER BY array_agg: {} -> {}",
    ...
    fn translate_order_by(sql: &str) -> Result<String, PgSqliteError> {
        ...
        for (original, replacement) in replacements {
            result = result.replace(&original, &replacement);
            debug!(
                "Translated ORDER BY array_agg: {} -> {} (ORDER BY not fully supported)",
                original, replacement
            );
        }
    ...
    fn extract_array_agg_metadata(sql: &str, metadata: &mut TranslationMetadata) {
        ...
        let alias_regex =
            Regex::new(r"(?i)array_agg(?:_distinct)?\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap();
...
mod tests {
    ...
    fn test_array_agg_with_alias() {
        ...
        assert_eq!(
            result,
            "SELECT array_agg_distinct(name) AS unique_names FROM users"
        );
    ...
    fn test_contains_enhanced_array_agg() {
        assert!(ArrayAggTranslator::contains_enhanced_array_agg(
