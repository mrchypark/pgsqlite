...
async fn test_row_to_json_basic_subquery() {
    let (_port, client) = timeout(Duration::from_secs(10), setup_test_db())
        .await
    ...
    assert!(
        json_result.contains("\"name\":\"Alice\"") || json_result.contains("\"name\": \"Alice\"")
    );
...
async fn test_row_to_json_multiple_columns() {
    ...
    assert!(
        json_result.contains("\"id\":1") || json_result.contains("\"id\": 1"),
        "Missing id field in: {json_result}"
    );
    assert!(
        json_result.contains("\"name\":\"Widget\"") || json_result.contains("\"name\": \"Widget\""),
        "Missing name field in: {json_result}"
    );
    assert!(
        json_result.contains("\"price\":19.99") || json_result.contains("\"price\": 19.99"),
        "Missing price field in: {json_result}"
    );
    assert!(
        json_result.contains("\"in_stock\":1")
            || json_result.contains("\"in_stock\": 1")
            || json_result.contains("\"in_stock\":true")
            || json_result.contains("\"in_stock\": true"),
...
async fn test_row_to_json_with_aliases() {
    ...
    assert!(
        json_result.contains("\"fname\":\"John\"") || json_result.contains("\"fname\": \"John\"")
    );
    assert!(
        json_result.contains("\"lname\":\"Doe\"") || json_result.contains("\"lname\": \"Doe\"")
    );
...
async fn test_row_to_json_simple_function_call() {
    ...
    let rows = client
        .query("SELECT row_to_json('test value')", &[])
        .await
        .expect("Failed to execute query");
...
async fn test_row_to_json_multiple_rows() {
    ...
    assert!(
        json_result1.contains("\"category\":\"Fruit\"")
            || json_result1.contains("\"category\": \"Fruit\"")
    );

    assert!(
        json_result2.contains("\"name\":\"Carrot\"")
            || json_result2.contains("\"name\": \"Carrot\"")
    );
    assert!(
        json_result2.contains("\"category\":\"Vegetable\"")
            || json_result2.contains("\"category\": \"Vegetable\"")
    );
