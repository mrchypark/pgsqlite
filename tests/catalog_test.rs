...
async fn test_pg_class_queries() {
    ...
    for row in &response.rows {
        ...
        if let Some(Some(name_bytes)) = row.first() {
            // relname is at index 0 now
            let name = String::from_utf8_lossy(name_bytes);
            if name == "test_table" {
                found_table = true;
                // Check relkind is 'r' for regular table
                if let Some(Some(kind_bytes)) = row.get(1) {
                    // relkind is at index 1 now
...
async fn test_pg_attribute_queries() {
    ...
    for row in &response.rows {
        if let Some(Some(name_bytes)) = row.first() {
            // attname is at index 0 (first selected column)
            ...
            if col_name == "id" {
                ...
                if let Some(Some(notnull_bytes)) = row.get(2) {
                    // attnotnull is at index 2 (third selected column)
    ...
    assert!(
        column_count >= 3,
        "Should have at least 3 columns for test_table"
    );
