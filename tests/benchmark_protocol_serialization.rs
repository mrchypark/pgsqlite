...
async fn profile_type_conversions(client: &tokio_postgres::Client) {
    ...
    let test_cases = vec![
        (
            "INTEGER",
            "SELECT int_col FROM bench_protocol WHERE id < 50",
        ),
        ("TEXT", "SELECT text_col FROM bench_protocol WHERE id < 50"),
        (
            "BOOLEAN",
            "SELECT bool_col FROM bench_protocol WHERE id < 50",
        ),
        (
            "FLOAT",
            "SELECT float_col FROM bench_protocol WHERE id < 50",
        ),
        (
            "NUMERIC",
            "SELECT numeric_col FROM bench_protocol WHERE id < 50",
        ),
    ];
