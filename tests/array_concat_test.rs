...
async fn test_array_concatenation_operators() {
    let server = setup_test_server_with_init(|client| {
        Box::pin(async move {
            ...
            client
                .execute(
                    "INSERT INTO test_arrays (id, tags, numbers, items_list, category_names) VALUES
                (1, '{tag1,tag2}', '{1,2,3}', '{item1,item2}', '{cat1,cat2}'),
                (2, '{tag3,tag4}', '{4,5,6}', '{item3,item4}', '{cat3,cat4}')",
                )
                .await?;
    ...
    let result = client
        .query("SELECT '{a,b}' || '{c,d}' AS concat_result", &[])
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT tags || '{new_tag}' AS extended_tags FROM test_arrays WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT tags || category_names AS all_tags FROM test_arrays WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT '{a,b}' || tags AS combined FROM test_arrays WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT 'id_' || id::TEXT AS prefixed_id FROM test_arrays WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_array_concatenation_with_aliases() {
    let server = setup_test_server_with_init(|client| {
        Box::pin(async move {
            ...
            client
                .execute(
                    "INSERT INTO test_concat (id, tags_array, items) VALUES
                (1, '{tag1,tag2}', '{item1,item2}')",
                )
                .await?;
    ...
    let result = client
        .query(
            "SELECT tags_array || items AS combined_data FROM test_concat",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT '{extra}' || tags_array AS final_tags FROM test_concat",
            &[],
        )
        .await
        .unwrap();
...
async fn test_array_concatenation_function_calls() {
    let server = setup_test_server_with_init(|client| {
        Box::pin(async move {
            ...
            client
                .execute("INSERT INTO test_funcs (id, base_array) VALUES (1, '{a,b}')")
                .await?;
...
async fn test_array_concatenation_naming_patterns() {
    let server = setup_test_server_with_init(|client| {
        Box::pin(async move {
            ...
            client
                .execute(
                    "INSERT INTO naming_test VALUES
                ('{tag1}', '{1,2}', '{cat1}', '{kw1}', '{el1}', '{val1}', '{name1}', 'text')",
                )
                .await?;
    ...
    for query in array_queries {
        let result = client
            .query(&format!("{query} FROM naming_test"), &[])
            .await;
    ...
    let result = client
        .query("SELECT simple_string || ' suffix' FROM naming_test", &[])
        .await
        .unwrap();
...
async fn test_array_syntax_concatenation() {
    let server = setup_test_server_with_init(|client| {
        Box::pin(async move {
            client
                .execute(
                    "CREATE TABLE array_syntax_test (
                id INTEGER PRIMARY KEY,
                int_array INTEGER[],
                text_array TEXT[]
            )",
            ...
            client
                .execute(
                    "INSERT INTO array_syntax_test (id, int_array, text_array) VALUES
                (1, '{1,2,3}', '{a,b,c}'),
                (2, '{4,5,6}', '{d,e,f}')",
                )
                .await?;
    ...
    let result = client
        .query("SELECT '{1,2}' || '{3,4}' AS combined", &[])
        .await
        .unwrap();
    assert_eq!(result.len(), 1);
    ...
    let result = client
        .query(
            "SELECT '{0}' || int_array AS prepended FROM array_syntax_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT text_array || '{z}' AS appended FROM array_syntax_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let result = client
        .query(
            "SELECT '{prefix}' || text_array AS wrapped FROM array_syntax_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
