...
async fn test_fts_create_table_integration() {
    ...
    assert_eq!(
        String::from_utf8_lossy(check_result.rows[0][0].as_ref().unwrap()),
        "articles"
    );
...
async fn test_fts_functions_registration() {
    ...
    let result = db
        .query_with_session("SELECT to_tsvector('english', 'hello world')", &session_id)
        .await
        .unwrap();
    ...
    let result = db
        .query_with_session("SELECT to_tsquery('english', 'hello & world')", &session_id)
        .await
        .unwrap();
    ...
    let result = db
        .query_with_session("SELECT ts_rank('dummy', 'dummy')", &session_id)
        .await
        .unwrap();
...
fn test_fts_translator_unit() {
    ...
    assert!(FtsTranslator::contains_fts_operations(
    ...
    let select_query =
        "SELECT * FROM documents WHERE search_vector @@ to_tsquery('english', 'test')";
...
fn test_type_mapping_integration() {
    use pgsqlite::types::{PgType, TypeMapper};
