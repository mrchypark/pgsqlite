...
async fn test_extract_simple() {
    ...
    let result = client
        .query("SELECT extract('year', 1686840645.0)", &[])
        .await;
...
async fn test_date_trunc_simple() {
    ...
    let result = client
        .query("SELECT date_trunc('hour', 1686840645.0)", &[])
        .await;
...
async fn test_extract_uppercase() {
    ...
    let result = client
        .query("SELECT EXTRACT(YEAR FROM 1686840645.0) as year", &[])
        .await;
...
async fn test_extract_query_one() {
    ...
    let result = client
        .query_one("SELECT EXTRACT(YEAR FROM 1686840645.0) as year", &[])
        .await;
