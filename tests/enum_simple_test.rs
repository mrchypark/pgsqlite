...
async fn test_enum_simple_query() {
    ...
    client
        .simple_query("CREATE TABLE tasks (id INTEGER PRIMARY KEY, name TEXT, status status)")
        .await
        .expect("Failed to create table");

    // Insert some data using simple query
    client
        .simple_query("INSERT INTO tasks (id, name, status) VALUES (1, 'Task 1', 'active')")
        .await
        .expect("Failed to insert row 1");

    // Query the data using simple query
    let results = client
        .simple_query("SELECT id, name, status FROM tasks ORDER BY id")
        .await
...
async fn test_enum_check_constraint() {
    ...
    // Create a table with an ENUM column
    client
        .simple_query("CREATE TABLE items (id INTEGER PRIMARY KEY, priority priority)")
        .await
        .expect("Failed to create table");

    // Test that invalid ENUM values are rejected
    let result = client
        .simple_query("INSERT INTO items (id, priority) VALUES (1, 'invalid')")
        .await;
    ...
    client
        .simple_query("INSERT INTO items (id, priority) VALUES (1, 'high')")
        .await
