...
async fn test_arithmetic_in_subqueries() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE customers (id INTEGER PRIMARY KEY, name TEXT, discount REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO customers VALUES (1, 'Alice', 0.1), (2, 'Bob', 0.15)")
            .await
            .unwrap();
...
async fn test_arithmetic_aliases_in_join_subqueries() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, base_price REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO products VALUES (1, 'Item A', 100.0), (2, 'Item B', 200.0)")
            .await
            .unwrap();
        ...
        db_handler
            .execute(
                "INSERT INTO regions VALUES (1, 'North', 0.08, 10.0), (2, 'South', 0.06, 15.0)",
            )
            .await
            .unwrap();
...
async fn test_nested_subquery_arithmetic() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE metrics (id INTEGER PRIMARY KEY, category TEXT, value REAL)")
            .await
            .unwrap();
        db_handler
            .execute(
