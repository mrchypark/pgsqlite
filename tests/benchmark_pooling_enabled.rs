...
async fn test_pooled_concurrent_reads() {
    ...
    unsafe {
        env::set_var("PGSQLITE_USE_POOLING", "true");
    }
            ...
            loop {
                match listener.accept().await {
                    Ok((stream, addr)) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) =
                                pgsqlite::handle_test_connection_with_pool(stream, addr, db_clone)
                                    .await
                            {
    ...
    for client_id in 0..num_clients {
        let handle = tokio::spawn(async move {
            ...
            for i in 0..queries_per_client {
                ...
                let rows = client
                    .query(
                        &format!("SELECT value, description FROM pooling_test WHERE id = {id}"),
                        &[],
                    )
                    .await
                    .unwrap();
            ...
            println!(
                "  Client {} completed {} queries in {:.3}s ({:.0} queries/sec)",
                client_id,
                queries_per_client,
                elapsed.as_secs_f64(),
    ...
    println!(
        "📊 Total: {} queries in {:.3}s ({:.0} queries/sec)",
        total_queries,
...
async fn test_pooled_mixed_workload() {
        ...
        loop {
            match listener.accept().await {
                Ok((stream, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) =
                            pgsqlite::handle_test_connection_with_pool(stream, addr, db_clone).await
                        {
                            eprintln!("Connection error: {e}");
                        }
                    });
    ...
    for reader_id in 0..num_readers {
        let handle = tokio::spawn(async move {
            ...
            for i in 0..operations_per_client {
                ...
                let _rows = client
                    .query(
                        &format!("SELECT value FROM mixed_test WHERE id = {id}"),
                        &[],
                    )
                    .await
                    .unwrap();
    ...
    for writer_id in 0..num_writers {
        let handle = tokio::spawn(async move {
            ...
            let elapsed = start.elapsed();
            println!(
                "  Writer {} completed {} updates in {:.3}s",
                writer_id,
                operations_per_client,
                elapsed.as_secs_f64()
            );
    ...
    println!(
        "📊 Total: {} operations in {:.3}s ({:.0} ops/sec)",
        total_operations,
        total_elapsed.as_secs_f64(),
        ops_per_second
    );
...
async fn test_pooling_effectiveness() {
    ...
    unsafe {
        env::remove_var("PGSQLITE_USE_POOLING");
    }
    ...
    for i in 0..queries_to_run {
        ...
        let _rows = client
            .query(
                &format!("SELECT data FROM effectiveness_test WHERE id = {id}"),
                &[],
            )
            .await
            .unwrap();
    ...
    println!(
        "  Without pooling: {} queries in {:.3}s ({:.0} queries/sec)",
        queries_to_run,
        without_pooling.as_secs_f64(),
        without_pooling_qps
    );
    ...
    println!(
        "  With pooling: {} queries in {:.3}s ({:.0} queries/sec)",
        queries_to_run,
        with_pooling.as_secs_f64(),
        with_pooling_qps
    );
