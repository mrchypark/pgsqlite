    ...
    pub fn analyze_query(query: &str) -> TranslationMetadata {
        ...
        if let Some(select_caps) = SELECT_CLAUSE_PATTERN.captures(query) {
            ...
            for caps in ARITHMETIC_EXPR_PATTERN.captures_iter(select_list) {
                ...
                debug!(
                    "Found arithmetic expression '{}' aliased as '{}'",
                    expression, alias
                );
                ...
                if !columns.is_empty() {
                    ...
                    info!(
                        "Detected arithmetic on columns {:?} aliased as '{}', using '{}' as source",
                        columns, alias, source_column
                    );
            ...
            for caps in implicit_alias_pattern.captures_iter(select_list) {
                ...
                debug!(
                    "Found arithmetic expression '{}' with implicit alias '{}'",
                    expression, alias
                );
                ...
                if !columns.is_empty() {
                    ...
                    let source_column = columns[0].clone();
                    info!(
                        "Detected arithmetic on columns {:?} implicitly aliased as '{}', using '{}' as source",
                        columns, alias, source_column
                    );
...
mod tests {
    ...
    fn test_needs_analysis() {
        assert!(ArithmeticAnalyzer::needs_analysis(
            "SELECT price * 1.1 AS total FROM items"
        ));
        assert!(ArithmeticAnalyzer::needs_analysis(
    ...
    fn test_analyze_arithmetic_with_explicit_alias() {
        ...
        let hint = metadata
            .get_hint("price_with_tax")
            .expect("Should have hint for price_with_tax");
        assert_eq!(hint.source_column.as_ref().unwrap(), "price");
        assert!(hint.is_expression);
        assert_eq!(
            hint.expression_type,
            Some(crate::translator::ExpressionType::ArithmeticOnFloat)
        );
    }
    ...
    fn test_analyze_arithmetic_with_implicit_alias() {
        ...
        let hint = metadata
            .get_hint("total")
            .expect("Should have hint for total");
        assert_eq!(hint.source_column.as_ref().unwrap(), "price");
    }
    ...
    fn test_analyze_complex_expressions() {
        ...
        let hint = metadata
            .get_hint("total_cost")
            .expect("Should have hint for total_cost");
