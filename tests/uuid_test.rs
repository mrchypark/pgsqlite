...
async fn test_uuid_support() {
    ...
    let result = client
        .simple_query("SELECT gen_random_uuid() as uuid")
        .await
        .unwrap();
    let uuid1 = result
        .iter()
    ...
    let result = client
        .simple_query("SELECT uuid_generate_v4() as uuid")
        .await
        .unwrap();
    let uuid3 = result
        .iter()
    ...
    let result = client
        .simple_query(&format!("SELECT is_valid_uuid('{uuid1}') as valid"))
        .await
        .unwrap();
    let valid = result
        .iter()
    ...
    let result = client
        .simple_query("SELECT is_valid_uuid('not-a-uuid') as valid")
        .await
        .unwrap();
    let invalid = result
        .iter()
    ...
    let result = client
        .simple_query(&format!(
            "SELECT uuid_normalize('{upper_uuid}') as normalized"
        ))
        .await
        .unwrap();
    ...
    let result = client
        .simple_query(&format!("SELECT id, name FROM users WHERE id = '{uuid1}'"))
        .await
        .unwrap();
    result
        .iter()
    ...
    client
        .simple_query("INSERT INTO users (id, name) VALUES (gen_random_uuid(), 'Bob')")
        .await
        .unwrap();

    let result = client
        .simple_query("SELECT COUNT(*) as count FROM users")
        .await
        .unwrap();
    let count = result
        .iter()
        .find_map(|msg| match msg {
            tokio_postgres::SimpleQueryMessage::Row(row) => {
                Some(row.get(0).unwrap().parse::<i64>().unwrap())
            }
            _ => None,
        })
        .expect("Expected to find a row");
    ...
    client
        .simple_query("CREATE TABLE uuid_test (id TEXT COLLATE uuid)")
        .await
        .unwrap();
    ...
    let result = client
        .simple_query("SELECT COUNT(DISTINCT id) as count FROM uuid_test")
        .await
        .unwrap();
    let count = result
        .iter()
