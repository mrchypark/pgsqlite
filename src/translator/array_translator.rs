...
use crate::translator::{ColumnTypeHint, ExpressionType, TranslationMetadata};
...
static ARRAY_CONTAINS_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(\b\w+(?:\.\w+)*)\s*@>\s*('[^']+'|"[^"]+"|'\[[^\]]+\]')"#).unwrap());
...
static ARRAY_OVERLAP_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(\b\w+(?:\.\w+)*)\s*&&\s*(\b\w+(?:\.\w+)*|'[^']+'|"[^"]+"|'\[[^\]]+\]')"#)
        .unwrap()
});

static ARRAY_LITERAL_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"ARRAY\[([^\]]*)\]").unwrap());

static ARRAY_SUBSCRIPT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)*)\[(\d+)\]").unwrap());

static ARRAY_SLICE_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)*)\[(\d+):(\d+)\]").unwrap());

static ANY_OPERATOR_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"('[^']+'|"[^"]+"|[^\s=]+)\s*=\s*ANY\s*\(('[^']+'|"[^"]+"|[\w\.]+)\)"#).unwrap()
});

static ALL_OPERATOR_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)*|\d+)\s*([><=!]+)\s*ALL\s*\(").unwrap());
...
static ARRAY_FUNCTION_ALIAS_REGEXES: Lazy<Vec<(&'static str, Regex)>> = Lazy::new(|| {
    vec![
        ...
        (
            "array_agg",
            Regex::new(r"(?i)array_agg\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_append",
            Regex::new(r"(?i)array_append\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_prepend",
            Regex::new(r"(?i)array_prepend\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_cat",
            Regex::new(r"(?i)array_cat\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_remove",
            Regex::new(r"(?i)array_remove\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_replace",
            Regex::new(r"(?i)array_replace\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_slice",
            Regex::new(r"(?i)array_slice\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "string_to_array",
            Regex::new(r"(?i)string_to_array\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_positions",
            Regex::new(r"(?i)array_positions\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        // Array functions that return integers
        (
            "array_length",
            Regex::new(r"(?i)array_length\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_upper",
            Regex::new(r"(?i)array_upper\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_lower",
            Regex::new(r"(?i)array_lower\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_ndims",
            Regex::new(r"(?i)array_ndims\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_position",
            Regex::new(r"(?i)array_position\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "json_array_length",
            Regex::new(r"(?i)json_array_length\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        // Array functions that return booleans
        (
            "array_contains",
            Regex::new(r"(?i)array_contains\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_contained",
            Regex::new(r"(?i)array_contained\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "array_overlap",
            Regex::new(r"(?i)array_overlap\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        // Array functions that return text
        (
            "array_to_string",
            Regex::new(r"(?i)array_to_string\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
        (
            "unnest",
            Regex::new(r"(?i)unnest\s*\([^)]+\)\s+(?:AS\s+)?(\w+)").unwrap(),
        ),
    ]
});
...
impl ArrayTranslator {
    ...
    fn contains_array_functions(sql: &str) -> bool {
        ...
        if sql.contains("array_agg(")
            ...
            || sql.contains("unnest(")
            || sql.contains("UNNEST(")
            || sql.contains("array_length(")
            || sql.contains("ARRAY_LENGTH(")
            || sql.contains("array_to_string(")
            || sql.contains("ARRAY_TO_STRING(")
        {
            return true;
        }
    ...
    pub fn translate_with_metadata(
        sql: &str,
    ) -> Result<(String, TranslationMetadata), PgSqliteError> {
        // Early exit: if no array functions/operators detected, return unchanged
        if !Self::contains_array_functions(sql) {
    ...
    fn translate_any_operator(sql: &str) -> Result<String, PgSqliteError> {
        ...
        while let Some(captures) = any_array_regex.captures(&result) {
            ...
            debug!(
                "ArrayTranslator: Converting ANY(ARRAY[...]) to IN: {} -> {}",
        ...
        while let Some(captures) = ANY_OPERATOR_REGEX.captures(&result) {
            ...
            let replacement =
                format!("EXISTS (SELECT 1 FROM json_each({array_col}) WHERE value = {value})");
            result = result.replace(&captures[0], &replacement);
        }
    ...
    fn translate_all_operator(sql: &str) -> Result<String, PgSqliteError> {
        ...
        while let Some(captures) = ALL_OPERATOR_REGEX.captures(&result) {
            ...
            let replacement = if subquery_or_array.contains("SELECT") {
                ...
                if let Some(from_pos) = subquery_or_array.to_ascii_uppercase().find(" FROM") {
    ...
    fn is_likely_array_column(s: &str) -> bool {
        ...
        if s_lower.ends_with("s")
            || s_lower.ends_with("_list")
            || s_lower.ends_with("_array")
            || s_lower.ends_with("_data")
        {
            return true;
        }
    ...
    fn translate_concat_operator_with_metadata(
        sql: &str,
    ) -> Result<(String, TranslationMetadata), PgSqliteError> {
        let mut metadata = TranslationMetadata::new();
...
mod tests {
    ...
    fn test_any_operator() {
        ...
        assert!(
            result.contains("EXISTS (SELECT 1 FROM json_each(tags) WHERE value = 'electronics')")
        );
    }
    ...
    fn test_all_operator_debug() {
        ...
        if let Some(captures) = captures {
            ...
            let start_pos = captures.get(0).unwrap().end();
            let subquery =
                ArrayTranslator::extract_balanced_parentheses(sql, start_pos - 1).unwrap();
    ...
    fn test_type_aware_resolution() {
        ...
        println!(
            "is_likely_array for tags || categories: {}",
            ArrayTranslator::is_likely_array_concatenation("tags", "categories")
        );
        ...
        println!(
            "is_likely_array for name || description: {}",
            ArrayTranslator::is_likely_array_concatenation("name", "description")
        );
    ...
    fn test_is_likely_array_concatenation() {
        ...
        assert!(ArrayTranslator::is_likely_array_concatenation(
            "ARRAY[1,2]",
            "ARRAY[3,4]"
        ));
        assert!(ArrayTranslator::is_likely_array_concatenation(
    ...
    fn test_array_literal_translation() {
        ...
        assert_eq!(
            result,
            "SELECT id FROM products WHERE tags = '[\"electronics\",\"computers\"]'"
        );
        ...
        assert_eq!(
            result,
            "INSERT INTO products (tags) VALUES ('[\"new\",\"product\"]')"
        );
    }
