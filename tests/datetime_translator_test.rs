...
async fn test_datetime_trigger_creation() {
    ...
    assert!(
        trigger_check.is_empty(),
        "Datetime triggers should not be created anymore"
    );
...
async fn test_insert_translator_conversion() {
    ...
    client
        .simple_query("INSERT INTO translator_test VALUES (1, '2024-01-15')")
        .await
        .unwrap();

    // Check storage type
    let type_check = client
        .simple_query("SELECT typeof(date_col) FROM translator_test WHERE id = 1")
        .await
        .unwrap();

    if let Some(msg) = type_check
        .into_iter()
        ...
        && let tokio_postgres::SimpleQueryMessage::Row(data) = msg
    {
        ...
        assert_eq!(
            col_type, "integer",
            "InsertTranslator should convert to INTEGER"
        );
    }

    // Test retrieval conversion
    let value_check = client
        .simple_query("SELECT date_col FROM translator_test WHERE id = 1")
        .await
        .unwrap();

    if let Some(msg) = value_check
        .into_iter()
        .find(|m| matches!(m, tokio_postgres::SimpleQueryMessage::Row(_)))
        && let tokio_postgres::SimpleQueryMessage::Row(data) = msg
    {
        ...
        assert_eq!(
            date_str, "2024-01-15",
            "Value converter should convert back to date string"
        );
    }
