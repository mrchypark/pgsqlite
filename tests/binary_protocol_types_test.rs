    ...
    fn from_sql(
        ...
        if !raw.is_empty() {
            ...
            let lower_str = if lower_bound_infinite {
                ...
                if pos + 4 > raw.len() {
                    return Err(
                        "Invalid range format: insufficient data for lower bound length".into(),
                    );
                }
                let lower_len =
            ...
            let upper_str = if upper_bound_infinite {
                ...
                if pos + 4 > raw.len() {
                    return Err(
                        "Invalid range format: insufficient data for upper bound length".into(),
                    );
                }
                let upper_len =
                    u32::from_be_bytes([raw[pos], raw[pos + 1], raw[pos + 2], raw[pos + 3]])
...
async fn test_money_binary_protocol() {
    ...
    let stmt = client
        .prepare("SELECT id, CAST(amount AS TEXT) FROM money_test ORDER BY id")
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT amount FROM money_test WHERE id = $1", &[&999i32])
        .await
        .unwrap();
...
async fn test_int4range_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        ...
        assert_eq!(
            &retrieved, expected,
            "Failed for INT4RANGE value: {expected}"
        );
    }

    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, CAST(range_val AS TEXT) FROM int4range_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
...
async fn test_int8range_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        ...
        assert_eq!(
            &retrieved, expected,
            "Failed for INT8RANGE value: {expected}"
        );
    }

    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, CAST(range_val AS TEXT) FROM int8range_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
...
async fn test_numrange_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        ...
        assert_eq!(
            &retrieved.0, expected,
            "Failed for NUMRANGE value: {expected}"
        );
    }

    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, range_val FROM numrange_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
...
async fn test_cidr_binary_protocol() {
    ...
    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, CAST(cidr_val AS TEXT) FROM cidr_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
    ...
    let row = client
        .query_one("SELECT cidr_val FROM cidr_test WHERE id = $1", &[&999i32])
        .await
        .unwrap();
...
async fn test_inet_binary_protocol() {
    ...
    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, CAST(inet_val AS TEXT) FROM inet_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
    ...
    let row = client
        .query_one("SELECT inet_val FROM inet_test WHERE id = $1", &[&999i32])
        .await
        .unwrap();
...
async fn test_macaddr_binary_protocol() {
    ...
    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, CAST(mac_val AS TEXT) FROM macaddr_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
    ...
    let row = client
        .query_one("SELECT mac_val FROM macaddr_test WHERE id = $1", &[&999i32])
        .await
        .unwrap();
...
async fn test_macaddr8_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        ...
        assert_eq!(
            &retrieved.0, expected,
            "Failed for MACADDR8 value: {expected}"
        );
    }

    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, mac_val FROM macaddr8_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
...
async fn test_bit_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        let _row = client
            .query_one("SELECT bit_val FROM bit_test WHERE id = $1", &[&(i as i32)])
            .await
            .unwrap();
    ...
    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, bit_val::text FROM bit_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
    ...
    let row = client
        .query_one("SELECT bit_val FROM bit_test WHERE id = $1", &[&999i32])
        .await
        .unwrap();
...
async fn test_bit_varying_binary_protocol() {
    ...
    for (i, (_value, expected)) in test_values.iter().enumerate() {
        ...
        assert_eq!(
            &retrieved, expected,
            "Failed for BIT VARYING value: {expected}"
        );
    }

    // Test with prepared statement for binary encoding
    let stmt = client
        .prepare("SELECT id, bit_val::text FROM bit_varying_test ORDER BY id")
        .await
        .unwrap();
    let rows = client.query(&stmt, &[]).await.unwrap();
