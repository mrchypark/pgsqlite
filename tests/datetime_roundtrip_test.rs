...
async fn test_datetime_roundtrip() {
        ...
        .find(|m| matches!(m, tokio_postgres::SimpleQueryMessage::Row(_)))
    {
        if let tokio_postgres::SimpleQueryMessage::Row(data) = msg {
            ...
            println!(
                "  timestamp: {}",
                data.get("typeof(timestamp_col)").unwrap()
            );
        }
    }
...
async fn test_simple_query_datetime_conversion() {
    ...
    // Create table
    client
        .execute("CREATE TABLE simple_test (id INT, date_col DATE)", &[])
        .await
        .unwrap();

    // Insert with simple_query
    client
        .simple_query("INSERT INTO simple_test VALUES (1, '2024-01-15')")
        .await
        .unwrap();

    // Retrieve with simple_query - should get converted value
    let result = client
        .simple_query("SELECT date_col FROM simple_test WHERE id = 1")
        .await
        .unwrap();
        ...
        .find(|m| matches!(m, tokio_postgres::SimpleQueryMessage::Row(_)))
    {
        if let tokio_postgres::SimpleQueryMessage::Row(data) = msg {
            ...
            assert_eq!(
                date_str, "2024-01-15",
                "Date should be converted from INTEGER to string"
            );
        }
    }
