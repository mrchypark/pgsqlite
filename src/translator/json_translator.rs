...
impl JsonTranslator {
    ...
    fn translate_text_extract_operator(sql: &str) -> Result<String, PgSqliteError> {
        ...
        static RE_INT: Lazy<Regex> = Lazy::new(|| {
            Regex::new(
                r"(\b\w+(?:\.\w+)?|pgsqlite_json_get_(?:json|array_json)\([^)]+\))\s*->>\s*(\d+)",
            )
            .expect("Invalid regex")
        });
        ...
        // Handle string keys
        result = RE
            .replace_all(&result, r"pgsqlite_json_get_text($1, '$2')")
            .to_string();

        // Handle integer indices
        result = RE_INT
            .replace_all(&result, r"pgsqlite_json_get_array_text($1, $2)")
            .to_string();
    ...
    fn translate_json_extract_operator(sql: &str) -> Result<String, PgSqliteError> {
        ...
        // Handle string keys
        result = RE
            .replace_all(&result, r"pgsqlite_json_get_json($1, '$2')")
            .to_string();

        // Handle integer indices
        result = RE_INT
            .replace_all(&result, r"pgsqlite_json_get_array_json($1, $2)")
            .to_string();
    ...
    fn translate_path_text_operator(sql: &str) -> Result<String, PgSqliteError> {
        static RE: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"(\b\w+(?:\.\w+)?)\s*#>>\s*'\{([^}]+)\}'").expect("Invalid regex")
        });
    ...
    fn translate_contains_operators(sql: &str) -> Result<String, PgSqliteError> {
        static RE_CONTAINS: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)?)\s*@>\s*'([^']+)'").expect("Invalid regex"));

        static RE_CONTAINED: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(\b\w+(?:\.\w+)?)\s*<@\s*'([^']+)'").expect("Invalid regex"));

        // Also handle reversed format: 'json' <@ column
        static RE_CONTAINED_REV: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"'([^']+)'\s*<@\s*(\b\w+(?:\.\w+)?)").expect("Invalid regex"));

        let mut result = sql.to_string();

        // Translate @> (contains)
        result = RE_CONTAINS
            .replace_all(&result, r"jsonb_contains($1, '$2')")
            .to_string();

        // Translate <@ (is contained by) - normal format
        result = RE_CONTAINED
            .replace_all(&result, r"jsonb_contained($1, '$2')")
            .to_string();

        // Translate <@ (is contained by) - reversed format
        result = RE_CONTAINED_REV
            .replace_all(&result, r"jsonb_contains($2, '$1')")
            .to_string();
    ...
    fn translate_existence_operators(sql: &str) -> Result<String, PgSqliteError> {
        ...
        result = RE_HAS_KEY
            .replace_all(&result, r"pgsqlite_json_has_key($1, '$2')")
            .to_string();

        // Translate ?| operator (has any key)
        result = RE_HAS_ANY_KEY
            .replace_all(&result, r"pgsqlite_json_has_any_key($1, '$2')")
            .to_string();

        // Translate ?& operator (has all keys)
        result = RE_HAS_ALL_KEYS
            .replace_all(&result, r"pgsqlite_json_has_all_keys($1, '$2')")
            .to_string();
...
mod tests {
    ...
    fn test_json_operation_detection() {
        assert!(JsonTranslator::contains_json_operations(
            "SELECT data->>'name' FROM users"
        ));
        assert!(JsonTranslator::contains_json_operations(
            "SELECT * WHERE config @> '{\"active\": true}'"
        ));
        assert!(JsonTranslator::contains_json_operations(
    ...
    fn test_text_extract_operator() {
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_get_text(data, 'name') FROM users"
        );
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_get_array_text(items, 0) FROM orders"
        );
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_get_text(u.data, 'email') FROM users u"
        );
    }
    ...
    fn test_json_extract_operator() {
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_get_json(data, 'address') FROM users"
        );
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_get_array_json(tags, 1) FROM posts"
        );
    }
    ...
    fn test_path_operators() {
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_path_text(data, 'address,city') FROM users"
        );
        ...
        assert_eq!(
            translated,
            "SELECT pgsqlite_json_path_json(data, 'items,0') FROM orders"
        );
    }
    ...
    fn test_contains_operators() {
        ...
        assert_eq!(
            translated,
            "SELECT * FROM users WHERE jsonb_contains(data, '{\"active\": true}')"
        );
        ...
        assert_eq!(
            translated,
            "SELECT * FROM items WHERE jsonb_contained(metadata, '{\"type\": \"product\", \"status\": \"active\"}')"
        );
        ...
        assert_eq!(
            translated,
            "SELECT id FROM users WHERE jsonb_contains(data, '{\"name\": \"Bob\"}')"
        );
    }
    ...
    fn test_existence_operators() {
        ...
        assert_eq!(
            translated,
            "SELECT * FROM users WHERE pgsqlite_json_has_key(data, 'name')"
        );
        ...
        assert_eq!(
            translated,
            "SELECT * FROM users WHERE pgsqlite_json_has_any_key(config, 'admin,user')"
        );
        ...
        assert_eq!(
            translated,
            "SELECT * FROM items WHERE pgsqlite_json_has_all_keys(metadata, 'name,price,category')"
        );
        ...
        assert_eq!(
            translated,
            "SELECT u.id FROM users u WHERE pgsqlite_json_has_key(u.profile, 'email')"
        );
    }
}
