...
async fn test_json_operators() {
    ...
    let rows = client
        .simple_query("SELECT data->>'name' AS name FROM test_json_ops WHERE id = 1")
        .await
        .unwrap();
    let name = rows
        .iter()
    ...
    let rows = client
        .simple_query("SELECT data->'items' AS items FROM test_json_ops WHERE id = 2")
        .await
        .unwrap();
    let items = rows
        .iter()
    ...
    let rows = client
        .simple_query(
            "SELECT data->'nested'->'level1'->>'level2' AS value FROM test_json_ops WHERE id = 3",
        )
        .await
        .unwrap();
    let value = rows
    ...
    let rows = client
        .simple_query("SELECT id FROM test_json_ops WHERE data @> '{\"name\": \"Alice\"}'")
        .await
        .unwrap();
    let count = rows
        .iter()
    ...
    let rows = client
        .simple_query("SELECT id FROM test_json_ops WHERE '{\"name\": \"Bob\"}' <@ data")
        .await
        .unwrap();
    let count = rows
        .iter()
...
async fn test_json_functions() {
    ...
    let rows = client
        .simple_query("SELECT json_array_length('[1, 2, 3, 4, 5]') AS len")
        .await
        .unwrap();
    let len = rows
        .iter()
    ...
    // Test json_typeof
    let rows = client
        .simple_query("SELECT json_typeof('{\"key\": \"value\"}') AS type")
        .await
        .unwrap();
    let json_type = rows
        .iter()
    ...
    let rows = client
        .simple_query("SELECT jsonb_object_keys('{\"a\": 1, \"b\": 2, \"c\": 3}') AS keys")
        .await
        .unwrap();
    let keys = rows
        .iter()
    ...
    let rows = client
        .simple_query("SELECT json_strip_nulls('{\"a\": 1, \"b\": null, \"c\": 3}') AS stripped")
        .await
        .unwrap();
    let stripped = rows
        .iter()
    ...
    // Test jsonb_set
    let rows = client
        .simple_query("SELECT jsonb_set('{\"a\": 1, \"b\": 2}', '{b}', '99') AS updated")
        .await
        .unwrap();
    let updated = rows
        .iter()
    ...
    let rows = client
        .simple_query("SELECT json_extract_path('{\"a\": {\"b\": {\"c\": 42}}}', 'a.b.c') AS value")
        .await
        .unwrap();
    let value = rows
        .iter()
    ...
    let rows = client
        .simple_query(
            "SELECT json_extract_path_text('{\"name\": \"John\", \"age\": 30}', 'name') AS name",
        )
        .await
        .unwrap();
    let name = rows
