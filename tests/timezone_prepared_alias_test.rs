...
async fn test_prepared_at_time_zone_with_alias() {
    ...
    assert!(
        (utc_time - timestamp).abs() < 1.0,
        "UTC time should match original timestamp, got {utc_time} expected {timestamp}"
    );

    // Test 2: Basic query to ensure the test framework works
    println!("\nTest 2: Basic query test");
    let rows = client
        .query("SELECT event_time FROM events WHERE id = $1", &[&1i32])
        .await
        .unwrap();
    ...
    assert!(
        (retrieved_time - timestamp).abs() < 1.0,
        "Retrieved time should match original timestamp, got {retrieved_time} expected {timestamp}"
    );
}
