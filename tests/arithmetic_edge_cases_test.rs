...
async fn test_division_by_zero() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        let db_handler = std::sync::Arc::new(pgsqlite::session::DbHandler::new(&db_path).unwrap());

        // Create test table
        db_handler
            .execute(
                "CREATE TABLE test_div (id INTEGER PRIMARY KEY, numerator REAL, denominator REAL)",
            )
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO test_div VALUES (1, 10.0, 0.0)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO test_div VALUES (2, 10.0, 2.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT numerator / denominator AS result FROM test_div WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT numerator / denominator AS result FROM test_div WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
...
async fn test_very_large_numbers() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE large_nums (id INTEGER PRIMARY KEY, big_val REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO large_nums VALUES (1, 1e30)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO large_nums VALUES (2, 1e-30)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT big_val * 1000 AS huge FROM large_nums WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT big_val / 1000 AS tiny FROM large_nums WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
...
async fn test_negative_number_arithmetic() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE negatives (id INTEGER PRIMARY KEY, val REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO negatives VALUES (1, -42.5)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO negatives VALUES (2, -10.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT val + 100 AS result FROM negatives WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT val * -2 AS result FROM negatives WHERE id = 2", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
    let result: f64 = rows[0].get(0);
...
async fn test_case_sensitivity_in_aliases() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE items (id INTEGER PRIMARY KEY, price REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO items VALUES (1, 50.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT price * 1.1 AS TotalPrice FROM items WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT price * 1.2 as finalprice FROM items WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT price * 1.3 As MixedCase FROM items WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_arithmetic_with_cast() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("INSERT INTO mixed_types VALUES (1, 42, '3.14', 2.5)")
            .await
            .unwrap();
    ...
    let simple_test = client
        .query(
            "SELECT int_val * real_val AS simple_mult FROM mixed_types WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT CAST(text_val AS REAL) + 10 AS text_math FROM mixed_types WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_multiple_arithmetic_operators() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE calc (id INTEGER PRIMARY KEY, a REAL, b REAL, c REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO calc VALUES (1, 10.0, 5.0, 2.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query("SELECT a + b AS addition FROM calc WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT a - b AS subtraction FROM calc WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT a * b AS multiplication FROM calc WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT a / b AS division FROM calc WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT CAST(a AS INTEGER) % CAST(c AS INTEGER) AS modulo FROM calc WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
...
async fn test_floating_point_precision() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE precision_test (id INTEGER PRIMARY KEY, val DOUBLE PRECISION)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO precision_test VALUES (1, 0.1)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO precision_test VALUES (2, 0.2)")
            .await
            .unwrap();
