...
async fn test_datetime_query_debug() {
    ...
    match client
        .query("SELECT EXTRACT(YEAR FROM 1686840645.0)", &[])
        .await
    {
    ...
    match client
        .query(
            "SELECT extract('year', ts) FROM test_events WHERE id = 1",
            &[],
        )
        .await
    {
    ...
    match client
        .query(
            "SELECT EXTRACT(YEAR FROM ts) FROM test_events WHERE id = 1",
            &[],
        )
        .await
    {
        ...
        Err(e) => {
            ...
            match client
                .query("SELECT ts FROM test_events WHERE id = 1", &[])
                .await
            {
                Ok(rows) => {
