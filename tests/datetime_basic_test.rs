...
async fn test_now_function() {
    ...
    assert!(
        now_str.contains('-'),
        "NOW() should return formatted timestamp with dashes, got: '{now_str}'"
    );
    assert!(
        now_str.contains(':'),
        "NOW() should return formatted timestamp with colons, got: '{now_str}'"
    );
    assert!(
        now_str.contains('.'),
        "NOW() should return formatted timestamp with microseconds, got: '{now_str}'"
    );
    assert!(
        now_str.len() > 20,
        "NOW() should return full timestamp string, got: '{now_str}'"
    );

    // Verify it's NOT just raw microseconds
    assert!(
        now_str.parse::<i64>().is_err(),
        "NOW() should not return raw integer microseconds"
    );

    // Also test that CURRENT_TIMESTAMP works the same way
    let row2 = client
        .query_one("SELECT CURRENT_TIMESTAMP as ts", &[])
        .await
        .unwrap();
    ...
    match row2.try_get::<_, String>("ts") {
        Ok(ts_str) => {
            assert!(
                ts_str.contains('-'),
                "CURRENT_TIMESTAMP should return formatted timestamp"
            );
        }
        Err(e) => {
...
async fn test_current_date_function() {
    ...
    let row = client
        .query_one("SELECT CAST(CURRENT_DATE AS TEXT) as today", &[])
        .await
        .unwrap();
...
async fn test_date_trunc_function_direct() {
    ...
    let row = client
        .query_one(
            &format!(
                "SELECT DATE_TRUNC('hour', {test_timestamp_micros}) as hour_trunc,
...
async fn test_interval_arithmetic_direct() {
    ...
    let row = client
        .query_one(
            &format!(
                "SELECT {test_timestamp_micros} + INTERVAL '1 day' as tomorrow,
