...
static CREATE_TYPE_ENUM_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)CREATE\s+TYPE\s+(\w+)\s+AS\s+ENUM\s*\((.*)\)").unwrap());
...
static DROP_TYPE_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)DROP\s+TYPE\s+(?:IF\s+EXISTS\s+)?(\w+)(?:\s+CASCADE)?").unwrap());
...
impl EnumDdlHandler {
    ...
    pub fn handle_enum_ddl(conn: &mut Connection, query: &str) -> Result<(), PgSqliteError> {
        let upper = query.trim().to_uppercase();
        ...
        if upper.starts_with("CREATE TYPE") && upper.contains("AS ENUM") {
            ...
            Err(PgSqliteError::Protocol(
                "Not an ENUM DDL statement".to_string(),
            ))
        }
    }

    /// Handle CREATE TYPE ... AS ENUM
    fn handle_create_type_enum(conn: &mut Connection, query: &str) -> Result<(), PgSqliteError> {
        // Parse the CREATE TYPE statement
        let (type_name, values) = Self::parse_create_type_enum(query)?;

        info!(
            "Creating ENUM type '{}' with {} values",
            type_name,
            values.len()
        );
        ...
        info!(
            "Successfully created ENUM type '{}' with OID {}",
            type_name, type_oid
        );
        Ok(())
    }
    ...
    fn parse_create_type_enum(query: &str) -> Result<(String, Vec<String>), PgSqliteError> {
        ...
        let captures = CREATE_TYPE_ENUM_REGEX.captures(query).ok_or_else(|| {
            PgSqliteError::Protocol("Invalid CREATE TYPE AS ENUM syntax".to_string())
        })?;
                ...
                _ => {
                    ...
                    if !ch.is_whitespace() {
                        return Err(PgSqliteError::Protocol(format!(
                            "Invalid character '{ch}' outside quotes"
                        )));
                    }
                }
            }
    ...
    fn handle_alter_type(conn: &mut Connection, query: &str) -> Result<(), PgSqliteError> {
        ...
        if let Some(captures) = ALTER_TYPE_ADD_VALUE_REGEX.captures(query) {
            ...
            let type_oid = EnumMetadata::get_enum_type(conn, type_name)
                ...
                .ok_or_else(|| {
                    PgSqliteError::Protocol(format!("Type '{type_name}' does not exist"))
                })?
                .type_oid;
            ...
            match position {
                ...
                _ => EnumMetadata::add_enum_value(conn, type_name, new_value, None, None),
            }
            ...
            info!(
                "Successfully added value '{}' to ENUM type '{}'",
                new_value, type_name
            );
            return Ok(());
        }
        ...
        Err(PgSqliteError::Protocol(
            "Unsupported ALTER TYPE operation".to_string(),
        ))
    }

    /// Handle DROP TYPE statements
    fn handle_drop_type(conn: &mut Connection, query: &str) -> Result<(), PgSqliteError> {
        // Parse DROP TYPE
        let captures = DROP_TYPE_REGEX
            .captures(query)
        ...
        if let Some(et) = enum_type {
            ...
            if !cascade {
                ...
                let mut stmt = conn.prepare(check_sql).map_err(|e| {
                    PgSqliteError::Protocol(format!("Failed to prepare dependency check: {e}"))
                })?;

                let dependent_tables: Vec<String> = stmt
                    .query_map([type_name], |row| row.get::<_, String>(0))
                    .map_err(|e| {
                        PgSqliteError::Protocol(format!("Failed to check dependencies: {e}"))
                    })?
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(|e| {
                        PgSqliteError::Protocol(format!("Failed to collect dependencies: {e}"))
                    })?;

                if !dependent_tables.is_empty() {
                    return Err(PgSqliteError::Protocol(format!(
                        "cannot drop type {type_name} because other objects depend on it"
                    )));
                }
            } else {
                ...
                let mut stmt = conn.prepare(usage_sql).map_err(|e| {
                    PgSqliteError::Protocol(format!("Failed to prepare usage query: {e}"))
                })?;

                let usages: Vec<(String, String)> = stmt
                    ...
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(|e| {
                        PgSqliteError::Protocol(format!("Failed to collect usage: {e}"))
                    })?;
                ...
                conn.execute(
                    "DELETE FROM __pgsqlite_enum_usage WHERE enum_type = ?1",
                    [type_name],
                )
...
mod tests {
    ...
    #[test]
    fn test_is_enum_ddl() {
        assert!(EnumDdlHandler::is_enum_ddl(
