...
async fn test_numeric_literal_validation() {
    ...
    client
        .execute("UPDATE prepared_test SET amount = 999.99 WHERE id = 1", &[])
        .await
        .unwrap();
...
async fn test_numeric_transaction_behavior() {
    ...
    // Insert initial data
    client
        .execute("INSERT INTO tx_test VALUES (1, 1000.00)", &[])
        .await
        .unwrap();
...
async fn test_numeric_error_messages() {
    ...
    assert!(
        err_str.contains("numeric") || err_str.contains("NUMERIC"),
...
async fn test_numeric_mixed_operations() {
    ...
    // Valid insert
    client
        .execute("INSERT INTO mixed_test VALUES (0, 100)", &[])
        .await
        .unwrap();

    // Invalid insert - should fail
    let result = client
        .execute("INSERT INTO mixed_test VALUES (1, 100000)", &[])
        .await;
    assert!(result.is_err());

    // Valid insert
    client
        .execute("INSERT INTO mixed_test VALUES (2, 200)", &[])
        .await
        .unwrap();

    // Invalid insert - should fail
    let result = client
        .execute("INSERT INTO mixed_test VALUES (3, 100000)", &[])
        .await;
    assert!(result.is_err());

    // Valid insert
    client
        .execute("INSERT INTO mixed_test VALUES (4, 400)", &[])
        .await
        .unwrap();
...
async fn test_numeric_with_computed_expressions() {
    ...
    // Insert test data
    client
        .execute("INSERT INTO prices VALUES (1, 100.00, 0.15)", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT total::text FROM orders WHERE id = 1", &[])
        .await
        .unwrap();
...
async fn test_numeric_special_values() {
    ...
    // Leading zeros
    client
        .execute("INSERT INTO special_test VALUES (2, 00123.45)", &[])
        .await
        .unwrap();

    // Positive sign
    client
        .execute("INSERT INTO special_test VALUES (3, +456.78)", &[])
        .await
        .unwrap();

    // Very small number
    client
        .execute("INSERT INTO special_test VALUES (4, 0.01)", &[])
        .await
        .unwrap();

    // Verify all values
    let rows = client
        .query("SELECT id, value::text FROM special_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_numeric_with_default_values() {
    ...
    client
        .execute("INSERT INTO defaults_test (id) VALUES (1)", &[])
        .await
        .unwrap();
...
async fn test_numeric_cast_operations() {
    ...
    let row = client
        .query_one("SELECT numeric_val::text FROM cast_test WHERE id = 3", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT numeric_val::text FROM cast_test WHERE id = 2", &[])
        .await
        .unwrap();
