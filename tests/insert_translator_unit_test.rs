...
async fn test_insert_execution_path() {
    ...
    let query =
        "INSERT INTO test_table (id, date_col, time_col) VALUES (1, '2024-01-15', '14:30:00')";
    ...
    // Check storage type
    let type_check = server
        .client
        .simple_query("SELECT typeof(date_col), typeof(time_col) FROM test_table WHERE id = 1")
        .await
        .unwrap();

    if let Some(row) = type_check
        .into_iter()
    ...
    let value_check = server
        .client
        .simple_query("SELECT date_col, time_col FROM test_table WHERE id = 1")
        .await
        .unwrap();

    if let Some(row) = value_check
        .into_iter()
        .find(|m| matches!(m, tokio_postgres::SimpleQueryMessage::Row(_)))
        && let tokio_postgres::SimpleQueryMessage::Row(data) = row
    {
        ...
        assert_eq!(
            ...
            "Date should be converted back to string"
        );
        assert_eq!(
            time_value, "14:30:00",
            "Time should be converted back to string"
        );
