...
async fn setup_large_dataset(client: &Client) {
    ...
    for batch in 0..(total_records / batch_size) {
        ...
        for i in 0..batch_size {
            ...
            values_clause.push_str(&format!("(${}, ${}, ${})", i * 3 + 1, i * 3 + 2, i * 3 + 3));

            batch_data.push((
                record_id % 10,
                format!("Data record {record_id}"),
        ...
        let query =
            format!("INSERT INTO large_dataset (category, data, value) VALUES {values_clause}");
        client
...
async fn benchmark_memory_efficiency(client: &Client) {
    ...
    let rows = client
        .query("SELECT * FROM large_dataset ORDER BY id", &[])
        .await
    ...
    let stmt = client
        .prepare("SELECT * FROM large_dataset ORDER BY id")
        .await
    ...
    println!(
        "  💾 Peak memory per chunk: ~{:.2} MB",
        (chunk_size * 200) as f64 / 1_000_000.0
    );

    // Calculate efficiency metrics
    let memory_reduction =
        ((row_count * 200) as f64 - (chunk_size * 200) as f64) / (row_count * 200) as f64 * 100.0;
    ...
    println!(
        "  💾 Memory reduction: {:.1}% (from ~{:.2}MB to ~{:.2}MB peak)",
        memory_reduction,
        (row_count * 200) as f64 / 1_000_000.0,
        (chunk_size * 200) as f64 / 1_000_000.0
    );
    println!(
        "  ⚡ Time per row - Full: {time_per_row_full:.2}μs, Portal: {time_per_row_portal:.2}μs"
    );

    if time_per_row_portal < time_per_row_full {
        println!(
            "  🚀 Portal approach is {:.1}x faster per row",
            time_per_row_full / time_per_row_portal
        );
    } else {
        println!(
            "  ⚠️  Portal approach is {:.1}x slower per row (but uses {:.1}% less memory)",
            time_per_row_portal / time_per_row_full,
            memory_reduction
        );
    }
}
...
async fn benchmark_partial_fetching(client: &Client) {
    ...
    for &result_size in &test_sizes {
        ...
        for &chunk_size in &chunk_sizes {
            ...
            let stmt = client
                .prepare(&format!(
                    "SELECT * FROM large_dataset ORDER BY id LIMIT {chunk_size}"
                ))
                .await
...
async fn benchmark_concurrent_portals() {
    ...
    for &portal_count in &portal_counts {
        ...
        let avg_portal_time: Duration =
            results.iter().map(|(_, time, _)| *time).sum::<Duration>() / portal_count as u32;
        ...
        println!(
            "  🚀 Concurrency efficiency: {:.1}x",
            avg_portal_time.as_micros() as f64 / total_time.as_micros() as f64
                * portal_count as f64
        );
        println!();
    }
...
async fn benchmark_protocol_comparison(client: &Client) {
    ...
    let test_queries = vec![
        (
            "Small result set",
            "SELECT * FROM large_dataset WHERE category = 0 LIMIT 10",
        ),
        (
            "Medium result set",
            "SELECT * FROM large_dataset WHERE category < 3 LIMIT 100",
        ),
        (
            "Large result set",
            "SELECT * FROM large_dataset WHERE category < 5 LIMIT 1000",
        ),
    ];
    ...
    for (test_name, query) in test_queries {
        ...
        let extended_rows = client
            .query(&stmt, &[])
            .await
            .expect("Extended query failed");
        let extended_time = start.elapsed();

        println!(
            "  📋 Simple Protocol:   {:?} ({} rows)",
            simple_time,
            simple_rows.len()
        );
        println!(
            "  🔧 Extended Protocol: {:?} ({} rows)",
            extended_time,
            extended_rows.len()
        );
...
async fn benchmark_resource_management(client: &Client) {
    ...
    println!(
        "  🏗️  Portal creation: {:?} ({:.2}μs per portal)",
        creation_time,
        creation_time.as_micros() as f64 / portal_operations as f64
    );
    println!(
        "  ⚡ Portal execution: {:?} ({:.2}μs per operation)",
        execution_time,
        execution_time.as_micros() as f64 / portal_operations as f64
    );
    println!("  📊 Total rows processed: {total_rows}");
    println!(
        "  🎯 Average rows per portal: {:.1}",
        total_rows as f64 / portal_operations as f64
    );
    ...
    for chunk_size in chunk_sizes {
        ...
        for i in 0..iterations {
            ...
            if i % (iterations / 10) == 0 {
                println!(
                    "  Progress: {}/{} chunks, {} rows, chunk time: {:?}",
                    i + 1,
                    iterations,
                    total_fetched,
                    chunk_time
        ...
        println!(
            ...
            peak_memory_estimate as f64 / 1_000_000.0
        );
        println!(
...
async fn setup_stress_dataset(client: &Client) {
    client
        ...
        .await
        .expect("Failed to create stress table");
    ...
    for batch in 0..(total_records / batch_size) {
        ...
        let query = format!(
            "INSERT INTO stress_dataset (data1, data2, data3, value1, value2) VALUES {}",
            values.join(", ")
        );
        client
            .execute(&query, &params)
            .await
            .expect("Failed to insert stress batch");
