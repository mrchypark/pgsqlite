...
mod tests {
    ...
    fn test_load_constraints() {
        ...
        validator
            .load_table_constraints(&conn, "test_table")
            .unwrap();
    ...
    fn test_validate_valid_values() {
        ...
        assert!(
            validator
                .validate_value("test_table", "amount", "123.45")
                .is_ok()
        );
        assert!(
            validator
                .validate_value("test_table", "amount", "0.99")
                .is_ok()
        );
        assert!(
            validator
                .validate_value("test_table", "amount", "99999999.99")
                .is_ok()
        );
    ...
    fn test_format_value() {
        ...
        assert_eq!(
            validator.format_value("test_table", "amount", "123"),
            "123.00"
        );
        assert_eq!(
            validator.format_value("test_table", "amount", "123.4"),
            "123.40"
        );
        assert_eq!(
            validator.format_value("test_table", "amount", "123.456"),
            "123.46"
        ); // Rounded
    }
}
