...
async fn test_protocol_overhead_breakdown() {
    ...
    db.execute_with_session(
        "CREATE TABLE protocol_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)",
        &session_id,
    )
    .await
    ...
    for batch in 0..100 {
        for i in 0..10 {
            let query = format!(
                "INSERT INTO protocol_test (name, value) VALUES ('batch{}', {})",
                batch * 10 + i,
                i * 100
            );
            db.execute_with_session(&query, &session_id)
    ...
    println!(
        "  100 INSERTs without transaction: {:?}, avg: {:?}",
        no_txn,
        no_txn / 100
    );
    println!(
        "  100 INSERTs with transaction: {:?}, avg: {:?}",
        with_txn,
        with_txn / 100
    );
    ...
    println!(
        "Estimated protocol overhead: ~{:?}",
        Duration::from_micros(200) - direct_avg
    );
...
async fn test_connection_handling_overhead() {
    ...
    for i in 0..100 {
        db.execute_with_session(
            &format!("INSERT INTO concurrent_test (value) VALUES ({i})"),
            &session_id,
        )
        .await
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            db.create_session_connection(thread_session_id)
                .await
                .expect("Failed to create session connection");
    ...
    println!(
        "  Single-threaded (100 INSERTs): {:?}, avg: {:?}",
        single_time,
        single_time / 100
    );
    println!(
        "  Multi-threaded (10x10 INSERTs): {:?}, avg: {:?}",
        multi_time,
        multi_time / 100
    );
    println!(
        "  Contention factor: {:.2}x",
        multi_time.as_secs_f64() / single_time.as_secs_f64()
    );
