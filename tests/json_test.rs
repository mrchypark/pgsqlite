...
async fn test_json_support() {
    ...
    let result = client
        .simple_query(r#"SELECT json_valid('{"valid": true}') as is_valid"#)
        .await
        .unwrap();
    let valid = result
        .iter()
    ...
    // Test invalid JSON
    let result = client
        .simple_query(r#"SELECT json_valid('{invalid json}') as is_valid"#)
        .await
        .unwrap();
    let invalid = result
        .iter()
    ...
    let result = client
        .simple_query(r#"SELECT json_typeof('{"key": "value"}') as type"#)
        .await
        .unwrap();
    let json_type = result
        .iter()
        .find_map(|msg| match msg {
    ...
    let result = client
        .simple_query(r#"SELECT json_array_length('[1, 2, 3, 4, 5]') as length"#)
        .await
        .unwrap();
    let length = result
        .iter()
    ...
    let result = client
        .simple_query(r#"SELECT to_json('hello world') as json_str"#)
        .await
        .unwrap();
    let json_str = result
        .iter()
    ...
    let result = client
        .simple_query("SELECT id, config, settings FROM test_json WHERE id = 1")
        .await
        .unwrap();
    result
        .iter()
