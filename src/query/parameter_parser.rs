...
impl ParameterParser {
    ...
    pub fn cast_type_to_oid(type_name: &str) -> i32 {
        match type_name.to_uppercase().as_str() {
            ...
            "TIMESTAMPTZ" | "TIMESTAMP WITH TIME ZONE" => {
                crate::types::PgType::Timestamptz.to_oid()
            }
    ...
    pub fn substitute_python_parameters(
        sql: &str,
...
mod tests {
    ...
    fn test_parameter_counting() {
        ...
        assert_eq!(
            ParameterParser::count_parameters("SELECT * FROM users WHERE id = $1"),
            1
        );
        assert_eq!(
            ParameterParser::count_parameters("SELECT * FROM users WHERE id = $1 AND name = $2"),
            2
        );

        // Parameters in string literals should be ignored
        assert_eq!(
            ParameterParser::count_parameters("SELECT data->>'$.items[0]' FROM users"),
            0
        );
        assert_eq!(
            ParameterParser::count_parameters("SELECT data->>'$1' FROM users"),
            0
        );

        // Mixed case: real parameter and $ in string literal
        assert_eq!(
            ParameterParser::count_parameters(
                "SELECT data->>'$.items[0]' FROM users WHERE id = $1"
            ),
            1
        );
        assert_eq!(
            ParameterParser::count_parameters("SELECT data->>'$1' FROM users WHERE id = $2"),
            1
        );
    }
    ...
    fn test_cast_type_helpers() {
        ...
        assert_eq!(
            ParameterParser::cast_type_to_oid("INTEGER"),
            crate::types::PgType::Int4.to_oid()
        );
        assert_eq!(
            ParameterParser::cast_type_to_oid("DECIMAL"),
    ...
    fn test_parameter_finding() {
        let params =
            ParameterParser::find_parameters("SELECT * FROM users WHERE id = $1 AND name = $3");
        assert_eq!(params, vec![1, 3]);

        // Parameters in string literals should be ignored
        let params =
            ParameterParser::find_parameters("SELECT data->>'$1' FROM users WHERE id = $2");
    ...
    fn test_parameter_substitution() {
        ...
        assert_eq!(
            result,
            "SELECT data->>'$.items[0]' FROM users WHERE id = 42"
        );
    ...
    fn test_escaped_quotes() {
        ...
        assert_eq!(
            result,
            "SELECT 'It''s a test with $1' FROM users WHERE id = 42"
        );
    }
    ...
    fn test_double_quoted_identifiers() {
        ...
        let params =
            ParameterParser::find_parameters("SELECT \"column_$1\" FROM users WHERE id = $2");
        assert_eq!(params, vec![2]);
    ...
    fn test_json_path_dollar_signs() {
        ...
        assert_eq!(
            count, 0,
            "JSON path '$.items[0]' should not be counted as parameter"
        );
        ...
        assert_eq!(
            count, 0,
            "JSON path '$1' should not be counted as parameter"
        );
        ...
        assert_eq!(
            count, 1,
            "Should count only the real parameter $1, not the JSON path"
        );

        let params =
            ParameterParser::find_parameters("SELECT data->>'$.items[0]' FROM users WHERE id = $1");
        ...
        assert_eq!(
            count, 1,
            "Should count only the real parameter, not JSON paths"
        );
        ...
        .unwrap();
        assert_eq!(
            result, "SELECT json_extract(config, '$.items[$1]') FROM users WHERE id = 42",
    ...
    fn test_python_parameter_parsing() {
        ...
        let params =
            ParameterParser::find_python_parameters("SELECT * FROM users WHERE id = %(user_id)s");
        assert_eq!(params, vec!["user_id"]);

        let params = ParameterParser::find_python_parameters(
        ...
        let params = ParameterParser::find_python_parameters(
            "SELECT 'test %(param)s' FROM users WHERE id = %(real_param)s",
        );
    ...
    fn test_python_parameter_substitution() {
        ...
        assert_eq!(
            result,
            "INSERT INTO users (name, id) VALUES ('John Doe', 42)"
        );
    ...
    fn test_python_parameter_counting() {
        assert_eq!(
            ParameterParser::count_python_parameters("SELECT * FROM users WHERE id = %(user_id)s"),
            1
        );
        assert_eq!(
            ParameterParser::count_python_parameters(
                "INSERT INTO users (name, email) VALUES (%(name)s, %(email)s)"
            ),
            2
        );
        ...
        // Parameters in string literals should be ignored
        assert_eq!(
            ParameterParser::count_python_parameters("SELECT 'test %(param)s' FROM users"),
            0
        );
        assert_eq!(
            ParameterParser::count_python_parameters(
