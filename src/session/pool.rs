...
impl PoolConnection {
    ...
    fn health_check(&mut self) -> Result<()> {
        match self
            .conn
            .prepare("SELECT 1")
            .and_then(|mut stmt| stmt.query_map([], |_row| Ok(()))?.next().unwrap_or(Ok(())))
        {
            Ok(_) => {
...
impl SqlitePool {
    ...
    async fn background_health_check(&self) {
        ...
        loop {
                    ...
                    match pool_conn.health_check() {
                        ...
                        Err(e) => {
                            ...
                            if pool_conn.failure_count >= 3 {
                                error!(
                                    "Removing unhealthy connection after {} failures",
                                    pool_conn.failure_count
                                );
                                to_remove.push(i);
                                stats.connections_dropped += 1;
                                stats.total_connections -= 1;
            ...
            // Update idle connection count. Active connections are tracked by acquire/drop.
            stats.idle_connections = conns.len();

            if !to_remove.is_empty() {
                debug!(
                    "Health check completed: removed {} connections, {} idle remaining",
                    to_remove.len(),
                    conns.len()
                );
            }
