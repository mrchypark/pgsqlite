...
async fn test_standalone_now_returns_formatted_timestamp() {
    ...
    assert!(
        diff < 3600,
        "NOW() should return a recent timestamp, but got {now_value} (diff: {diff} seconds)"
    );

    // Verify it has microsecond precision
    assert!(
        now_value.nanosecond() > 0,
        "NOW() should have sub-second precision"
    );

    // Also verify that when cast to text, it's properly formatted
    let row_text = client
        .query_one("SELECT CAST(NOW() AS TEXT)", &[])
        .await
        .unwrap();
    let now_text: String = row_text.get(0);
    assert!(
        now_text.contains('-') && now_text.contains(':'),
...
async fn test_standalone_current_timestamp_returns_formatted() {
    ...
    assert!(
        diff < 3600,
        "CURRENT_TIMESTAMP() should return a recent timestamp, but got {ts_value} (diff: {diff} seconds)"
    );

    // Verify it has microsecond precision
    assert!(
        ts_value.nanosecond() > 0,
        "CURRENT_TIMESTAMP() should have sub-second precision"
    );
}
...
async fn test_datetime_functions_with_table_context() {
    ...
    client
        .execute("CREATE TABLE test_ts (id INTEGER, ts TIMESTAMPTZ)", &[])
        .await
        .unwrap();

    // Insert using NOW()
    client
        .execute("INSERT INTO test_ts VALUES (1, NOW())", &[])
        .await
        .unwrap();

    // Select should return a proper timestamp (TIMESTAMPTZ type)
    let row = client
        .query_one("SELECT ts FROM test_ts WHERE id = 1", &[])
        .await
        .unwrap();
    let ts_value: DateTime<Utc> = row.get(0);
    ...
    assert!(
        diff < 3600,
        "Timestamp from table should be recent, but got {ts_value} (diff: {diff} seconds)"
    );
}
