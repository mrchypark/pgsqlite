...
async fn test_batch_insert_column_count_error() {
    ...
    assert!(
        err.to_string()
            .contains("all VALUES must have the same number of terms"),
...
async fn test_batch_insert_date_format_error() {
    ...
    let err = result.unwrap_err();
    assert!(
        err.to_string().contains("Invalid date value '01/15/2025'"),
        "Error should show invalid date: {err}"
    );
    assert!(
        err.to_string().contains("Expected format: YYYY-MM-DD"),
...
async fn test_batch_insert_time_format_error() {
    ...
    let err = result.unwrap_err();
    assert!(
        err.to_string().contains("Invalid time value '2:30 PM'"),
        "Error should show invalid time: {err}"
    );
    assert!(
        err.to_string().contains("Expected format: HH:MM:SS"),
        "Error should show expected format: {err}"
    );
...
async fn test_batch_insert_timestamp_format_error() {
    ...
    assert!(
        err.to_string()
            .contains("Invalid timestamp value '2025-01-02T15:45:00Z'"),
        "Error should show invalid timestamp: {err}"
    );
    assert!(
        err.to_string()
            .contains("Expected format: YYYY-MM-DD HH:MM:SS"),
        "Error should show expected format: {err}"
    );
