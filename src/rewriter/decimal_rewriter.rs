...
use super::implicit_cast_detector::{ImplicitCast, ImplicitCastDetector};
use crate::types::PgType;
use rusqlite::Connection;
use sqlparser::ast::{
    BinaryOperator, Cte, DataType, Expr, Function, FunctionArg, FunctionArgExpr,
    FunctionArgumentList, FunctionArguments, GroupByExpr, Ident, ObjectName, ObjectNamePart,
    OrderBy, OrderByKind, Query, SelectItem, SetExpr, Statement, TableFactor,
};
...
impl SchemaCache {
    ...
    fn cache_table_has_decimal(&mut self, table: &str, has_decimal: bool) {
        self.decimal_table_cache
            .insert(table.to_string(), has_decimal);
    }
}
...
impl<'a> DecimalQueryRewriter<'a> {
    ...
    fn any_table_has_decimal_columns(&mut self, tables: &[String]) -> bool {
        ...
        for table in tables {
            ...
            // Cache the result
            self.schema_cache
                .cache_table_has_decimal(table, has_decimal);
        ...
        match stmt {
            ...
            Statement::Update {
                ...
                if let sqlparser::ast::TableFactor::Table { name, .. } = &table.relation {
                    ...
                    let has_decimal_columns =
                        self.any_table_has_decimal_columns(std::slice::from_ref(&table_name));
                ...
                match &delete.from {
                    sqlparser::ast::FromTable::WithFromKeyword(table_list) => {
                        for table_with_joins in table_list {
                            self.extract_table_names_from_table_factor(
    ...
    fn rewrite_query_with_context(
        &mut self,
        query: &mut Query,
        outer_context: Option<&QueryContext>,
    ) -> Result<(), String> {
        ...
        if let Some(outer_ctx) = outer_context {
            ...
            for (cte_name, columns) in &outer_ctx.cte_columns {
                if !context.cte_columns.contains_key(cte_name) {
                    context
                        .cte_columns
                        .insert(cte_name.clone(), columns.clone());
                }
            }
    ...
    fn rewrite_cte_with_context(
            ...
            && let SetExpr::Select(base_select) = &**left
        {
            ...
            for table in &base_select.from {
                self.resolver.process_table_with_joins(
                    &table.relation,
                    &table.joins,
                    &mut base_context,
                );
            ...
            cte_context
                .cte_columns
                .insert(cte.alias.name.value.clone(), cte_column_types.clone());
        }
            ...
            SetExpr::Select(select) => {
                ...
                for item in &mut select.projection {
                    match item {
                        SelectItem::UnnamedExpr(expr) => {
                            if has_decimal_columns {
                                self.rewrite_expression_with_optimization(
                                    expr,
                                    &local_context,
                                    has_decimal_columns,
                                )?;
    ...
    fn rewrite_expression_with_optimization(
        &mut self,
        expr: &mut Expr,
        context: &QueryContext,
        ...
        match expr {
            Expr::BinaryOp { left, op, right } => {
                ...
                if should_use_decimal {
                    ...
                    *expr = self.create_decimal_function_expr(
                        op_clone,
                        left_clone,
                        right_clone,
                        left_type,
    ...
    fn create_decimal_function_expr(
        ...
        let wrapped_left = if let Some(cast) =
            ImplicitCastDetector::needs_implicit_cast(&left, left_type, &op, &right, right_type)
        {
                    ...
                    if expr == &left {
                        ...
                        if let Expr::Nested(inner) = &left {
                            if self.contains_arithmetic(inner) {
                                ...
                                if self
                                    .force_decimal_arithmetic(&mut arithmetic_expr, context)
                                    .is_err()
                                {
    ...
    fn wrap_in_decimal_from_text(&mut self, expr: Expr, context: &QueryContext) -> Expr {
        ...
        if let Expr::Nested(inner) = &expr {
            ...
            if self.contains_arithmetic(inner) {
                ...
                if self
                    .force_decimal_arithmetic(&mut unwrapped_expr, context)
                    .is_err()
                {
        ...
        let text_expr = match &expr {
            Expr::Value(val) => match &val.value {
                sqlparser::ast::Value::Number(_, _) => Expr::Cast {
                    expr: Box::new(expr),
            ...
            OrderByKind::Expressions(order_exprs) => {
                for order_expr in order_exprs {
                    ...
                    if let Expr::Identifier(_) | Expr::CompoundIdentifier(_) = &order_expr.expr {
                        ...
                        if expr_type == PgType::Numeric
                            || expr_type == PgType::Float4
                            || expr_type == PgType::Float8
                        {
    ...
    fn check_column_decimal_storage(
        &mut self,
        table: Option<&str>,
        column: &str,
        context: &QueryContext,
        ...
        if table_name == "__derived__"
            || context.cte_columns.contains_key(&table_name)
        ...
        match self
            .resolver
            .conn()
            .query_row(query, [&table_name, column], |_| Ok(()))
        {
            Ok(_) => {
                ...
                if let Ok(pg_type_str) =
                    self.resolver
                        .conn()
                        .query_row(pg_type_query, [&table_name, column], |row| {
                            row.get::<_, String>(0)
                        })
                {
    ...
    fn rewrite_expression_for_implicit_casts(
        &mut self,
        expr: &mut Expr,
        context: &QueryContext,
    ) -> Result<(), String> {
        match expr {
            Expr::BinaryOp { left, op, right } => {
                ...
                let needs_implicit_cast = ImplicitCastDetector::needs_implicit_cast(
    ...
    fn rewrite_aggregate_to_decimal(
        &mut self,
        func: &mut Function,
        context: &QueryContext,
    ) -> Result<(), String> {
    ...
    fn should_process_float_arithmetic(
        &mut self,
        left: &Expr,
        right: &Expr,
        context: &QueryContext,
    ...
    fn check_column_has_decimal_storage(&self, table: &str, column: &str) -> bool {
        ...
        if let Ok(sqlite_type) = self
            .resolver
            .conn()
            .query_row(query, [table, column], |row| {
    ...
    fn contains_float_returning_function(&self, expr: &Expr) -> bool {
        match expr {
            Expr::Function(func) => self.is_float_returning_math_function(&func.name),
    ...
    fn rewrite_expression_without_decimal_arithmetic(
        &mut self,
        expr: &mut Expr,
        context: &QueryContext,
    ) -> Result<(), String> {
        ...
        match expr {
            Expr::BinaryOp { left, op, right } => {
                ...
                if matches!(
                    ...
                    let forced_right_type =
                        if right_type == PgType::Numeric || left_type == PgType::Numeric {
