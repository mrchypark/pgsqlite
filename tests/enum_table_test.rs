...
fn test_create_table_with_enum() {
    ...
    // Create an ENUM type
    EnumDdlHandler::handle_enum_ddl(
        &mut conn,
    ...
    let (translated_sql, type_mappings) =
        CreateTableTranslator::translate_with_connection(create_table_sql, Some(&conn)).unwrap();
...
fn test_enum_check_constraint() {
    ...
    let (translated_sql, _) =
        CreateTableTranslator::translate_with_connection(create_table_sql, Some(&conn)).unwrap();
    conn.execute(&translated_sql, []).unwrap();

    // Valid insert should succeed
    conn.execute("INSERT INTO task (id, status) VALUES (1, 'active')", [])
        .unwrap();
    ...
    assert!(
        error_msg.contains("invalid input value for enum status") || error_msg.contains("ABORT")
    );
}
...
fn test_enum_with_quotes() {
    ...
    EnumDdlHandler::handle_enum_ddl(
        &mut conn,
        "CREATE TYPE quote_test AS ENUM ('it''s', 'quote\"test', 'normal')",
    )
    .unwrap();
