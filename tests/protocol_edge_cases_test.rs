...
async fn test_large_result_set() {
    ...
    let rows = client
        .query("SELECT id, data FROM large_data ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_large_parameter_values() {
    ...
    // Verify retrieval
    let row = client
        .query_one("SELECT large_text FROM blob_data WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT large_blob FROM blob_data WHERE id = 2", &[])
        .await
        .unwrap();
...
async fn test_mixed_format_modes() {
    ...
    let stmt = client
        .prepare("SELECT id, num, txt, bin FROM format_test WHERE id = $1")
        .await
        .unwrap();
...
async fn test_portal_suspension() {
    ...
    let rows = client
        .query("SELECT * FROM suspension_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_null_parameters() {
    ...
    let row = client
        .query_one(
            "SELECT opt_int, opt_text, opt_blob FROM null_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let row = client
        .query_one(
            "SELECT opt_int, opt_text, opt_blob FROM null_test WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
