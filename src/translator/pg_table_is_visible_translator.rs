...
impl PgTableIsVisibleTranslator {
    ...
    pub fn translate(query: &str) -> String {
        ...
        let pattern =
            Regex::new(r"(?i)\s+AND\s+(?:pg_catalog\.)?pg_table_is_visible\s*\([^)]+\)").unwrap();
...
mod tests {
    ...
    fn test_remove_pg_table_is_visible() {
        ...
        assert_eq!(
            translated,
            "SELECT pg_class.relname FROM pg_class WHERE pg_class.relname = 'test'"
        );
    }
    ...
    fn test_remove_pg_catalog_pg_table_is_visible() {
        ...
        assert_eq!(
            translated,
            "SELECT * FROM pg_class WHERE relkind = 'r' AND relname = 'test'"
        );
    }
