...
impl QueryMetrics {
    pub fn report(&self) -> String {
        ...
        report.push_str(&format!(
            "Protocol Parsing: {:.2}ms total, {:.2}µs avg ({}x)\n",
            parse_total,
            parse_avg,
            self.protocol_parse_count.load(Ordering::Relaxed)
        ));
        ...
        if self.cast_translation_count.load(Ordering::Relaxed) > 0 {
            report.push_str(&format!(
                "Cast Translation: {:.2}ms total, {:.2}µs avg ({}x)\n",
                cast_total,
                cast_avg,
                self.cast_translation_count.load(Ordering::Relaxed)
            ));
        ...
        if self.datetime_translation_count.load(Ordering::Relaxed) > 0 {
            report.push_str(&format!(
                "DateTime Translation: {:.2}ms total, {:.2}µs avg ({}x)\n",
                dt_total,
                dt_avg,
                self.datetime_translation_count.load(Ordering::Relaxed)
            ));
        ...
        if self.decimal_rewriting_count.load(Ordering::Relaxed) > 0 {
            report.push_str(&format!(
                "Decimal Rewriting: {:.2}ms total, {:.2}µs avg ({}x)\n",
                dec_total,
                dec_avg,
                self.decimal_rewriting_count.load(Ordering::Relaxed)
            ));
        ...
        let (cache_total, cache_avg) =
            format_timing(self.cache_lookup_ns.load(Ordering::Relaxed), cache_lookups);
        ...
        report.push_str(&format!(
            "SQLite Prepare: {:.2}ms total, {:.2}µs avg ({}x)\n",
            prepare_total,
            prepare_avg,
            self.sqlite_prepare_count.load(Ordering::Relaxed)
        ));
        ...
        report.push_str(&format!(
            "SQLite Execute: {:.2}ms total, {:.2}µs avg ({}x)\n",
            exec_total,
            exec_avg,
            self.sqlite_execute_count.load(Ordering::Relaxed)
        ));
        ...
        if self.type_resolution_count.load(Ordering::Relaxed) > 0 {
            report.push_str(&format!(
                "Type Resolution: {:.2}ms total, {:.2}µs avg ({}x)\n",
                type_total,
                type_avg,
                self.type_resolution_count.load(Ordering::Relaxed)
            ));
        ...
        report.push_str(&format!(
            "Result Formatting: {:.2}ms total, {:.2}µs avg ({}x)\n",
            fmt_total,
            fmt_avg,
            self.result_format_count.load(Ordering::Relaxed)
        ));
        ...
        report.push_str(&format!(
            "Protocol Serialization: {:.2}ms total, {:.2}µs avg ({}x)\n",
            ser_total,
            ser_avg,
            self.protocol_serialize_count.load(Ordering::Relaxed)
        ));
        ...
        report.push_str(&format!(
            "Fast Path: {fast_attempts} attempts, {fast_success} success ({fast_rate:.1}% rate)\n"
        ));
...
macro_rules! time_protocol_parse {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.protocol_parse_ns,
            &$crate::profiling::METRICS.protocol_parse_count,
            $op
        )
    };
}
...
macro_rules! time_cast_translation {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.cast_translation_ns,
            &$crate::profiling::METRICS.cast_translation_count,
            $op
        )
    };
}
...
macro_rules! time_datetime_translation {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.datetime_translation_ns,
            &$crate::profiling::METRICS.datetime_translation_count,
            $op
        )
    };
}
...
macro_rules! time_cache_lookup {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.cache_lookup_ns,
            &$crate::profiling::METRICS.cache_lookup_count,
            $op
        )
    };
}
...
macro_rules! time_sqlite_prepare {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.sqlite_prepare_ns,
            &$crate::profiling::METRICS.sqlite_prepare_count,
            $op
        )
    };
}
...
macro_rules! time_sqlite_execute {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.sqlite_execute_ns,
            &$crate::profiling::METRICS.sqlite_execute_count,
            $op
        )
    };
}
...
macro_rules! time_result_format {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.result_format_ns,
            &$crate::profiling::METRICS.result_format_count,
            $op
        )
    };
}
...
macro_rules! time_protocol_serialize {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.protocol_serialize_ns,
            &$crate::profiling::METRICS.protocol_serialize_count,
            $op
        )
    };
}
...
macro_rules! time_type_resolution {
    ($op:expr) => {
        $crate::time_operation!(
            &$crate::profiling::METRICS.type_resolution_ns,
            &$crate::profiling::METRICS.type_resolution_count,
            $op
        )
    };
}
