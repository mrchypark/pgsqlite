...
pub fn register_datetime_functions(conn: &Connection) -> Result<()> {
    ...
    conn.create_scalar_function("current_timestamp", 0, FunctionFlags::SQLITE_UTF8, |_ctx| {
        let now = Utc::now();
        Ok(now.format("%Y-%m-%d %H:%M:%S%.6f").to_string())
    })?;
    ...
    conn.create_scalar_function("current_time", 0, FunctionFlags::SQLITE_UTF8, |_ctx| {
        let now = Utc::now();
        let time = now.time();
        let micros =
            time.num_seconds_from_midnight() as i64 * 1_000_000 + (time.nanosecond() / 1000) as i64;
        Ok(micros)
    })?;

    // date_part(field, timestamp) / extract(field FROM timestamp)
    // Extract a specific part from a timestamp
    conn.create_scalar_function("date_part", 2, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function("age", 2, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function("age", 1, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function("to_timestamp", 1, FunctionFlags::SQLITE_UTF8, |ctx| {
        use rusqlite::types::ValueRef;
    ...
    conn.create_scalar_function("epoch", 0, FunctionFlags::SQLITE_UTF8, |_ctx| Ok(0i64))?;

    // make_date(year, month, day) - Create date from components
    conn.create_scalar_function("make_date", 3, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function("make_time", 3, FunctionFlags::SQLITE_UTF8, |ctx| {
    ...
    conn.create_scalar_function(
        ...
        |ctx| {
            ...
            let formats_with_tz = ["%Y-%m-%d %H:%M:%S%.f%:z", "%Y-%m-%d %H:%M:%S%:z"];
    ...
    conn.create_scalar_function("pg_date_from_text", 1, FunctionFlags::SQLITE_UTF8, |ctx| {
        use rusqlite::types::ValueRef;
    ...
    conn.create_scalar_function("pg_time_from_text", 1, FunctionFlags::SQLITE_UTF8, |ctx| {
        // Check if the parameter is already an integer (microseconds)
        if let Ok(micros) = ctx.get::<i64>(0) {
            // Already converted to microseconds, just return it
            return Ok(micros);
...
fn truncate_date(field: &str, timestamp: i64) -> Result<i64> {
    ...
    let truncated = match field.to_lowercase().as_str() {
        ...
        "week" => {
            ...
            let start_of_week =
                datetime.date_naive() - chrono::Duration::days(days_from_monday as i64);
...
mod tests {
    ...
    fn test_pg_timestamp_from_text() {
        ...
        for (input, expected) in test_cases {
            let result: i64 = conn
                .query_row("SELECT pg_timestamp_from_text(?)", [input], |row| {
                    row.get(0)
                })
