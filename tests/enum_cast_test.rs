...
async fn test_text_to_enum_cast() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE status AS ENUM ('active', 'inactive', 'pending')")
        .await
    ...
    client
        .simple_query("CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT, status status)")
        .await
        .expect("Failed to create table");

    // Test casting text to ENUM in INSERT
    client
        .simple_query("INSERT INTO items (id, name, status) VALUES (1, 'Item 1', 'active'::status)")
        .await
        .expect("Failed to insert with cast");

    // Test casting text to ENUM in SELECT
    let results = client
        .simple_query("SELECT 'pending'::status as casted_status")
        .await
    ...
    let results = client
        .simple_query("SELECT CAST('inactive' AS status) as casted_status")
        .await
...
async fn test_enum_to_text_cast() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE priority AS ENUM ('low', 'medium', 'high')")
        .await
        .expect("Failed to create ENUM type");

    // Create a table and insert data
    client
        .simple_query("CREATE TABLE tasks (id INTEGER PRIMARY KEY, title TEXT, priority priority)")
        .await
        .expect("Failed to create table");

    client
        .simple_query("INSERT INTO tasks VALUES (1, 'Task 1', 'high'), (2, 'Task 2', 'low')")
        .await
        .expect("Failed to insert tasks");

    // Test casting ENUM to text
    let results = client
        .simple_query("SELECT id, priority::text as priority_text FROM tasks ORDER BY id")
        .await
    ...
    for result in &results {
        if let tokio_postgres::SimpleQueryMessage::Row(row) = result {
            rows.push((row.get("id").unwrap(), row.get("priority_text").unwrap()));
        }
    }
...
async fn test_invalid_enum_cast() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE color AS ENUM ('red', 'green', 'blue')")
        .await
        .expect("Failed to create ENUM type");

    // Create a table
    client
        .simple_query("CREATE TABLE items (id INTEGER PRIMARY KEY, color color)")
        .await
        .expect("Failed to create table");

    // Test invalid value cast - should fail
    let result = client
        .simple_query("INSERT INTO items (id, color) VALUES (1, 'yellow'::color)")
        .await;
    ...
    if let Err(e) = result {
        ...
        assert!(
            error_msg.contains("CHECK constraint failed")
    ...
    let result = client
        .simple_query("SELECT 'purple'::color as invalid_color")
        .await;

    // This should succeed - the cast returns the value even if it's not valid
    assert!(
        result.is_ok(),
        "Cast in SELECT should succeed even with invalid value"
    );

    // But trying to use it in a WHERE clause against an enum column should fail
    let result = client
        .simple_query("SELECT * FROM items WHERE color = 'purple'::color")
        .await;
...
async fn test_enum_cast_in_where_clause() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE state AS ENUM ('draft', 'published', 'archived')")
        .await
        .expect("Failed to create ENUM type");

    // Create table and insert data
    client
        .simple_query("CREATE TABLE posts (id INTEGER PRIMARY KEY, title TEXT, state state)")
        .await
        .expect("Failed to create table");
    ...
    let results = client
        .simple_query("SELECT id, title FROM posts WHERE state = 'published'::state ORDER BY id")
        .await
...
async fn test_enum_cast_with_functions() {
    ...
    // Create an ENUM type
    client
        .simple_query("CREATE TYPE mood AS ENUM ('happy', 'sad', 'neutral')")
        .await
        .expect("Failed to create ENUM type");

    // Test concatenation with cast
    let results = client
        .simple_query("SELECT 'I am ' || 'happy'::mood::text as message")
        .await
    ...
    let results = client
        .simple_query("SELECT UPPER('sad'::mood::text) as upper_mood")
        .await
