...
async fn test_portal_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db_handler
        .execute("CREATE TABLE test_portal (id INTEGER PRIMARY KEY, value TEXT)")
        .await?;
    for i in 1..=10 {
        db_handler
            .execute(&format!("INSERT INTO test_portal VALUES ({i}, 'value{i}')"))
            .await?;
    }
    ...
    session
        .portal_manager
        .update_execution_state("test_portal", 1, false, Some(cached_result))?;
...
async fn test_multiple_concurrent_portals() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=5 {
        ...
        session
            .portal_manager
            .create_portal(format!("portal{i}"), portal)?;
    }
    ...
    let state1 = session
        .portal_manager
        .get_execution_state("portal1")
        .unwrap();
    let state2 = session
        .portal_manager
        .get_execution_state("portal2")
        .unwrap();
...
async fn test_portal_cleanup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    session
        .portal_manager
        .create_portal("test_portal".to_string(), portal)?;
    session
        .portal_manager
        .update_execution_state("test_portal", 0, false, None)?;
    ...
    assert!(
        session
            .portal_manager
            .get_execution_state("test_portal")
            .is_some()
    );
...
async fn test_stale_portal_cleanup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let removed = session
        .portal_manager
        .cleanup_stale_portals(std::time::Duration::from_millis(40));
...
async fn test_portal_with_values() -> Result<(), Box<dyn std::error::Error>> {
    ...
    session
        .portal_manager
        .create_portal("param_portal".to_string(), portal)?;
    ...
    session
        .portal_manager
        .update_execution_state("param_portal", 0, false, Some(cached_result))?;

    // Verify state
    let state = session
        .portal_manager
        .get_execution_state("param_portal")
        .unwrap();
    ...
    session
        .portal_manager
        .update_execution_state("param_portal", 1, false, None)?;
