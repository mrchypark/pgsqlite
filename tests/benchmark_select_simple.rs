...
async fn benchmark_select_simple() {
    ...
    let test_queries = vec![
        ...
        (
            "SELECT * FROM test_select WHERE id = 5000",
            "Primary key lookup",
        ),
        (
            "SELECT * FROM test_select WHERE value = 500 LIMIT 10",
            "Indexed column search",
        ),
        ("SELECT COUNT(*) FROM test_select", "Aggregate query"),
        (
            "SELECT id, name FROM test_select LIMIT 100",
            "Specific columns",
        ),
        (
            "SELECT * FROM test_select ORDER BY score DESC LIMIT 10",
            "ORDER BY query",
        ),
    ];
