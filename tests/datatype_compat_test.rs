...
async fn test_numeric_types() {
    ...
    client
        .execute("INSERT INTO numeric_test VALUES (1, 32767)", &[])
        .await
        .unwrap();

    // Test smallint - should use proper PostgreSQL type from metadata
    let row = client
        .query_one("SELECT small_int FROM numeric_test WHERE id = 1", &[])
        .await
        .unwrap();
...
async fn test_boolean_representations() {
    ...
    let rows = client
        .query("SELECT id, flag FROM bool_test ORDER BY id", &[])
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT id FROM bool_test WHERE flag = true", &[])
        .await
        .unwrap();
...
async fn test_null_handling() {
    ...
    let row = client
        .query_one(
            "SELECT nullable_int, nullable_text, nullable_real FROM null_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    // Test IS NULL
    let rows = client
        .query("SELECT id FROM null_test WHERE nullable_int IS NULL", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 2);
    ...
    let rows = client
        .query(
            "SELECT id FROM null_test WHERE nullable_text IS NOT NULL",
            &[],
        )
        .await
        .unwrap();
    ...
    let row = client
        .query_one(
            "SELECT COALESCE(nullable_int, 0) FROM null_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT COUNT(*), COUNT(nullable_int) FROM null_test", &[])
        .await
        .unwrap();
...
async fn test_text_and_string_types() {
    ...
    let row = client
        .query_one("SELECT short_text FROM text_test WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    // Test empty strings
    let row = client
        .query_one("SELECT short_text FROM text_test WHERE id = 2", &[])
        .await
        .unwrap();
    let val: String = row.get(0);
    assert_eq!(val, "");
    ...
    let row = client
        .query_one(
            "SELECT short_text || ' ' || fixed_char FROM text_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT LENGTH(long_text) FROM text_test WHERE id = 1", &[])
        .await
        .unwrap();
    let val: i32 = row.get(0);
    ...
    let row = client
        .query_one(
            "SELECT UPPER(short_text), LOWER(fixed_char) FROM text_test WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT id FROM text_test WHERE long_text LIKE '%test%'",
            &[],
        )
        .await
        .unwrap();
...
async fn test_date_time_types() {
    ...
    let row = client
        .query_one("SELECT date_col FROM datetime_test WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT time_col FROM datetime_test WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT timestamp_col FROM datetime_test WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    if col.type_() == &tokio_postgres::types::Type::TEXT {
        ...
        assert!(
            epoch_days > 10957,
            "Epoch days should be after year 2000, got {epoch_days}"
        );
    } else if col.type_().oid() == 1082 {
        // DATE type OID
        let val: chrono::NaiveDate = row.get(0);
    ...
    let row = client
        .query_one("SELECT date('2024-01-15', '+1 day')", &[])
        .await
        .unwrap();
    let col = row.columns().first().unwrap();
    if col.type_() == &tokio_postgres::types::Type::TEXT {
        ...
        assert!(
            (epoch_days - expected_epoch_day).abs() <= 2,
...
async fn test_bytea_type() {
    ...
    let row = client
        .query_one("SELECT data FROM bytea_test WHERE id = 1", &[])
        .await
        .unwrap();
    ...
    client
        .execute("INSERT INTO bytea_test (id, data) VALUES (2, NULL)", &[])
        .await
        .unwrap();

    let row = client
        .query_one("SELECT data FROM bytea_test WHERE id = 2", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT LENGTH(data) FROM bytea_test WHERE id = 1", &[])
        .await
        .unwrap();
...
async fn test_type_coercion() {
    ...
    let row = client
        .query_one("SELECT CAST('123' AS INTEGER)", &[])
        .await
        .unwrap();
    let val: i32 = row.get(0);
    assert_eq!(val, 123);
    ...
    let row = client
        .query_one("SELECT CAST(3.14 AS INTEGER)", &[])
        .await
        .unwrap();
    let val: i32 = row.get(0);
    assert_eq!(val, 3);
...
async fn test_parameter_type_inference() {
    ...
    let row = client
        .query_one(
            "SELECT int_col, text_col, real_col FROM type_test WHERE id = 4",
            &[],
        )
        .await
        .unwrap();
...
async fn test_special_values() {
    ...
    let row = client
        .query_one("SELECT 9223372036854775807", &[])
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT -9223372036854775808", &[])
        .await
        .unwrap();
    let val: i64 = row.get(0);
