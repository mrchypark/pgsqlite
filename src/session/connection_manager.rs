...
impl ConnectionManager {
    ...
    pub fn create_connection(&self, session_id: Uuid) -> Result<(), PgSqliteError> {
        ...
        debug!(
            "Creating connection for session {} with path: {}",
            session_id, self.db_path
        );

        let conn =
            Connection::open_with_flags(&self.db_path, flags).map_err(PgSqliteError::Sqlite)?;
        ...
        // Initialize metadata
        crate::metadata::TypeMetadata::init(&conn).map_err(PgSqliteError::Sqlite)?;
        ...
        info!(
            "Created new connection for session {} (total connections: {})",
            session_id,
            connections.len()
        );
    ...
    pub fn execute_with_session<F, R>(&self, session_id: &Uuid, f: F) -> Result<R, PgSqliteError>
    where
        F: FnOnce(&Connection) -> Result<R, rusqlite::Error>,
    {
        ...
        let conn_arc = connections.get(session_id).ok_or_else(|| {
            PgSqliteError::Protocol(format!("No connection found for session {session_id}"))
        })?;
    ...
    pub fn remove_connection(&self, session_id: &Uuid) {
        ...
        if connections.remove(session_id).is_some() {
            info!(
                "Removed connection for session {} (remaining connections: {})",
                session_id,
                connections.len()
            );
        }
    ...
    pub fn refresh_all_other_connections(
        &self,
        excluding_session: &Uuid,
        ...
        for (session_id, conn_arc) in connections.iter() {
            ...
            match conn.execute_batch("BEGIN IMMEDIATE; ROLLBACK;") {
                Ok(_) => {
                    ...
                    match conn.query_row("PRAGMA wal_checkpoint(TRUNCATE)", [], |_row| Ok(())) {
                        Ok(_) => {
                            refresh_count += 1;
                            debug!(
                                "Refreshed WAL and transaction context for session {}",
                                session_id
                            );
                        }
                        Err(e) => {
                ...
                Err(e) => {
                    error_count += 1;
                    debug!(
                        "Failed to refresh transaction context for session {}: {}",
                        session_id, e
                    );
                }
            }
        ...
        debug!(
            "WAL refresh completed: {} success, {} errors, excluding session {}",
