...
impl Default for BufferPoolConfig {
    fn default() -> Self {
        Self {
            ...
            enable_monitoring: std::env::var("PGSQLITE_BUFFER_MONITORING").unwrap_or_default()
                == "1",
        }
    }
}
...
impl PooledBuffer {
    ...
    fn should_discard(&self, max_capacity: usize, cleanup_age: Duration) -> bool {
        ...
        self.buffer.capacity() > max_capacity || self.last_used.elapsed() > cleanup_age
    }
}
...
impl BufferPool {
    ...
    pub fn get_buffer(&self) -> PooledBytesMut {
        ...
        if let Some(mut pooled_buffer) = inner.buffers.pop_front() {
            ...
            if self.config.enable_monitoring {
                debug!(
                    "Reused buffer from pool: capacity={}B, pool_size={}",
                    pooled_buffer.buffer.capacity(),
                    inner.buffers.len()
                );
            }
    ...
    fn return_buffer(&self, buffer: BytesMut) {
        ...
        if buffer.capacity() > self.config.max_buffer_capacity {
            ...
            if self.config.enable_monitoring {
                debug!(
                    "Discarded oversized buffer: capacity={}B > max={}B",
                    buffer.capacity(),
        ...
        if self.config.enable_monitoring {
            debug!(
                "Returned buffer to pool: capacity={}B, pool_size={}",
                buffer_capacity,
                inner.buffers.len()
            );
        }
    ...
    fn cleanup_old_buffers(&self, inner: &mut BufferPoolInner) {
        ...
        if cleaned_count > 0 {
            ...
            if self.config.enable_monitoring {
                info!(
                    "Cleaned up {} old buffers from pool, new size: {}",
                    cleaned_count,
                    inner.buffers.len()
                );
            }
