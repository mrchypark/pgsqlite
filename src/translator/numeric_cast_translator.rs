...
static CAST_TO_NUMERIC_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?i)CAST\s*\(\s*([^)]+?)\s*AS\s*(?:NUMERIC|DECIMAL)\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*\)",
    )
    .unwrap()
});
...
impl NumericCastTranslator {
    ...
    pub fn translate_query(query: &str, _conn: &Connection) -> String {
        ...
        while let Some(captures) = CAST_TO_NUMERIC_REGEX.captures(&result) {
            ...
            tracing::info!(
                "Translating CAST({} AS NUMERIC({},{})) to numeric_cast",
                expr,
                precision,
                scale
            );
...
mod tests {
    ...
    fn test_cast_to_numeric_detection() {
        assert!(NumericCastTranslator::needs_translation(
            "SELECT CAST(text_val AS NUMERIC(10,2))"
        ));
        assert!(NumericCastTranslator::needs_translation(
    ...
    fn test_cast_translation() {
        ...
        assert_eq!(
            translated,
            "INSERT INTO t SELECT id, numeric_cast(amount, 5, 2) FROM s"
        );
    }
}
