...
async fn test_null_arithmetic_propagation() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE null_data (id INTEGER PRIMARY KEY, val1 REAL, val2 REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO null_data VALUES (1, 10.0, NULL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO null_data VALUES (2, NULL, 20.0)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO null_data VALUES (3, NULL, NULL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO null_data VALUES (4, 30.0, 40.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT val1 + val2 AS sum_result FROM null_data WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT val1 + val2 AS sum_result FROM null_data WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT val1 + val2 AS sum_result FROM null_data WHERE id = 3",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT val1 + val2 AS sum_result FROM null_data WHERE id = 4",
            &[],
        )
        .await
        .unwrap();
...
async fn test_null_with_constants() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE nullable (id INTEGER PRIMARY KEY, amount REAL)")
            .await
            .unwrap();
        db_handler
            ...
            .unwrap();
        db_handler
            .execute("INSERT INTO nullable VALUES (2, 100.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT amount * 1.1 AS with_tax FROM nullable WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT amount * 1.1 AS with_tax FROM nullable WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
...
async fn test_null_in_complex_expressions() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE complex_null (id INTEGER PRIMARY KEY, a REAL, b REAL, c REAL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO complex_null VALUES (1, 10.0, NULL, 5.0)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO complex_null VALUES (2, 10.0, 20.0, NULL)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO complex_null VALUES (3, 10.0, 20.0, 5.0)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT (a + b) * c AS result FROM complex_null WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT (a + b) * c AS result FROM complex_null WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT (a + b) * c AS result FROM complex_null WHERE id = 3",
            &[],
        )
        .await
        .unwrap();
...
async fn test_coalesce_with_arithmetic() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("INSERT INTO defaults VALUES (1, 100.0, 0.1)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO defaults VALUES (2, 200.0, NULL)")
            .await
            .unwrap();
...
async fn test_null_handling_extended_protocol() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        db_handler
            .execute("CREATE TABLE param_null (id INTEGER PRIMARY KEY, base_val DOUBLE PRECISION)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO param_null VALUES (1, 50.0)")
            .await
            .unwrap();
        db_handler
            .execute("INSERT INTO param_null VALUES (2, NULL)")
            .await
            .unwrap();
    ...
    let rows = client
        .query(
            "SELECT base_val + 10.0 AS total FROM param_null WHERE id = 1",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT base_val + 10.0 AS total FROM param_null WHERE id = 2",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query("SELECT CAST(50.0 + NULL AS REAL) AS total", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
    let result: Option<f64> = rows[0].get(0);
