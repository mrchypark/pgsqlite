use pgsqlite::session::{CachedQueryResult, DbHandler, Portal, SessionState};
use std::sync::Arc;
...
async fn setup_test_data(db_handler: &DbHandler) {
    ...
    for batch in 0..(record_count / batch_size) {
        ...
        let query = format!(
            "INSERT INTO large_table (id, data, value, category) VALUES {}",
            values.join(", ")
        );
...
async fn test_portal_chunked_fetch(db_handler: &DbHandler, session: &Arc<SessionState>) {
    ...
    session
        .portal_manager
        .create_portal(portal_name.clone(), portal)
    ...
    let first_result = db_handler
        .query(&format!(
            "SELECT * FROM large_table ORDER BY id LIMIT {chunk_size}"
        ))
        .await
        .expect("Failed to execute first chunk");
    ...
    session
        .portal_manager
        .update_execution_state(&portal_name, total_rows, false, Some(cached_result))
    ...
    println!(
        "  💾 Peak memory: ~{:.2} MB (vs full fetch)",
...
async fn test_portal_resource_management(session: &Arc<SessionState>) {
    ...
    let removed = session
        .portal_manager
        .cleanup_stale_portals(std::time::Duration::from_secs(0));
    ...
    println!(
        "  📊 Remaining portals: {}",
        session.portal_manager.portal_count()
    );
}
...
async fn test_concurrent_portals(session: &Arc<SessionState>) {
    ...
    for portal_id in 0..concurrent_count {
        ...
        let handle = tokio::spawn(async move {
            ...
            let portal = Portal {
                ...
                query: format!(
                    "SELECT count(*) FROM large_table WHERE id % {} = 0",
                    portal_id + 1
                ),
                translated_query: None,
                bound_values: vec![],
    ...
    let avg_portal_time: std::time::Duration = results
        .iter()
        .map(|(_, _, time)| *time)
        .sum::<std::time::Duration>()
        / concurrent_count as u32;
