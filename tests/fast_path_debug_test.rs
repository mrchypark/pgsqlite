...
async fn test_fast_path_with_metadata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let queries = vec![
        (
            "INSERT INTO products (name, price) VALUES ('test', 10.99)",
            false,
            "should not use fast path with DECIMAL",
        ),
        (
            "SELECT * FROM products",
            false,
            "should not use fast path with DECIMAL table",
        ),
        (
            "UPDATE products SET price = 20.99",
            false,
            "should not use fast path with DECIMAL",
        ),
    ];
...
async fn test_fast_path_decimal_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Insert data
    db_handler
        .execute("INSERT INTO numbers (price, quantity, weight) VALUES (10.99, 5, 2.5)")
        .await?;
    ...
    // Test arithmetic
    let calc_result = db_handler
        .query("SELECT price * quantity FROM numbers")
        .await?;
