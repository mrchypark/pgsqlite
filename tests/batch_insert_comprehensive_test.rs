...
async fn test_batch_insert_edge_cases() {
    ...
    let result = client
        .simple_query("INSERT INTO edge_test (id, name, value) VALUES")
        .await;
    assert!(result.is_err(), "Empty VALUES should fail");

    // Test 2: Single row in multi-row syntax (should work)
    let result = client
        .simple_query("INSERT INTO edge_test (id, name, value) VALUES (1, 'single', 100)")
        .await;
    ...
    // Verify count
    let count = client
        .query_one("SELECT COUNT(*) FROM edge_test", &[])
        .await
        .unwrap();
...
async fn test_batch_insert_with_nulls() {
    ...
    // Verify NULLs
    let rows = client
        .query("SELECT * FROM null_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_batch_insert_with_returning() {
    ...
    let result = client
        .simple_query("INSERT INTO returning_test (name) VALUES ('first') RETURNING id, name")
        .await;

    // SQLite RETURNING is supported - just check it succeeded
    assert!(
        result.is_ok(),
        "Single-row INSERT with RETURNING should work"
    );

    // Test that batch INSERT without RETURNING works
    let result = client
        .simple_query("INSERT INTO returning_test (name) VALUES ('second'), ('third')")
        .await;
    assert!(result.is_ok(), "Batch INSERT without RETURNING should work");

    // Verify all rows
    let rows = client
        .query("SELECT id, name FROM returning_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_batch_insert_constraint_violations() {
    ...
    let count = client
        .query_one("SELECT COUNT(*) FROM constraint_test", &[])
        .await
        .unwrap();
    let count_val: i64 = count.get(0);
...
async fn test_batch_insert_with_escaped_quotes() {
    ...
    // Verify data
    let rows = client
        .query("SELECT * FROM quote_test ORDER BY id", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 3);
...
async fn test_batch_insert_performance_comparison() {
    ...
    assert!(
        speedup > 5.0,
        "Batch should be at least 5x faster than single-row inserts"
    );
...
async fn test_batch_insert_mixed_value_types() {
    ...
    // Verify data
    let rows = client
        .query("SELECT * FROM mixed_test ORDER BY id", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 3);
