...
async fn profile_message_encoding_overhead(client: &tokio_postgres::Client) {
    ...
    let queries = vec![
        ...
        (
            "Small result",
            "SELECT id FROM profile_simple WHERE id = 1",
            1,
        ),
        (
            "Medium result",
            "SELECT * FROM profile_simple WHERE id < 10",
            10,
        ),
        (
            "Large result",
            "SELECT * FROM profile_simple WHERE id < 100",
            100,
        ),
        (
            "Complex types",
            "SELECT * FROM profile_complex WHERE id < 10",
            10,
        ),
    ];
    ...
    for (label, query, _expected_rows) in queries {
        ...
        if avg_rows > 0.0 {
            println!(
                "  Time per row: {:?}",
                Duration::from_secs_f64(avg_time.as_secs_f64() / avg_rows)
            );
        }
    }
}
...
async fn profile_value_conversion_overhead(client: &tokio_postgres::Client) {
    ...
    let type_queries = vec![
        (
            "INTEGER",
            "SELECT int_small, int_large FROM profile_complex WHERE id < 100",
        ),
        (
            "TEXT",
            "SELECT text_short, text_long FROM profile_complex WHERE id < 100",
        ),
        (
            "FLOAT/DOUBLE",
            "SELECT float_val, double_val FROM profile_complex WHERE id < 100",
        ),
        (
            "BOOLEAN",
            "SELECT bool_val FROM profile_complex WHERE id < 100",
        ),
        (
            "BYTEA",
            "SELECT bytes_small, bytes_large FROM profile_complex WHERE id < 100",
        ),
        (
            "NUMERIC",
            "SELECT numeric_val FROM profile_complex WHERE id < 100",
        ),
        (
            "TIMESTAMP",
            "SELECT timestamp_val FROM profile_complex WHERE id < 100",
        ),
    ];
...
async fn profile_protocol_vs_direct(client: &tokio_postgres::Client) {
    ...
    for i in 1000..1100 {
        let query = format!(
            "SELECT * FROM profile_simple WHERE id = {} -- {}",
            i % 100,
            i
        );
        let start = Instant::now();
    ...
    // Reused execution
    let reused_stmt = client
        .prepare("SELECT * FROM profile_simple WHERE id = $1")
        .await
        .unwrap();
    ...
    println!(
        "  Speedup: {:.1}x",
        average(&first_times).as_secs_f64() / average(&reused_times).as_secs_f64()
    );
}
...
async fn profile_cache_effectiveness(client: &tokio_postgres::Client) {
    ...
    println!(
        "  Speedup: {:.1}x",
        average(&cold_times).as_secs_f64() / average(&warm_times).as_secs_f64()
    );

    // Test row description cache
    let stmt1 = client
        .prepare("SELECT * FROM profile_complex WHERE id = $1")
        .await
        .unwrap();
    let stmt2 = client
        .prepare("SELECT * FROM profile_complex WHERE id = $1 -- cached")
        .await
        .unwrap();
...
async fn cleanup(client: &tokio_postgres::Client) {
    client
        .execute("DROP TABLE IF EXISTS profile_simple", &[])
        .await
        .ok();
    client
        .execute("DROP TABLE IF EXISTS profile_complex", &[])
        .await
        .ok();
}
