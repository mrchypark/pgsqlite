...
impl NumericValidator {
    ...
    pub fn validate_insert(conn: &Connection, sql: &str, table_name: &str) -> Result<(), PgError> {
        ...
        for (col_name, value) in insert_data.iter() {
            if let Some((precision, scale)) = constraints.get(col_name) {
                ...
                Self::validate_value(value, *precision, *scale).map_err(|mut e| {
    ...
    pub fn validate_update(conn: &Connection, sql: &str, table_name: &str) -> Result<(), PgError> {
        // First ensure constraints are loaded
        let _ = Self::load_table_constraints(conn, table_name);
...
fn parse_insert_statement(
    ...
    static MULTI_INSERT_NO_COLS_REGEX: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?si)INSERT\s+INTO\s+\w+\s+VALUES\s+(.+)").unwrap());
...
fn parse_update_statement(sql: &str) -> Option<Vec<(String, String)>> {
    ...
    static UPDATE_REGEX: Lazy<Regex> =
        Lazy::new(|| Regex::new(r"(?i)UPDATE\s+\w+\s+SET\s+(.+?)(?:\s+WHERE|$)").unwrap());
...
fn is_computed_expression(value: &str) -> bool {
    ...
    if trimmed.contains('(') || trimmed.contains("CASE") || trimmed.contains("SELECT") {
        return true;
    }
