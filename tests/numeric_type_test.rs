...
async fn test_numeric_text_parameters() {
    ...
    client
        .execute(
            "INSERT INTO numeric_test (id, amount, price) VALUES (1, '123.45', '9999.9999')",
            &[],
        )
        .await
        .unwrap();
    client
        .execute(
            "INSERT INTO numeric_test (id, amount, price) VALUES (2, '0', '0.0001')",
            &[],
        )
        .await
        .unwrap();
    client
        .execute(
            "INSERT INTO numeric_test (id, amount, price) VALUES (3, '-456.78', '-1234.5678')",
            &[],
        )
        .await
        .unwrap();
    client
        .execute(
            "INSERT INTO numeric_test (id, amount, price) VALUES (4, '999999.99', '0')",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT id, amount::text, price::text FROM numeric_test ORDER BY id",
            &[],
        )
        .await
        .unwrap();
...
async fn test_numeric_invalid_parameters() {
    ...
    let result1 = client
        .execute(
            "INSERT INTO numeric_test (id, amount) VALUES (1, 'not_a_number')",
            &[],
        )
        .await;
    let result2 = client
        .execute(
            "INSERT INTO numeric_test (id, amount) VALUES (2, '123.45.67')",
            &[],
        )
        .await;
    let result3 = client
        .execute(
            "INSERT INTO numeric_test (id, amount) VALUES (3, '12e34')",
            &[],
        )
        .await;
    ...
    let rows = client
        .query("SELECT COUNT(*) FROM numeric_test", &[])
        .await
        .unwrap();
...
async fn test_numeric_binary_format() {
    ...
    let rows = client
        .query("SELECT id, amount::text FROM numeric_test ORDER BY id", &[])
        .await
        .unwrap();
...
async fn test_numeric_arithmetic() {
    ...
    let rows = client
        .query(
            "SELECT id, price::text, quantity FROM prices ORDER BY id",
            &[],
        )
        .await
        .unwrap();
    ...
    let row = client
        .query_one("SELECT (SUM(price))::text as total_price FROM prices", &[])
        .await
        .unwrap();
    ...
    assert!(
        total_price.starts_with("136"),
        "Expected total price to start with 136, got: {total_price}"
    );
    ...
    let row = client
        .query_one("SELECT (AVG(price))::text as avg_price FROM prices", &[])
        .await
        .unwrap();
...
async fn test_numeric_type_in_ranges() {
    ...
    let rows = client
        .query(
            "SELECT id, price_range::text FROM range_test ORDER BY id",
            &[],
        )
        .await
        .unwrap();
