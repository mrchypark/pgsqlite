...
async fn benchmark_extended_protocol_parameters() {
        ...
        client
            .execute(
                ...
                &[
                    ...
                    &name,
                    &(i * 10),
                    &rust_decimal::Decimal::from_str(&format!("{i}.99")).unwrap(),
                ],
            )
            .await
    ...
    for i in 100..200 {
        ...
        client
            .execute(&query, &[&i, &name, &(i * 10), &price])
            .await
    ...
    for i in 200..300 {
        ...
        let start = Instant::now();
        client
            .execute(&stmt, &[&i, &name, &(i * 10), &price])
            .await
            .expect("Failed to execute");
    ...
    let inference_improvement = if total_cached_time < total_inference_time {
        let improvement = (total_inference_time.as_micros() as f64
            - total_cached_time.as_micros() as f64)
            / total_inference_time.as_micros() as f64
            * 100.0;
