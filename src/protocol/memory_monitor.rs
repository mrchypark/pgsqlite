...
impl Default for MemoryMonitorConfig {
    fn default() -> Self {
        Self {
            ...
            enable_detailed_monitoring: std::env::var("PGSQLITE_MEMORY_MONITORING")
                .unwrap_or_default()
                == "1",
        }
    }
...
impl MemoryMonitor {
    ...
    fn log_memory_status(&self, current_memory: u64, pressure: MemoryPressure) {
        ...
        match pressure {
            MemoryPressure::Low => {
                debug!(
                    "Memory usage: {:.1}MB (threshold: {:.1}MB)",
                    current_mb, threshold_mb
                );
            }
            MemoryPressure::Medium => {
                info!(
                    "Memory usage: {:.1}MB - approaching threshold ({:.1}MB)",
                    current_mb, threshold_mb
                );
            }
            MemoryPressure::High => {
                warn!(
                    "Memory usage: {:.1}MB - above threshold ({:.1}MB)",
                    current_mb, threshold_mb
                );
            }
            MemoryPressure::Critical => {
                warn!(
                    "Memory usage: {:.1}MB - CRITICAL level (threshold: {:.1}MB)",
                    current_mb, threshold_mb
                );
            }
        }
