...
async fn test_now_function() {
    ...
    assert!(
        now_str.contains('-'),
        "NOW() should return formatted timestamp with dashes"
    );
    assert!(
        now_str.contains(':'),
        "NOW() should return formatted timestamp with colons"
    );
    assert!(
        now_str.contains('.'),
        "NOW() should return formatted timestamp with microseconds"
    );
    assert!(
        now_str.len() > 20,
        "NOW() should return full timestamp string"
    );
}
        ...
        if let tokio_postgres::SimpleQueryMessage::Row(row) = msg {
            if let Some(date_str) = row.get(0) {
                ...
                assert_eq!(
                    date_str.len(),
                    10,
                    "CURRENT_DATE should return date in YYYY-MM-DD format"
                );
...
async fn test_datetime_functions_with_table() {
    ...
    let to_timestamp_test = client
        .simple_query("SELECT to_timestamp(ts) FROM timestamps WHERE id = 1")
        .await;
    ...
    match results {
        Ok(msgs) => {
            for msg in msgs {
                if let tokio_postgres::SimpleQueryMessage::Row(row) = msg {
                    eprintln!(
                        "SUCCESS: EXTRACT with to_timestamp returned year={:?}, month={:?}, day={:?}, hour={:?}, minute={:?}",
                        row.get(0),
                        row.get(1),
                        row.get(2),
                        row.get(3),
...
async fn test_date_trunc_with_table() {
    ...
    assert!(
        (hour_val - expected_hour).abs() < 1_000_000,
        "hour_trunc: expected {expected_hour}, got {hour_val}"
    );
    // 2023-06-15 00:00:00
    let expected_day = 1686787200i64 * 1_000_000;
    assert!(
        (day_val - expected_day).abs() < 1_000_000,
        "day_trunc: expected {expected_day}, got {day_val}"
    );
    // 2023-06-01 00:00:00
    let expected_month = 1685577600i64 * 1_000_000;
    assert!(
        (month_val - expected_month).abs() < 1_000_000,
        "month_trunc: expected {expected_month}, got {month_val}"
    );
}
...
async fn test_interval_arithmetic_with_table() {
    ...
    assert_eq!(
        tomorrow, expected_tomorrow,
        "tomorrow: expected {expected_tomorrow}, got {tomorrow}"
    );
    assert_eq!(
        hour_ago, expected_hour_ago,
