...
async fn benchmark_select_detailed() {
    ...
    conn.execute("CREATE INDEX idx_value ON select_test(value)", [])
        .unwrap();
    conn.execute("CREATE INDEX idx_name ON select_test(name)", [])
        .unwrap();
    ...
    let test_queries = vec![
        ...
        (
            ...
            "SELECT * FROM select_test WHERE id = 5000",
        ),
        (
            "SELECT with indexed column",
            "SELECT * FROM select_test WHERE value = 500",
        ),
        (
            "SELECT with text match",
            "SELECT * FROM select_test WHERE name = 'name_50'",
        ),
        (
            "SELECT with range",
            "SELECT * FROM select_test WHERE value BETWEEN 100 AND 200",
        ),
        ("SELECT count", "SELECT COUNT(*) FROM select_test"),
        (
            "SELECT with aggregation",
            "SELECT value, COUNT(*) FROM select_test GROUP BY value LIMIT 10",
        ),
        (
            "SELECT with ORDER BY",
            "SELECT * FROM select_test ORDER BY score DESC LIMIT 10",
        ),
        (
            "SELECT specific columns",
            "SELECT id, name, value FROM select_test LIMIT 100",
        ),
        (
            "SELECT with boolean filter",
            "SELECT * FROM select_test WHERE active = true LIMIT 100",
        ),
    ];
    ...
    for limit in &[1, 10, 100, 1000] {
        ...
        println!(
            ...
            limit,
            avg_time,
            avg_time.as_micros() as f64 / *limit as f64
        );
    }
